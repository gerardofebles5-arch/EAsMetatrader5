...
//|                                                                    |
//| MODIFICADO: SL basado en ATR(14) en lugar de breakoutLevel        |
//| MODIFICADO: Breakeven a 1.2R en lugar de 0.8R                     |
//|                                                                    |
//| Timeframe: M15                                                     |
//| Filtro: EMA200 H1                                                  |
...
//+------------------------------------------------------------------+
#property copyright "Breakout Pullback System - ATR SL"
...
input double InpBreakevenRR = 1.2;              // RR para activar breakeven (MODIFICADO)
input int InpATRPeriod = 14;                    // ATR período para SL (NUEVO)
input int InpMagicNumber = 120001;              // Magic Number

//--- Global Variables
CTrade trade;
int emaHandle;
int atrHandle;                                  // NUEVO: Handle ATR
double emaBuffer[];
double atrBuffer[];                             // NUEVO: Buffer ATR
...
int OnInit()
{
   ...
   // NUEVO: Crear indicador ATR en M15
   ...
   ArraySetAsSeries(atrBuffer, true);           // NUEVO
   ...
   Print("=== EA V12.0 BREAKOUT + PULLBACK (ATR SL) ===");
   Print("Símbolo: ", _Symbol);
   Print("Timeframe: M15");
   Print("Riesgo: ", InpRiskPercent, "%");
   Print("Risk:Reward: 1:", InpRiskReward);
   Print("SL: 1 ATR(", InpATRPeriod, ")");      // NUEVO
   Print("Breakeven: ", InpBreakevenRR, "R");   // NUEVO
...
void OnDeinit(const int reason)
{
   ...
   if(atrHandle != INVALID_HANDLE)              // NUEVO
...
//+------------------------------------------------------------------+
//| Abrir trade (MODIFICADO: SL basado en ATR)                        |
//+------------------------------------------------------------------+
void OpenTrade(ENUM_ORDER_TYPE orderType, double entryReference)
{
   ...
   // NUEVO: Obtener ATR de la vela cerrada
   ...
   // MODIFICADO: SL = 1 ATR (no usar breakoutLevel)
   ...
   // Calcular lote basado en riesgo (usando distancia real entry-SL)
   ...
   // Ejecutar orden
   string comment = StringFormat("BP_V12_ATR_RR%.1f", InpRiskReward);
   
   if(orderType == ORDER_TYPE_BUY) {
      if(trade.Buy(lotSize, _Symbol, entryPrice, sl, tp, comment)) {
         Print("✓ LONG abierto: Lote=", lotSize, " Entry=", entryPrice);
         Print("  SL=", sl, " (ATR=", DoubleToString(atr, 2), " | ", DoubleToString(slPips, 1), " pips)");
