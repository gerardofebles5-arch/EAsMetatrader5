//+------------------------------------------------------------------+
//|  MoneyMachine7 v8.32 — FUSIÓN POR DATOS REALES DE BACKTEST     |
//|                                                                  |
//|  GANADORES REALES POR MÉTRICA (backtest 2026.03.02-03, $5k):   |
//|  PF/WR/EV/Sharpe → v8.28 (bar-lock + H1 MA200 ✓)              |
//|  DD/ConsecLoss/AvgDur → v8.27 (DD 1.83%, max 14 consec losses) |
//|  LR Correlation → v8.30 (0.983, equity más recta)              |
//|                                                                  |
//|  FUSIÓN QUIRÚRGICA:                                             |
//|  1. BASE v8.28 → H1 MA200 + bar-lock + kb[1]                   |
//|     Aporta: PF=1.51, WR=31%, EV=$1.33, Sharpe=100, 100% longs  |
//|                                                                  |
//|  2. DE v8.27 → SL Sentinel (re-open inmediato post-SL)         |
//|     v8.27 tenía mejor DD (1.83%) y menos consec losses (14)    |
//|     Su mecanismo: re-open inmediato cuando precio cae al SL     |
//|     y K Stoch sigue <30 → re-abre sin esperar nueva barra      |
//|     Implementación: g_slForceReopen bool flag                   |
//|     StealthCheckAll detecta TP vs SL → SL sets flag=true        |
//|     OnTick: if(g_slForceReopen) ignora bar-lock → re-abre      |
//|     Efecto esperado: DD 2.21%→1.8%, MaxCL 22→14               |
//|                                                                  |
//|  3. DE v8.30 → ThrottlePostTP: lock 1 barra extra post-TP      |
//|     v8.30 tenía mejor LR Correlation (0.983) = equity más recta |
//|     Su equity recta vino de más trades con H1 trend limpio      |
//|     ThrottlePostTP: tras TP, g_lastSignalBar = bar[0] actual    |
//|     → bloquea barra actual + siguiente → ~120s post-TP         |
//|     → K sube completamente desde <30 → señal nueva más limpia  |
//|     Efecto esperado: WR sube 31%→35%+, AvgDur sube 28→35s     |
//+------------------------------------------------------------------+
...
datetime g_lastSignalBar=0;     // BAR-LOCK: datetime bar[1] cuando última señal legacy abrió
bool     g_slForceReopen=false; // v8.27: SL hit → re-open inmediato, ignora bar-lock
datetime g_lastTPBar=0;         // v8.30: post-TP lock extra (bar[0] al momento del TP)
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      bool cls=false; bool hitTP=false;
      if(g_sp[i].dir==1)
      {
         if(g_sp[i].vTP>0 && bid>=g_sp[i].vTP){cls=true;hitTP=true;}
         else if(g_sp[i].vSL>0 && bid<=g_sp[i].vSL)  cls=true;
      }
      else
      {
         if(g_sp[i].vTP>0 && ask<=g_sp[i].vTP){cls=true;hitTP=true;}
      ...
      if(cls && StealthClosePos(g_sp[i].ticket,g_sp[i].dir))
      {
         ...
         if(hitTP)
         {
            // FUSIÓN v8.30: ThrottlePostTP
            // Tras TP (+2.50pts): precio subió → K Stoch sube de <30 hacia >50
            // Esperar que K complete la subida antes de re-entrar
            // g_lastTPBar = bar[0] actual → OnTick bloquea hasta bar[2] completa (~120s)
            // Efecto: WR sube (entramos cuando K re-cruza <30 desde arriba, no mid-bounce)
            // AvgDuration sube de 28s → ~35-40s (más cerca del original 41s)
            g_lastTPBar    = iTime(g_sym,_Period,0); // barra actual al cierre TP
            g_slForceReopen = false;
         }
         else
         {
            // FUSIÓN v8.27: SL Sentinel
            // Tras SL (-0.77pts): precio bajó sólo 0.77pts → K Stoch apenas cambia (<30)
            // El original re-abre en <5s en 23.6% de casos post-SL
            // → señal sigue válida → re-abrir inmediatamente sin esperar nueva barra
            g_slForceReopen = true;
            g_lastTPBar     = 0; // limpiar post-TP lock si había uno
         }
      }
   }
}
...
void DailyReset()
{
   ...
   g_lastSignalBar=0;g_slForceReopen=false;g_lastTPBar=0;g_fvgLastScan=0;g_fvgCnt=0;
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.32 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.32 | FUSIÓN: v8.28(WR)+v8.27(DD)+v8.30(LR) | Mode=",ms,
         " | TrendMA=H1(",Trend_MA_Period,")"
         " | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED,
         " | SL-Sentinel ✓ | PostTP-Lock ✓");
...
void OnTick()
{
   ...
   {
      // ══ TRI-STATE BAR-LOCK v8.32 ══════════════════════════════════
      // Estado A — POST-SL (g_slForceReopen=true):
      //   Ignora bar-lock completamente → re-abre en siguiente tick
      //   Fuente: v8.27 (mejor DD 1.83%, max 14 consec losses)
      //   Efecto: replica el 23.6% de re-opens <5s del original
      //
      // Estado B — POST-TP (g_lastTPBar != 0):
      //   Bloquea hasta que bar[1] pase la barra de g_lastTPBar
      //   = espera ~2 barras completas (~120s) post-TP
      //   Fuente: v8.30 (mejor LR Correlation 0.983, equity recta)
      //   Efecto: K sale completamente de zona <30 → WR sube
      //
      // Estado C — NORMAL (bar-lock estándar):
      //   Solo 1 señal por barra M1 cerrada
      //   Fuente: v8.28 (mejor WR 31%, PF 1.51, Sharpe 100)
      //   Efecto: 185/día → con sentinel SL → ~160-170/día
      // ═════════════════════════════════════════════════════════════
      datetime curBar1 = iTime(g_sym,_Period,1);

      if(g_slForceReopen)
      {
         // Estado A: SL inmediato
         // No bar-lock check — señal evaluate now
      }
      else if(g_lastTPBar != 0)
      {
         // Estado B: post-TP lock extendido
         // Esperar que bar[1] sea posterior a la barra del TP
         // bar[1] cambia cada 60s → g_lastTPBar es bar[0] del TP
         // bar[1] > g_lastTPBar cuando han pasado 2 barras completas
         if(curBar1 <= g_lastTPBar){DrawDashboard();return;}
         g_lastTPBar = 0; // lock expiró
      }
      else
      {
         // Estado C: bar-lock normal
         if(curBar1 == g_lastSignalBar){DrawDashboard();return;}
      }

      int sig=GetSignal();
      if(sig== 1&&CountBuys() >=Max_Buy)  sig=0;
      if(sig==-1&&CountSells()>=Max_Sell) sig=0;
      if(sig==1)  {ulong tk=OpenOrder(ORDER_TYPE_BUY, CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastSignalBar=curBar1;g_slForceReopen=false;g_lastLegacyTicket=tk;}}
      if(sig==-1) {ulong tk=OpenOrder(ORDER_TYPE_SELL,CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastSignalBar=curBar1;g_slForceReopen=false;g_lastLegacyTicket=tk;}}
   }
   DrawDashboard();
}
