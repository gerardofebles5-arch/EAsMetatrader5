//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v16.7 — LIVE PARITY ENGINE                                    |
//|                                                                  |
//|  FIXES v16.7 (sin alterar backtest):                           |
//|                                                                  |
//|  FIX 1 — VENTANA DE PROTECCION LIVE (SL_Guard_Secs)           |
//|  Durante los primeros N segundos de vida de una posicion en    |
//|  live, el vSL no actua. Esto replica el comportamiento del     |
//|  backtest donde el tester no genera ticks intra-barra que      |
//|  toquen el SL en el primer tick de apertura.                   |
//|  En backtest: SL_Guard_Secs=0 (desactivado, comportamiento     |
//|  identico al original). BIFURCADO con MQL_TESTER.             |
//|                                                                  |
//|  FIX 2 — TRAILING PROFIT ESCALADO POR LOTE (puntos fijos)     |
//|  Los umbrales TP_Trail_Activate y TP_Trail_Retrace se escalan  |
//|  por lotaje para mantener equivalencia en PUNTOS con backtest. |
//|  Backtest lot=0.05: $0.50 / ($5/pt) = 0.10 pts de activación  |
//|  Live    lot=0.01: scaled = $0.50 × (0.01/0.05) = $0.10       |
//|                    $0.10 / ($1/pt) = 0.10 pts ← MISMO         |
//|  Fórmula: activateUSD × (posLot / Trail_Lot_Ref)              |
//|  En backtest: lotScale=1.0 → sin cambio → idéntico a v16.6.   |
//|                                                                  |
//|  FIX 3 — COOLDOWN INDEPENDIENTE DE CountLegacyOpen            |
//|  El Entry_Cooldown_Secs ahora se verifica contra               |
//|  g_lastOpenAttemptTime, que se guarda en el momento de abrir   |
//|  independientemente de si la posicion ya cerro.                |
//|  Evita la re-entrada inmediata tras trades de 0 segundos.      |
//|                                                                  |
//|  FIX 4 — FILTRO DE TENDENCIA: MA50 M15 en lugar de MA200 H1   |
//|  MA200(H1) = 8.3 dias de historia → bloquea dirección entera  |
//|  durante días aunque el mercado intraday haya girado.          |
//|  Reemplazado por MA50(M15) = 12.5 horas → filtro intraday     |
//|  relevante para estrategia de 13s avg. BIFURCADO: backtest     |
//|  mantiene PERIOD_H1/200 exacto para no alterar resultados.     |
//|                                                                  |
//|  FIX 5 — vSL/vTP COMPENSADO POR SPREAD EN LIVE                |
//|  El vSL para SELL = BID_entry + SL_FIXED. El checker usa ASK  |
//|  (ask >= vSL). En live, ASK = BID + spread. Si no compensamos  |
//|  el spread al calcular vSL, la distancia efectiva es           |
//|  SL_FIXED - spread en lugar de SL_FIXED.                       |
//|  FIX: vSL_sell = BID + SL_FIXED + spread_actual (live only)   |
//|       vSL_buy  = ASK - SL_FIXED - spread_actual (live only)   |
//|                                                                  |
//|  CAPA 1: MTF Confluence M1+M5+M15 (posición vs EMA)           |
//|  CAPA 2: Order Flow Bias (delta volumen)                       |
//|  CAPA 3: Momentum Bias (EMA gap direction + acceleration)      |
//|  CAPA 4: Price Structure (HH/HL vs LH/LL)                     |
//|  CAPA 5: Stoch Timing (no entrar en cima/valle de impulso)    |
//+------------------------------------------------------------------+
...
input bool   Recovery_Mode_Enabled       = false;  // v16.9: OFF | G2 trades destruyen valor (WR=49.7%, PF=0.48)
                                                   // 147 trades G2 generaron -$99 en backtest → desactivado
...
input double TP_Trail_Activate_USD       = 2.00;   // v16.9: 0.50→2.00 | Con lot=0.05: activa en 0.40 pts
                                                   // Antes: avg_win=0.38 pts=1.55$ insuficiente para cubrir losses
                                                   // Ahora: espera al menos 0.40 pts de ganancia antes de trailing
input double TP_Trail_Retrace_USD        = 1.00;   // v16.9: 0.30→1.00 | Con lot=0.05: retroceso de 0.20 pts
                                                   // Deja correr los ganadores antes de cerrar
// v16.7: los umbrales se escalan por lotaje para mantener equivalencia en PUNTOS
// Con lot=0.05 (backtest): activa/retrace en puntos = TP_Trail_Activate_USD / (lot*20)
// Con lot=0.01 (live):     se multiplica por (LOT_REF/lot_actual) para mantener mismos PUNTOS
// LOT_REF = 0.05 (lote de referencia del backtest). En backtest NO se escala (identico a v16.6).
input double Trail_Lot_Ref               = 0.05;   // lote de referencia para escalar trail (backtest lot)
...
// +++ Forensic Core +++
input double MM7_TP_FIXED                = 5.00;   // v16.9: 2.50→5.00 | TP fijo más amplio para capturar movimientos grandes
input double MM7_SL_FIXED                = 1.50;   // v16.9: 0.77→1.50 | SL más amplio para evitar stopouts prematuros
                                                   // Análisis: avg_loss_move=-0.81 pts vs SL=0.77 pts → SL demasiado ajustado
...
input int    Entry_Cooldown_Secs         = 30;    // cooldown entre señales (medido desde g_lastOpenAttemptTime independiente de cierre)
...
// +++ v16.7 LIVE PARITY FIXES +++
input int    SL_Guard_Secs               = 4;     // segundos de gracia post-apertura donde vSL no actua (solo live)
                                                   // Replica backtest: el tester no genera ticks intra-barra que toquen SL
                                                   // En backtest = 0 (desactivado, bifurcado con MQL_TESTER)
input int    Trend_MA_Period_Live        = 50;    // periodo MA para filtro de tendencia en LIVE (M15 timeframe)
                                                   // MA50(M15) = 12.5h → intraday. Backtest mantiene MA200(H1)
input bool   Enable_Spread_SL_Comp      = true;  // compensar spread al calcular vSL en live (solo live)
                                                   // vSL_sell = entry + SL_FIXED + spread; protege del spread inicial
...
struct StealthPos {
   ...
   // v16.7: SL Guard — tiempo de apertura para ventana de proteccion live
   datetime openTime;         // momento en que se registró la posición (para SL_Guard_Secs)
};
...
int      g_hMA_Live=INVALID_HANDLE; // v16.7: MA50(M15) para filtro tendencia en live
...
// v16.1 — WARMUP GUARD
// Problema live: al arrancar, EMA_M5 y EMA_M15 tienen valores de hace
// 21+ minutos (warmup incompleto). GetMTFConfluence() puntúa la dirección
// INCORRECTA → score_sell > score_buy persistentemente → quema cuenta.
// Solución: bloquear señales hasta que M5 y M15 tengan barras frescas cargadas.
// Necesitamos al menos 1 barra nueva de M5 (5 min) y 1 de M15 (15 min)
// para que las EMAs reflejen el precio actual y no valores viejos.
// En backtest esto no ocurre porque el tester precarga todos los datos.
bool     g_warmupDone      = false; // true cuando los indicadores están listos
datetime g_warmupStartTime = 0;     // momento en que arrancó el EA
// v16.3 LIVE FIX: timestamps de barras al arrancar (para warmup real en live)
// En backtest estas vars no se usan porque (bool)MQLInfoInteger(MQL_TESTER)==true → warmup inmediato
datetime g_warmup_M5_bar   = 0;
datetime g_warmup_M15_bar  = 0;
...
// v16.7 FIX 3: cooldown independiente de CountLegacyOpen
datetime g_lastOpenAttemptTime = 0;
// v16.8: Bar Lock — evita múltiples entradas en la misma barra M1 en live
// En backtest: hay exactamente 1 tick por barra → lock natural
// En live: múltiples ticks/segundo → GetSignal puede retornar en la misma barra
datetime g_lastEntryBar     = 0;  // iTime(bar[0]) del último OpenOrder exitoso
// Cooling-off state
...
// v16.8: Throttle — precio anterior para verificar persistencia antes de cerrar por vSL/vTP
// En live los ticks pueden tener spikes momentáneos que activan el cierre por 1 tick
// solo para revertir en el siguiente. Guardamos el bid/ask del tick anterior.
double   g_prevBid = 0;
double   g_prevAsk = 0;
...
bool IsVolumeOK()
{
   ...
   // v16.8 FIX: en live usar bar[1] como referencia de volumen actual (barra cerrada).
   // En backtest bar[0] está completa (1 tick/barra) → comparación válida.
   // En live bar[0] está en formación → volumen siempre menor que barras completas → bloquea señales válidas.
   int vol_start = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
   if(CopyTickVolume(g_sym,_Period,vol_start,Volume_Ma_Period+1,vb)<Volume_Ma_Period+1) return true;
...
int GetTrend()
{
   if(!Enable_Trend_Filter) return 0;
   // v16.7 FIX 4: bifurcar filtro de tendencia live vs backtest
   // BACKTEST: MA200(H1) — identico a v16.6, backtest no cambia
   // LIVE:     MA50(M15) = 12.5h de historia intraday
   //           MA200(H1) = 8.3 dias → bloquea dirección días enteros aunque
   //           el mercado intraday haya girado → incompatible con scalping de 13s
   if((bool)MQLInfoInteger(MQL_TESTER))
   {
      // BACKTEST: MA200(H1) original
      if(g_hMA==INVALID_HANDLE) return 0;
   ...
   {
      // LIVE: MA50(M15) — refleja tendencia intraday de las últimas 12.5 horas
      if(g_hMA_Live==INVALID_HANDLE) return 0;
      double ml[]; ArraySetAsSeries(ml,true);
      if(CopyBuffer(g_hMA_Live,0,1,1,ml)<1) return 0;
      double bid=SymbolInfoDouble(g_sym,SYMBOL_BID);
      return (bid>ml[0])?1:(bid<ml[0])?-1:0;
   }
}
...
void ScanFVGs()
{
   // v16.3 LIVE FIX: en live usar bar[1] (cerrada) como trigger para evitar
   // doble-scan en primer tick de barra nueva. En backtest: bar[0] como siempre (v16.1).
   datetime bt = (bool)MQLInfoInteger(MQL_TESTER) ? iTime(g_sym,_Period,0) : iTime(g_sym,_Period,1);
   if(bt==g_fvgLastScan) return; g_fvgLastScan=bt;
...
//--------------------------------------------------------------------
// CAPA 1: Confluencia Multi-Timeframe
// Lee la dirección EMA(50) en M1, M5, M15
// Si ≥2 coinciden → señal de confluencia en esa dirección
//--------------------------------------------------------------------
int GetMTFConfluence(int dir)
{
   if(!Enable_MTF_Confluence) return 1; // bypass → score parcial garantizado

   int score = 0;

   // M1: precio vs EMA_Slow (ya calculada en g_hEMA_Slow)
   if(g_hEMA_Fast != INVALID_HANDLE)
   {
      double ef1[]; ArraySetAsSeries(ef1,true);
      double es1[]; ArraySetAsSeries(es1,true);
      if(CopyBuffer(g_hEMA_Fast,0,1,1,ef1)>=1 && CopyBuffer(g_hEMA_Slow,0,1,1,es1)>=1)
      {
         int d1 = (ef1[0] > es1[0]) ? 1 : -1;
         if(d1 == dir) score++;
      }
   }

   // M5: tendencia
   // v16.3 LIVE FIX: en live verificar que la barra M5[1] sea reciente antes de puntuar
   // En backtest: (bool)MQLInfoInteger(MQL_TESTER)==true → sin verificacion de frescura (comportamiento v16.1)
   if(g_hEMA_M5 != INVALID_HANDLE)
   {
      double em5[]; ArraySetAsSeries(em5,true);
      bool m5_data_ok = (CopyBuffer(g_hEMA_M5,0,1,1,em5)>=1);
      bool m5_fresh = (bool)MQLInfoInteger(MQL_TESTER) || (iTime(g_sym, PERIOD_M5, 1) > g_warmup_M5_bar);
      if(m5_data_ok && m5_fresh)
      {
         double bid = SymbolInfoDouble(g_sym, SYMBOL_BID);
         int d5 = (bid > em5[0]) ? 1 : -1;
         if(d5 == dir) score++;
      }
      else if(!m5_fresh) score++; // datos obsoletos en live → bypass neutral
   }
   ...
   if(g_hEMA_M15 != INVALID_HANDLE)
   {
      ...
      bool m15_data_ok = (CopyBuffer(g_hEMA_M15,0,1,1,em15)>=1);
      bool m15_fresh = (bool)MQLInfoInteger(MQL_TESTER) || (iTime(g_sym, PERIOD_M15, 1) > g_warmup_M15_bar);
      if(m15_data_ok && m15_fresh)
      {
         double bid = SymbolInfoDouble(g_sym, SYMBOL_BID);
         int d15 = (bid > em15[0]) ? 1 : -1;
         if(d15 == dir) score++;
      }
      else if(!m15_fresh) score++; // datos obsoletos en live → bypass neutral
   }

   // Retorna true si alcanza confluencia mínima
...
int GetMomentumBias(int dir)
{
   ...
   // v16.3 BIFURCACION (bool)MQLInfoInteger(MQL_TESTER):
   // BACKTEST: leer bar[0,1,2] — IDENTICO a v16.1 — backtest no cambia
   // LIVE:     leer bar[1,2,3] — solo barras cerradas — señales estables tick a tick
   //           bar[0] cambia cada tick en live → gap oscila → señales contradictorias
   double gap0, gap1, gap2;
   if((bool)MQLInfoInteger(MQL_TESTER))
   {
      // BACKTEST — comportamiento original v16.1
      if(CopyBuffer(g_hEMA_Fast,0,0,3,ef_now)<3) return 1;
      if(CopyBuffer(g_hEMA_Slow,0,0,3,es_now)<3) return 1;
      gap0 = ef_now[0] - es_now[0]; // bar0 = en formacion (ok en backtest, deterministico)
      gap1 = ef_now[1] - es_now[1];
      gap2 = ef_now[2] - es_now[2];
   }
   else
   {
      // LIVE — solo barras cerradas
      if(CopyBuffer(g_hEMA_Fast,0,1,4,ef_now)<4) return 1;
      if(CopyBuffer(g_hEMA_Slow,0,1,4,es_now)<4) return 1;
      gap0 = ef_now[1] - es_now[1]; // bar1 = ultima barra cerrada
      gap1 = ef_now[2] - es_now[2];
      gap2 = ef_now[3] - es_now[3];
   }

   // gap_delta = gap0 - gap1 (referencia futura para aceleración)
...
int GetStochTiming(int dir)
{
   ...
   double kb[]; ArraySetAsSeries(kb,true);
   // v16.6 FIX: en live leer bar[1] (cerrada) para evitar veto/allow inestable tick a tick
   // En backtest leer bar[0] como v16.1 original
   int stoch_start = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
   if(CopyBuffer(g_hStoch,0,stoch_start,2,kb)<2) return 1;
   double k_now = kb[0];
...
//--------------------------------------------------------------------
// MOTOR PREDICTIVO MAESTRO
// Calcula el score de predicción (0-5) y retorna la dirección
// Solo abre si score >= Min_Prediction_Score
//--------------------------------------------------------------------
int GetPredictiveSignal()
{
   // Probar ambas direcciones y ver cuál puntúa más
   int score_buy  = 0;
   int score_sell = 0;

   // Capas independientes por dirección
   int stk_buy  = GetStochTiming(1);
   int stk_sell = GetStochTiming(-1);
   score_buy  += GetMTFConfluence(1);
   score_buy  += GetOrderFlowBias(1);
   score_buy  += GetMomentumBias(1);
   score_buy  += GetPriceStructure(1);
   score_buy  += stk_buy;

   score_sell += GetMTFConfluence(-1);
   score_sell += GetOrderFlowBias(-1);
   score_sell += GetMomentumBias(-1);
   score_sell += GetPriceStructure(-1);
   score_sell += stk_sell;

   // El Stochastic Timing actúa como VETO si retorna 0
   // v16.8 FIX: reusar los resultados ya calculados — evita 2 CopyBuffer extra por tick
   bool stoch_veto_buy  = (stk_buy  == 0);
   bool stoch_veto_sell = (stk_sell == 0);

   // Guardar para dashboard
   g_lastPredScore = MathMax(score_buy, score_sell);
   g_lastPredDir   = (score_buy > score_sell) ? 1 : (score_sell > score_buy) ? -1 : 0;

   // Determinar señal ganadora
   if(score_buy >= Min_Prediction_Score && score_buy > score_sell && !stoch_veto_buy)
      return 1;
   if(score_sell >= Min_Prediction_Score && score_sell > score_buy && !stoch_veto_sell)
      return -1;

   // Empate con score suficiente → Stoch decide el timing
   if(score_buy == score_sell && score_buy >= Min_Prediction_Score)
   {
      double kb[]; ArraySetAsSeries(kb,true);
      int stoch_start = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
      if(CopyBuffer(g_hStoch,0,stoch_start,1,kb)>=1)
      {
         if(kb[0] <= Stoch_Buy_Level  && !stoch_veto_buy)  return 1;
         if(kb[0] >= Stoch_Sell_Level && !stoch_veto_sell) return -1;
      }
   }
...
// El motor predictivo reemplaza al Stoch como señal principal.
// El Stoch ahora es solo un confirmador de timing (Capa 5).
//============================================================
int GetSignal()
{
   ...
   // ── MODO PREDICTIVO (v15) ──────────────────────────────────────
   // Obtener señal del motor de predicción multi-capa
   int predSig = GetPredictiveSignal();

   // Si el motor predictivo tiene señal clara, verificar trend y usar
   if(predSig != 0)
   {
      int trend = GetTrend();
      if(trend != 0 && predSig != trend) return 0; // contra-tendencia H1 → rechazar

      // Verificar filtros intra adicionales (cooldown, fast-loss, etc.)
      // v16.6 FIX: en live leer bar[1] (cerrada), en backtest bar[0] como v16.1
      double kb[]; ArraySetAsSeries(kb,true);
      int kb_start = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
      if(CopyBuffer(g_hStoch,0,kb_start,3,kb)<3) return 0;
      double k_now  = kb[0];
      double k_prev = kb[1];

      if(!IntraFiltersPass(predSig, k_now, k_prev)) return 0;

      g_lastSigType = 4; // tipo 4 = señal predictiva v15
      return predSig;
   }

   // ── FALLBACK: SEÑALES ESTRUCTURALES (sweep/FVG) ───────────────
   // Si el motor predictivo no da señal pero hay estructura SMC, úsala
   int trend = GetTrend();
   double kb[]; ArraySetAsSeries(kb,true);
   int kb_start = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
   if(CopyBuffer(g_hStoch,0,kb_start,3,kb)<3) return 0;
   ...
   else // Hybrid (modo 1): FVG y Sweep como fallback
   {
      ...
      // P2: FVG fill con confluencia parcial (score ≥ 2)
      if(sig==0 && Enable_FVG_Strategy)
      {
         bool stOKbuy  = (k_now<=Stoch_Buy_Level+10);
         bool stOKsell = (k_now>=Stoch_Sell_Level-10);
         // Para FVG exigir score mínimo de 2 del motor predictivo como validación
         int fvg_score_buy  = GetOrderFlowBias(1)  + GetMomentumBias(1);
         int fvg_score_sell = GetOrderFlowBias(-1) + GetMomentumBias(-1);
         if(InFVG(1) &&stOKbuy &&(trend==0||trend== 1)&&fvg_score_buy>=1 &&IntraFiltersPass(1,k_now,k_prev)){sig= 1;g_lastSigType=2;}
         if(InFVG(-1)&&stOKsell&&(trend==0||trend==-1)&&fvg_score_sell>=1&&IntraFiltersPass(-1,k_now,k_prev)){sig=-1;g_lastSigType=2;}
      }

      // P3: Stoch puro como último recurso — solo si score predictivo >= 2
      if(sig==0)
      {
         ...
         // v15: para señal pura stoch exigir validación predictiva parcial
         if(crossBuy  && IntraFiltersPass(1, k_now,k_prev) && (trend==0||trend== 1))
         {
            int s_score = GetOrderFlowBias(1) + GetMomentumBias(1) + GetMTFConfluence(1);
            if(s_score >= 2) { sig= 1; g_lastSigType=3; }
         }
...
double CalcLot()
{
   ...
   // v16.8 FIX: en live usar min(balance,equity) para reflejar pérdidas flotantes.
   // Si hay posiciones abiertas en pérdida, BALANCE no lo refleja → lot demasiado grande.
   // En backtest: usar ACCOUNT_BALANCE (comportamiento original, sin posiciones simultáneas).
   double bal;
   if((bool)MQLInfoInteger(MQL_TESTER))
      bal = AccountInfoDouble(ACCOUNT_BALANCE);
   else
      bal = MathMin(AccountInfoDouble(ACCOUNT_BALANCE), AccountInfoDouble(ACCOUNT_EQUITY));
...
void StealthRegister(ulong ticket,double vTP,double vSL,int dir)
{
   ...
   // v16.6: log de registro para confirmar en live que el ticket quedo registrado
   Print("MM7 STEALTH REG #",ticket," dir=",dir," vTP=",vTP," vSL=",vSL);
   // inicializar trailing
   g_sp[g_spCnt].trailMaxProfit=0.0;
   g_sp[g_spCnt].trailActivated=false;
   // v16.7 FIX 1: guardar openTime para SL_Guard_Secs
   g_sp[g_spCnt].openTime = TimeCurrent();
   g_spCnt++;
}

bool StealthClosePos(ulong ticket,int dir)
{
   if(!PositionSelectByTicket(ticket)) return true; // ya cerrada
   ...
   // v16.5 LIVE FIX: probar filling modes en orden y verificar cierre entre intentos
   // Evita doble-cierre si el primero fue exitoso pero retcode no lo confirmo
   ...
   if(PositionSelectByTicket(ticket)) // verificar que siga abierta antes de reintentar
   {
      req.price=(dir==1)?SymbolInfoDouble(g_sym,SYMBOL_BID):SymbolInfoDouble(g_sym,SYMBOL_ASK);
...
//--------------------------------------------------------------------
// v16.6 LIVE FIX: StealthRecoverOrphans
// Si OpenOrder fallo al registrar el ticket en g_sp[], la posicion
// queda sin vTP/vSL y nunca cierra (puede durar horas).
// Este scan busca posiciones abiertas con nuestro magic que NO estan
// en g_sp[] y las registra con niveles calculados desde precio actual.
// Solo corre en live (no en backtest).
//--------------------------------------------------------------------
void StealthRecoverOrphans()
{
   ...
   for(int p = PositionsTotal()-1; p >= 0; p--)
   {
      ...
      // Verificar si ya esta en g_sp[]
      bool found = false;
      for(int j = 0; j < g_spCnt; j++)
         if(g_sp[j].ticket == tk && g_sp[j].active) { found = true; break; }
      if(found) continue;
      // Posicion huerfana — registrar ahora
      long   posTyp  = PositionGetInteger(POSITION_TYPE);
      int    dir     = (posTyp == POSITION_TYPE_BUY) ? 1 : -1;
      double entryPx = PositionGetDouble(POSITION_PRICE_OPEN);
      double curBid  = SymbolInfoDouble(g_sym, SYMBOL_BID);
      double curAsk  = SymbolInfoDouble(g_sym, SYMBOL_ASK);
      // Calcular vTP/vSL desde precio de entrada original
      double slPts = MM7_SL_FIXED;
      double vTP   = entryPx + dir * MM7_TP_FIXED;
      double vSL   = entryPx - dir * slPts;
      StealthRegister(tk, vTP, vSL, dir);
      // v16.7 FIX: posición huérfana ya lleva tiempo abierta — deshabilitar SL_Guard
      // openTime=0 indica que NO aplica la ventana de protección post-apertura
      // (la protección de primer tick no tiene sentido para posiciones ya abiertas)
      if(g_spCnt > 0) g_sp[g_spCnt-1].openTime = 0;
      Print("MM7 v16.7 | ORPHAN RECOVERED: ticket=", tk, " dir=", dir,
            " entry=", entryPx, " vTP=", vTP, " vSL=", vSL);
   }
}
...
void StealthCheckAll()
{
   ...
   // v16.8 GAP 4: Throttle de cierre en live
   // En live los ticks pueden tener spikes de spread momentáneos que activan
   // el vSL/vTP en un tick y se revierten en el siguiente.
   // En backtest: solo 1 tick por barra → sin spikes intermedios.
   // THROTTLE: en live, para cerrar por vSL o vTP (no por TrailingProfit)
   // exigimos que el precio haya estado en zona de cierre EN EL TICK ANTERIOR también.
   // g_prevBid/g_prevAsk se actualizan al FINAL de cada llamada a StealthCheckAll.
   double prevBid = (isLive && g_prevBid > 0) ? g_prevBid : bid;
   double prevAsk = (isLive && g_prevAsk > 0) ? g_prevAsk : ask;
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      // ── v16.7 FIX 1: SL_Guard_Secs — ventana de protección post-apertura ──
      // BACKTEST: SL_Guard_Secs siempre 0 → esta ventana nunca aplica
      //           El tester no genera ticks intra-barra que toquen el SL en 1er tick
      //           Comportamiento identico a v16.6
      // LIVE:     Durante los primeros SL_Guard_Secs segundos, el vSL no actua
      //           Solo el vTP puede cerrar (captura reversiones rápidas a favor)
      //           Replica la protección natural del tester contra ticks adversos
      //           en el primer segundo de vida de la posición
      bool slGuardActive = false;
      if(isLive && SL_Guard_Secs > 0 && g_sp[i].openTime > 0)
      {
         int age = (int)(now - g_sp[i].openTime);
         if(age < SL_Guard_Secs) slGuardActive = true;
      }

      // ── v16.7 FIX 2: Trailing Profit escalado por lote ───────────────
      // OBJETIVO: mantener la misma distancia en PUNTOS de activación
      // que el backtest, independiente del tamaño del lote.
      //
      // XAUUSD: profit_USD = move_pts * lot * contract_size
      //   lot=0.05: $1/pt × 0.05 × 100 = $5.00/pt → $0.50 activate = 0.10 pts
      //   lot=0.01: $1/pt × 0.01 × 100 = $1.00/pt → $0.50 activate = 0.50 pts
      //
      // Para mantener los MISMOS 0.10 puntos con lot=0.01:
      //   activateUSD_scaled = 0.10 pts × $1.00/pt = $0.10
      //   Fórmula: activateUSD_scaled = TP_Trail_Activate_USD × (posLot / Trail_Lot_Ref)
      //   = $0.50 × (0.01/0.05) = $0.50 × 0.2 = $0.10
      //
      // EN BACKTEST (isLive=false): lotScale=1.0 → sin escalar → idéntico a v16.6
      if(Enable_Trailing_Profit || Enable_MinUSD_TP)
      {
         double lotScale = 1.0;
         if(isLive && posLot > 0 && Trail_Lot_Ref > 0)
            lotScale = MathMax(0.1, MathMin(posLot / Trail_Lot_Ref, 2.0)); // escala proporcional al lot

         double activateUSD = TP_Trail_Activate_USD * lotScale;
         double retraceUSD  = TP_Trail_Retrace_USD  * lotScale;
         double minUSD      = TP_Min_USD;

         // MODO 1: TP por USD minimo — cierre inmediato
         if(Enable_MinUSD_TP && curProfit >= minUSD)
            cls = true;

         // MODO 2: Trailing Profit escalado
      ...
      if(!cls && Enable_TrailingStop)
      {
         ...
         double entryPx = PositionGetDouble(POSITION_PRICE_OPEN);
         double atr     = GetATR();
         double tsStart = atr * TS_Start_ATR_Multiplier;
         double tsDist  = atr * TS_Distance_ATR_Multiplier;

         if(posType == POSITION_TYPE_BUY)
         {
            if(bid >= entryPx + tsStart)
            {
               double newVSL = bid - tsDist;
               if(newVSL > g_sp[i].vSL) g_sp[i].vSL = newVSL;
            }
            // Throttle: en live exigir que el precio estuviera bajo vSL en tick anterior también
            bool slHit = (!slGuardActive && g_sp[i].vSL > 0 && bid <= g_sp[i].vSL);
            if(isLive && slHit) slHit = (prevBid <= g_sp[i].vSL); // confirmar con tick anterior
            if(slHit) cls = true;
         }
         else // SELL
         {
            if(ask <= entryPx - tsStart)
            {
               double newVSL = ask + tsDist;
               if(newVSL < g_sp[i].vSL || g_sp[i].vSL <= 0) g_sp[i].vSL = newVSL;
            }
            bool slHit = (!slGuardActive && g_sp[i].vSL > 0 && ask >= g_sp[i].vSL);
            if(isLive && slHit) slHit = (prevAsk >= g_sp[i].vSL);
            if(slHit) cls = true;
         }
      }
      ...
      if(!cls)
      {
         // TP fijo — ThrottLe: en live confirmar con tick anterior también
         bool tpHitB = (g_sp[i].dir== 1 && g_sp[i].vTP > 0 && bid >= g_sp[i].vTP);
         bool tpHitS = (g_sp[i].dir==-1 && g_sp[i].vTP > 0 && ask <= g_sp[i].vTP);
         if(isLive && tpHitB) tpHitB = (prevBid >= g_sp[i].vTP);
         if(isLive && tpHitS) tpHitS = (prevAsk <= g_sp[i].vTP);
         if(tpHitB || tpHitS) cls = true;

         if(!cls && !slGuardActive)
         {
            // SL fijo — Throttle: en live confirmar con tick anterior
            bool slHitB = (g_sp[i].dir== 1 && g_sp[i].vSL > 0 && bid <= g_sp[i].vSL);
            bool slHitS = (g_sp[i].dir==-1 && g_sp[i].vSL > 0 && ask >= g_sp[i].vSL);
            if(isLive && slHitB) slHitB = (prevBid <= g_sp[i].vSL);
            if(isLive && slHitS) slHitS = (prevAsk >= g_sp[i].vSL);
            if(slHitB || slHitS) cls = true;
         }
      }
   ...
   // v16.8: actualizar precio anterior para throttle del próximo tick
   if(isLive) { g_prevBid = bid; g_prevAsk = ask; }
}
...
ulong OpenOrder(ENUM_ORDER_TYPE type,string comment,double slMul)
{
   ...
   // v16.7 FIX 5: compensar spread al calcular vSL en live
   // BACKTEST: spread ≈ 0 → spread_comp = 0 → identico a v16.6
   // LIVE:     el checker de vSL usa ask/bid. Para SELL: verifica ask >= vSL.
   //           ASK = BID + spread. Si vSL = BID_entry + SL_FIXED (sin compensar),
   //           la distancia efectiva desde ASK es SL_FIXED - spread.
   //           Con spread=0.5 y SL=0.77: distancia real = 0.27 pts → se toca con cualquier tick.
   //           FIX: vSL = entry + SL_FIXED + spread (SELL) / entry - SL_FIXED - spread (BUY)
   //           Así la distancia desde el precio ASK/BID efectivo siempre es SL_FIXED completo.
   double spread_comp = 0.0;
   if(!(bool)MQLInfoInteger(MQL_TESTER) && Enable_Spread_SL_Comp)
      spread_comp = (ask - bid);  // spread actual en puntos

   double vTP = entry + dir*MM7_TP_FIXED;
   double vSL = entry - dir*(slPts + spread_comp);  // SL compensado por spread en live

   // v16.8 GAP 1 FIX: compensar spread también en vTP en live
   // En backtest: sin spread → vTP = entry ± 2.50 (se alcanza con 2.50pts de movimiento)
   // En live SELL: entry=BID, checker usa ask<=vTP. ask=BID+spread → necesita BID-2.50-spread
   //              → con spread=0.3: necesita 2.80pts de movimiento vs 2.50 en backtest
   // FIX: vTP_sell = BID - (TP_FIXED - spread) → checker ask <= BID-(TP_FIXED-spread)
   //      → BID+spread <= BID-(TP_FIXED-spread) → spread <= -(TP_FIXED-spread) NO
   // Corrección: vTP_sell = BID - TP_FIXED + spread → ask = BID+spread → BID+spread <= BID-TP_FIXED+spread → 0<=-TP_FIXED NUNCA
   // La lógica correcta: queremos que el trade gane cuando el precio se mueve TP_FIXED desde entry
   // Para SELL: entry=BID, ganamos cuando ask <= BID - TP_FIXED
   // El checker: ask <= g_sp[i].vTP con vTP = BID - TP_FIXED = entry - TP_FIXED
   // Con spread: al cerrar compramos en ASK. El profit = (entry - ASK_close)
   // Queremos profit >= TP_FIXED: entry - ASK_close >= TP_FIXED → ASK_close <= entry - TP_FIXED
   // Pero entry=BID_open, ASK_close = BID_close + spread_close
   // BID_close + spread_close <= BID_open - TP_FIXED
   // BID_close <= BID_open - TP_FIXED - spread_close
   // Con spread constante: BID_close <= BID_open - TP_FIXED - spread
   // → vTP correcto para SELL = BID_open - TP_FIXED - spread (más bajo = más difícil)
   // PERO queremos equivalencia con backtest: en backtest se cierra cuando precio baja TP_FIXED pts
   // En live también queremos cerrar cuando BID baja TP_FIXED pts desde BID_open
   // El checker usa ASK: ask <= vTP → BID+spread <= vTP → vTP = BID_open - TP_FIXED + spread
   // Así: BID+spread <= BID_open - TP_FIXED + spread → BID <= BID_open - TP_FIXED ✓ mismos puntos
   if(!(bool)MQLInfoInteger(MQL_TESTER) && Enable_Spread_SL_Comp)
   {
      // Ajustar vTP para que se active cuando el precio de referencia (BID/ASK base) se mueva TP_FIXED
      // SELL: vTP = entry - TP_FIXED + spread (checker: ask <= vTP)
      // BUY:  vTP = entry + TP_FIXED - spread (checker: bid >= vTP)
      vTP = entry + dir*MM7_TP_FIXED - dir*spread_comp;
   }

   // v16.8 FIX: g_lastOpenAttemptTime solo se actualiza para trades Legacy,
   // no para G2. De lo contrario un G2 bloquea el cooldown de 30s para Legacy.
   // Detectar si es Legacy buscando "[Legacy]" en el comment.
   // En backtest: g_lastOpenAttemptTime no existe → sin cambio.
   if(!(bool)MQLInfoInteger(MQL_TESTER) && StringFind(comment,"[Legacy]")>=0)
      g_lastOpenAttemptTime = TimeCurrent();
   ...
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED)
   {
      ...
      // v16.6 FIX CRITICO: obtener ticket de posicion de forma robusta.
      if(res.order > 0 && PositionSelectByTicket(res.order))
         posTk = res.order;

      if(posTk == 0 && res.deal > 0)
      {
         int maxTries = (bool)MQLInfoInteger(MQL_TESTER) ? 1 : 5;
         for(int attempt = 0; attempt < maxTries && posTk == 0; attempt++)
         {
            if(attempt > 0) Sleep(20);
            if(HistoryDealSelect(res.deal))
            {
               ulong pid = (ulong)HistoryDealGetInteger(res.deal, DEAL_POSITION_ID);
               if(pid > 0 && PositionSelectByTicket(pid)) posTk = pid;
            }
         }
      }
      ...
      if(posTk == 0)
      {
         ...
         for(int p = PositionsTotal()-1; p >= 0; p--)
         {
            ...
            long posT = PositionGetInteger(POSITION_TYPE);
            if(type == ORDER_TYPE_BUY  && posT != POSITION_TYPE_BUY)  continue;
            if(type == ORDER_TYPE_SELL && posT != POSITION_TYPE_SELL) continue;
            datetime t2 = (datetime)PositionGetInteger(POSITION_TIME);
            if((now - t2) > 5) continue;
            if(t2 >= bt){ bt = t2; bk = tk2; }
         }
         if(bk > 0) posTk = bk;
      }

      if(posTk == 0)
         Print("MM7 WARNING: ticket no encontrado para ",
               EnumToString(type), " res.order=", res.order,
               " res.deal=", res.deal, " retcode=", res.retcode);
...
void DailyReset()
{
   ...
   g_dayStart=today;
   // v16.7 DD FIX: usar min(balance, equity) como referencia del nuevo día
   // Protege si el día empieza con posiciones abiertas en pérdida
   double bal_r = AccountInfoDouble(ACCOUNT_BALANCE);
   double eq_r  = AccountInfoDouble(ACCOUNT_EQUITY);
   g_dayStartBal = MathMin(bal_r, eq_r);
   g_haltedToday=false;
   ...
   g_lastOpenAttemptTime=0; // v16.7: cooldown independiente
   g_lastEntryBar=0;        // v16.8: bar lock reset
}
...
void CheckHaltConditions()
{
   ...
   // v16.7 DD FIX: usar g_dayStartBal como referencia FIJA para ambas métricas.
   // ANTES: (bal - eq)/bal → se reseteaba con cada cierre manual (bal bajaba, eq≈bal → DD≈0)
   // AHORA: ambas métricas usan g_dayStartBal como denominador — no fluctúa con cierres

   // 1. Equity Drawdown: equity actual vs balance al inicio del día
   //    Captura pérdidas FLOTANTES + realizadas. Se activa aunque no se cierre nada.
   //    Cierres manuales no resetean este check porque g_dayStartBal es fijo.
   if(InpMaxEquityDrawdown > 0 && g_dayStartBal > 0)
   {
      double eq_dd = (g_dayStartBal - eq) / g_dayStartBal * 100.0;
      if(eq_dd >= InpMaxEquityDrawdown)
      {
         g_haltedToday = true;
         Print("MM7 HALT: MaxEquityDrawdown=", InpMaxEquityDrawdown, "% alcanzado. eq_dd=", DoubleToString(eq_dd,2), "% (eq=$", DoubleToString(eq,2), " ref=$", DoubleToString(g_dayStartBal,2), ")");
         return;
      }
   }

   // 2. Daily Loss %: balance actual vs balance al inicio del día
   //    Pérdidas realizadas (cierres) + pérdidas flotantes vía equity.
   //    Usar equity en lugar de balance para detectar DD antes del cierre.
   if(InpMaxDailyLossPct > 0 && g_dayStartBal > 0)
   {
      double daily_loss = (g_dayStartBal - eq) / g_dayStartBal * 100.0;
      if(daily_loss >= InpMaxDailyLossPct)
      {
         g_haltedToday = true;
         Print("MM7 HALT: MaxDailyLoss=", InpMaxDailyLossPct, "% alcanzado. loss=", DoubleToString(daily_loss,2), "% (eq=$", DoubleToString(eq,2), " ref=$", DoubleToString(g_dayStartBal,2), ")");
      }
   }
}
...
void DrawDashboard()
{
   ...
   // v16.7 DD FIX: mostrar DD real vs referencia fija del día (g_dayStartBal)
   // No usar (bal-eq)/bal que se resetea con cada cierre manual
   double dd_ref = (g_dayStartBal > 0) ? g_dayStartBal : bal;
   double dd=(dd_ref>0)?(dd_ref-eq)/dd_ref*100.0:0; // DD vs inicio del día
   datetime ds=StringToTime(TimeToString(TimeCurrent(),TIME_DATE));
   ...
   DashLbl("0","[ MoneyMachine7 v16.9 ]",clrGold,       0*lh,x,y,co,fn);
   string warmup_str = g_warmupDone ? "" : " [WARMUP "+IntegerToString((int)(TimeCurrent()-g_warmupStartTime))+"s/~15min]";
   DashLbl("1","Mode : "+ms+(hv?" [HiVol→Sweep]":"")+warmup_str+" Trail:"+(Enable_Trailing_Profit?"ON":"off")+" SLg:"+IntegerToString(SL_Guard_Secs)+"s",g_warmupDone?clrCyan:clrOrangeRed,1*lh,x,y,co,fn);
   ...
   // v16.7: trail escalado por lot — mismos PUNTOS que backtest
   double curLot = CalcLot();
   double lScaleD = (!(bool)MQLInfoInteger(MQL_TESTER) && curLot>0 && Trail_Lot_Ref>0)
                    ? MathMax(0.1, MathMin(curLot/Trail_Lot_Ref, 2.0)) : 1.0;
   string trail_str="Act:$"+DoubleToString(TP_Trail_Activate_USD*lScaleD,2)+" Ret:$"+DoubleToString(TP_Trail_Retrace_USD*lScaleD,2)+(lScaleD!=1.0?" [×"+DoubleToString(lScaleD,2)+"]":" [FIJO]");
   DashLbl("trl","Trail "+trail_str,clrCyan,10*lh,x,y,co,fn);
   string cool_str="";
   ...
   int cd_remain=(int)(((bool)MQLInfoInteger(MQL_TESTER)?g_lastEntryTime:g_lastOpenAttemptTime)+Entry_Cooldown_Secs-now_d);
...
int OnInit()
{
   ...
   // v16.7 FIX 4: MA live para filtro tendencia intraday (solo en live)
   if(Enable_Trend_Filter && !(bool)MQLInfoInteger(MQL_TESTER))
   {
      g_hMA_Live=iMA(g_sym,PERIOD_M15,Trend_MA_Period_Live,0,MODE_SMA,PRICE_CLOSE);
      if(g_hMA_Live==INVALID_HANDLE){Alert("MA_Live failed");return INIT_FAILED;}
   }
   ...
   // v16.7 DD FIX: capturar EQUITY como referencia del día, no solo balance.
   // Si el EA se reinicia a media sesión con posiciones abiertas en pérdida,
   // ACCOUNT_BALANCE no refleja esas pérdidas flotantes.
   // Usando equity: g_dayStartBal = balance + P&L flotante actual → referencia real.
   // Si ya hay pérdidas al reiniciar → el límite diario se mide desde ese punto.
   // NOTA: si se reinicia con posiciones en ganancia, la referencia sube → más conservador.
   {
      double bal_init = AccountInfoDouble(ACCOUNT_BALANCE);
      double eq_init  = AccountInfoDouble(ACCOUNT_EQUITY);
      // Usar el MENOR entre balance y equity: protege en cualquier escenario
      g_dayStartBal = MathMin(bal_init, eq_init);
   }
   g_legacyClosedAtLastG2=CountLegacyClosedToday();
   // v16.3: inicializar warmup guard
   g_warmupDone      = false;
   g_warmupStartTime = TimeCurrent();
   // v16.3 LIVE FIX: capturar baseline de barras cerradas al arrancar
   // Warmup real: esperar que iTime(M5,1) e iTime(M15,1) avancen = barra nueva
   // En backtest: (bool)MQLInfoInteger(MQL_TESTER)==true → se salta esta lógica → warmup inmediato
   g_warmup_M5_bar  = (Enable_MTF_Confluence) ? iTime(g_sym, PERIOD_M5,  1) : TimeCurrent();
   g_warmup_M15_bar = (Enable_MTF_Confluence) ? iTime(g_sym, PERIOD_M15, 1) : TimeCurrent();
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.9 | LIVE PARITY ENGINE | Mode=",ms,
         " | PredScore>=",Min_Prediction_Score,"/5",
         " | SL_Guard=",SL_Guard_Secs,"s SpreadComp=",Enable_Spread_SL_Comp,
         " | TrailLotRef=",Trail_Lot_Ref," TrendLive=MA",Trend_MA_Period_Live,"(M15)",
         " | v16.8: vTP_spread+BarLock+Throttle+PostLock bifurcados MQL_TESTER");
   // ── v16.6 FIX CRITICO: recuperar posiciones existentes al reiniciar ──────
   // Si el EA se reinicia (reconnect, chart reload, MT5 restart), g_sp[] queda vacio.
   // Sin este codigo, cualquier posicion abierta NUNCA cierra porque StealthCheckAll
   // no tiene ningun registro → posiciones abiertas indefinidamente → perdidas enormes.
   // Solucion: registrar posiciones existentes con vSL de seguridad (vTP=0=desactivado).
   if(Use_Stealth_Mode)
   {
      for(int p=0;p<PositionsTotal();p++)
      {
         ...
         if(PositionGetString(POSITION_SYMBOL)!=g_sym) continue;
         if(PositionGetInteger(POSITION_MAGIC)!=g_magic) continue;
         long posT=(long)PositionGetInteger(POSITION_TYPE);
         int  posDir=(posT==POSITION_TYPE_BUY)?1:-1;
         double posEntry=PositionGetDouble(POSITION_PRICE_OPEN);
         // vSL de seguridad: entry ± SL_FIXED (misma logica que OpenOrder)
         // vTP=0 desactivado (no sabemos el TP exacto original)
         double recSL = posEntry - posDir * MM7_SL_FIXED;
         double recTP = 0; // desactivado — solo SL protege
         StealthRegister(tk, recTP, recSL, posDir);
         // v16.7: marcar openTime como 0 para que SL_Guard no aplique a posiciones previas al reinicio
         // (ya llevan tiempo abiertas, la protección de primer tick no tiene sentido)
         if(g_spCnt > 0) g_sp[g_spCnt-1].openTime = 0;
         Print("MM7 v16.7 | RECOVERY: ticket=",tk," dir=",posDir," entry=",posEntry," vSL=",recSL);
      }
   }
   EventSetTimer(5);
   // v16.8 FIX: inicializar g_prevBid/g_prevAsk para que el throttle funcione desde el 1er tick.
   // Sin esto, las posiciones recuperadas en OnInit tienen openTime=0 (SL_Guard inactivo)
   // y prevBid=0 (throttle inactivo) → pueden cerrarse en el primer tick si el precio cruzó vSL.
   if(!(bool)MQLInfoInteger(MQL_TESTER))
   {
      g_prevBid = SymbolInfoDouble(g_sym, SYMBOL_BID);
      g_prevAsk = SymbolInfoDouble(g_sym, SYMBOL_ASK);
   }
...
void OnDeinit(const int reason)
{
   ...
   if(g_hMA_Live!=INVALID_HANDLE)IndicatorRelease(g_hMA_Live);
...
{
   StealthCheckAll();
   // v16.7 DD FIX: DailyReset y CheckHaltConditions corren SIEMPRE,
   // ANTES del warmup guard. Antes corrían después del warmup → durante
   // el warmup (hasta 15 min) los límites de DD no se aplicaban.
   // Ahora: si el DD se supera durante el warmup → halt inmediato.
   DailyReset();
   CheckHaltConditions();
   if(g_haltedToday){DrawDashboard();return;}

   // v16.6 LIVE FIX: Recovery scan de posiciones huerfanas
   // Si OpenOrder fallo al registrar el ticket en g_sp[], la posicion queda sin vTP/vSL
   // Este scan detecta posiciones abiertas con nuestro magic que no estan en g_sp[]
   // y las registra usando precio actual como referencia (evita que queden abiertas horas)
   if(!(bool)MQLInfoInteger(MQL_TESTER)) StealthRecoverOrphans();
   ...
   // ── v16.3 WARMUP GUARD (bifurcado IsTesting) ────────────────────
   // BACKTEST: (bool)MQLInfoInteger(MQL_TESTER)==true → warmup inmediato (tester precarga historico)
   //           Comportamiento 100% identico a v16.1 en backtest.
   // LIVE:     Espera que cierren barras NUEVAS de M5 y M15 desde el arranque.
   //           Evita que EMAs con datos de hace 14+ min puntuen direccion incorrecta.
   //           FIX raiz del problema "solo SELLs en live".
   if(!g_warmupDone)
   {
      if((bool)MQLInfoInteger(MQL_TESTER))
      {
         // BACKTEST: warmup inmediato — identico a v16.1
         // CopyBuffer ya tiene datos precargados desde tick 1
         double em5_check[]; ArraySetAsSeries(em5_check, true);
         double em15_check[]; ArraySetAsSeries(em15_check, true);
         bool m5_ready  = (g_hEMA_M5  == INVALID_HANDLE) || (CopyBuffer(g_hEMA_M5,  0, 1, 3, em5_check)  >= 3);
         bool m15_ready = (g_hEMA_M15 == INVALID_HANDLE) || (CopyBuffer(g_hEMA_M15, 0, 1, 3, em15_check) >= 3);
         if(m5_ready && m15_ready)
         {
            g_warmupDone = true;
            // No print en backtest para no saturar el log
         }
         else { return; }
      }
      else
      {
         // LIVE: warmup real — esperar barra nueva cerrada en M5 Y M15
         // Comparar iTime actual vs baseline capturado en OnInit
         datetime cur_M5_bar  = (!Enable_MTF_Confluence || g_hEMA_M5  == INVALID_HANDLE) ? TimeCurrent() : iTime(g_sym, PERIOD_M5,  1);
         datetime cur_M15_bar = (!Enable_MTF_Confluence || g_hEMA_M15 == INVALID_HANDLE) ? TimeCurrent() : iTime(g_sym, PERIOD_M15, 1);
         bool m5_new  = (cur_M5_bar  > g_warmup_M5_bar);
         bool m15_new = (cur_M15_bar > g_warmup_M15_bar);
         int secs = (int)(TimeCurrent() - g_warmupStartTime);
         if(m5_new && m15_new)
         {
            g_warmupDone = true;
            Print("MM7 v16.9 | WARMUP LIVE COMPLETADO tras ", secs, "s | M5+M15 barras nuevas | Senales activadas");
         }
         else
         {
            if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
            {
               Comment("MM7 v16.7 | WARMUP LIVE: M5:", (m5_new?"OK":"wait"), " M15:", (m15_new?"OK":"wait"), " (", secs, "s)");
               g_lastDashTime = TimeCurrent();
            }
            return;
         }
   ...
   if(CountLegacyOpen()==0)
   {
      ...
      // 2. Entry cooldown: tiempo mínimo entre entradas consecutivas
      //    v16.7 FIX 3: en live usar g_lastOpenAttemptTime (se actualiza en OpenOrder
      //    independientemente del cierre de la posición). Evita saltarse el cooldown
      //    cuando un trade de 0s resetea CountLegacyOpen inmediatamente.
      //    En backtest: usar g_lastEntryTime (comportamiento original v16.6)
      datetime coolRef = (bool)MQLInfoInteger(MQL_TESTER) ? g_lastEntryTime : g_lastOpenAttemptTime;
      if(now - coolRef < Entry_Cooldown_Secs){DrawDashboard();return;}

      // v16.8 FIX: verificar bar lock ANTES de llamar GetSignal()
      // GetSignal() hace 10+ CopyBuffer y cálculos — si la barra ya fue usada, saltarlo.
      // En backtest: solo 1 tick/barra → esta verificación es siempre false → sin impacto.
      if(!(bool)MQLInfoInteger(MQL_TESTER))
      {
         datetime curBarPre = iTime(g_sym, _Period, 0);
         if(curBarPre == g_lastEntryBar){DrawDashboard();return;}
      }
      ...
      // v16.8 GAP 2: BAR LOCK en live (segunda verificación tras GetSignal para race conditions)
      // En backtest: 1 tick/barra → imposible abrir 2 trades en la misma barra M1
      // En live: múltiples ticks/segundo → sin bar lock puede abrir en cada tick
      // Lock: si ya se abrió un trade en esta barra M1, no abrir otro en la misma barra
      if(sig != 0 && !(bool)MQLInfoInteger(MQL_TESTER))
      {
         datetime curBar = iTime(g_sym, _Period, 0);
         if(curBar == g_lastEntryBar) sig = 0; // misma barra → bloquear
      }
      ...
      if(sig==1)
      {
         ...
         if(tk>0)
         {
            g_lastEntryTime=now; g_lastSignalBar=now; g_lastLegacyTicket=tk;
            // v16.8: guardar barra actual para bar lock
            if(!(bool)MQLInfoInteger(MQL_TESTER)) g_lastEntryBar = iTime(g_sym, _Period, 0);
            // v16.8 GAP 6: reforzar post-close lockout desde apertura
            // Garantiza separación mínima entre trades sin depender del cierre
            g_postCloseLockout = TimeCurrent() + PostClose_Lockout_Secs;
         }
      }
      if(sig==-1)
