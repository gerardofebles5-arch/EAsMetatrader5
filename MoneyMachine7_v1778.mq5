//+------------------------------------------------------------------+
//|  MoneyMachine7_v1778.mq5                                        |
//|  v17.78 — BASE v17.75 + Balance Trailing + WR Tracker           |
//|                                                                  |
//|  APRENDIZAJE TOTAL DE LA HISTORIA:                              |
//|                                                                  |
//|  ✓ BASE: v17.75 (mejor versión: WR=64%, EV=$6.39, Sharpe=1.10) |
//|  ✗ NO TOCAR: señal, horas, trailing, HardCap, régimen           |
//|  ✗ LECCIONES: umbral % del TP → siempre inalcanzable            |
//|                lot dinámico vía risk% → aplana a 0.01 siempre   |
//|                demasiados cambios a la vez → deadlock/quema      |
//|                                                                  |
//|  CAUSA DE CAÍDA MAR 17-19 (v17.77):                             |
//|  H15 pasó de WR=79% a WR=14% en 3 días.                        |
//|  WR rolling de 80% a 10% en 20 trades.                          |
//|  Sistema siguió operando a tamaño completo: -$108               |
//|                                                                  |
//|  2 SISTEMAS NUEVOS (no afectan la señal ni el timing):          |
//|                                                                  |
//|  1. BALANCE TRAILING STOP                                        |
//|     Igual que un trailing stop en precio, pero sobre el balance  |
//|     Peak del balance → cuando cae X% → reduce lot               |
//|     Simulación sobre v17.77: $357 vs $274 (+$83)                |
//|     DD 0-10%: lot×1.0 | DD 10-15%: lot×0.5 | DD>15%: lot×0.25 |
//|                                                                  |
//|  2. WR TRACKER EN TIEMPO REAL                                   |
//|     WR rolling de las últimas 8 operaciones (sin importar hora) |
//|     WR<30% → lot×0.30 (frena en malas rachas inmediatamente)    |
//|     WR>70% → lot×1.50 (aprovecha rachas ganadoras)             |
//|     Reacciona en 5 trades, no en 10 por hora como el autolearn  |
//|                                                                  |
//|  3. PARTIAL EN $ FLOTANTES (de v17.76)                          |
//|     PnL flotante >= $8 → cerrar 40% → funciona en el 75% wins  |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.78"
#property strict
...
input int    InpMagicNumber      = 177800;
input int    InpSlippagePoints   = 10;

//=== RÉGIMEN (v17.75 — no pausa completa) ===
input int    Regime_Trend_Bars   = 60;
input double Regime_Trend_Pts    = 8.0;
input double Regime_Lot_Scale    = 0.50;
input int    Regime_Bad_Days     = 1;
input int    Regime_Good_Days    = 1;

//=== SISTEMA 1: BALANCE TRAILING STOP ===
// Funciona como trailing stop pero sobre el BALANCE
// Cuando el balance cae X% desde el pico → reduce lot automáticamente
// Simulación v17.77: $357 vs $274 (+$83)
input bool   Use_Balance_Trail   = true;
input double BalTrail_DD1_Pct    = 0.10; // DD>10% → lot×BalTrail_Scale1
input double BalTrail_Scale1     = 0.50; // lot×0.5 en DD moderado
input double BalTrail_DD2_Pct    = 0.15; // DD>15% → lot×BalTrail_Scale2
input double BalTrail_Scale2     = 0.25; // lot×0.25 en DD severo

//=== SISTEMA 2: WR TRACKER EN TIEMPO REAL ===
// WR rolling de las últimas N operaciones — más rápido que autoaprendizaje
// Reacciona en 5-8 trades, no en 10 por hora como el autolearn por hora
input bool   Use_WR_Tracker      = true;
input int    WR_Track_Window     = 8;
input double WR_Track_Low        = 0.30;
...
void UpdateWRTracker(bool is_win)
{
   ...
   if(window < 3) return; // pocas muestras → neutro
   double sum = 0;
   ...
   if(wr_roll <= WR_Track_Low)       g_wr_track_scale = WR_Track_Scale_Low;
   else if(wr_roll >= WR_Track_High) g_wr_track_scale = WR_Track_Scale_High;
   else {
      // Escala lineal entre Low y High
   ...
   if(MathAbs(g_wr_track_scale - prev_scale) > 0.05)
      Print("WR_TRACK: wr=",DoubleToString(wr_roll*100,1),"% → lot×",
            DoubleToString(g_wr_track_scale,2));
}
...
int OnInit()
{
   ...
   Print("MM7 v17.78 | ",g_sym,
         " | BASE=v17.75 + BalTrail + WR_Tracker + Partial$",Partial_USD_Trigger,
         " | BalTrail DD1=",BalTrail_DD1_Pct*100,"% DD2=",BalTrail_DD2_Pct*100,"%",
         " | WR_Track w=",WR_Track_Window," lo=",WR_Track_Low*100,"% hi=",WR_Track_High*100,"%",
         " | Regimen pause@",Regime_Bad_Days," retoma@",Regime_Good_Days);
