//+------------------------------------------------------------------+
//|                         XAUUSD_V12_2_ASIAN_BREAKOUT.mq5          |
//|                                    Estrategia Breakout + Pullback |
//|                                                                    |
//| V12.2 ASIAN RANGE BREAKOUT:                                       |
//| - Define rango asiático (00:00-07:00)                             |
//| - Solo opera si precio rompe ese rango                            |
//| - Opera en dirección del rompimiento                              |
//| - Mantiene lógica breakout + pullback                             |
//| - Sin breakeven                                                   |
//| - Máximo 1 trade por día                                          |
//|                                                                    |
//| Timeframe: M15                                                     |
//| Filtro: EMA200 H1 + Rango Asiático                                |
...
//+------------------------------------------------------------------+
#property copyright "Asian Breakout System"
...
input int InpMaxTradesPerDay = 1;               // Máximo trades por día (CAMBIADO)
...
input int InpAsianStartHour = 0;                // Hora inicio sesión asiática
input int InpAsianEndHour = 7;                  // Hora fin sesión asiática
input int InpMinRangePips = 20;                 // Rango mínimo válido (pips)
...
input int InpMagicNumber = 120201;              // Magic Number
...
datetime asianRangeDate = 0;
bool asianRangeValid = false;
int asianBreakoutDirection = 0; // 1=alcista, -1=bajista, 0=sin breakout
...
int OnInit()
{
   ...
   Print("═══════════════════════════════════════════════");
   Print("  EA V12.2 ASIAN RANGE BREAKOUT");
   Print("═══════════════════════════════════════════════");
   ...
   Print("Sesión asiática: ", InpAsianStartHour, ":00 - ", InpAsianEndHour, ":00");
   Print("Sin breakeven");
   Print("═══════════════════════════════════════════════");
...
void OnTick()
{
   ...
   // Actualizar rango asiático si es necesario
   UpdateAsianRange();
   ...
   // Gestionar posiciones abiertas (sin breakeven)
   ManageOpenPositions();
   ...
   if(!asianRangeValid) {
      Comment("Esperando rango asiático válido...");
      return;
   }
   
   // Detectar breakout del rango asiático (solo una vez por día)
   if(asianBreakoutDirection == 0) {
      DetectAsianRangeBreakout();
   }
   
   // Si ya hubo breakout, buscar señales en esa dirección
   if(asianBreakoutDirection != 0) {
...
void UpdateAsianRange()
{
   ...
   // Si cambió el día, recalcular rango
   if(today != asianRangeDate) {
      CalculateAsianRange();
      asianRangeDate = today;
      asianBreakoutDirection = 0; // Reset dirección de breakout
   }
}

//+------------------------------------------------------------------+
//| Calcular rango asiático del día actual                            |
//+------------------------------------------------------------------+
void CalculateAsianRange()
{
   ...
   // Si aún estamos en sesión asiática, esperar
   if(timeStruct.hour < InpAsianEndHour) {
      asianRangeValid = false;
      return;
   }
   
   // Buscar high/low entre las horas definidas
   double high = 0;
   double low = DBL_MAX;
   ...
   for(int i = 1; i < 100; i++) { // Buscar en últimas 100 velas
      ...
      // Verificar si la barra está en el rango horario asiático de hoy
      if(timeStruct.hour >= InpAsianStartHour && timeStruct.hour < InpAsianEndHour) {
      ...
      // Si llegamos a ayer, parar
      if(timeStruct.hour >= InpAsianEndHour && i > 30)
         break;
   }
   
   // Verificar que el rango sea válido
   double rangePips = (high - low) / (_Point * 10);
   
   if(rangePips >= InpMinRangePips && high > 0 && low < DBL_MAX) {
      asianRangeHigh = high;
      asianRangeLow = low;
      asianRangeValid = true;
      
      Print("✓ Rango asiático calculado:");
      Print("  High: ", asianRangeHigh);
      Print("  Low: ", asianRangeLow);
      Print("  Rango: ", DoubleToString(rangePips, 1), " pips");
   }
   else {
      asianRangeValid = false;
      Print("⚠ Rango asiático inválido (", DoubleToString(rangePips, 1), " pips < ", InpMinRangePips, " pips)");
   }
}

//+------------------------------------------------------------------+
//| Detectar breakout del rango asiático                              |
//+------------------------------------------------------------------+
void DetectAsianRangeBreakout()
{
   ...
   // Breakout alcista
   if(high1 > asianRangeHigh && close1 > asianRangeHigh) {
      asianBreakoutDirection = 1;
      Print("🔼 BREAKOUT ALCISTA del rango asiático en ", asianRangeHigh);
      Print("   Solo se permitirán trades LONG hoy");
   }
   
   // Breakout bajista
   if(low1 < asianRangeLow && close1 < asianRangeLow) {
      asianBreakoutDirection = -1;
      Print("🔽 BREAKOUT BAJISTA del rango asiático en ", asianRangeLow);
      Print("   Solo se permitirán trades SHORT hoy");
   }
}
...
void DetectSwingBreakout()
{
   ...
   // Breakout alcista (SOLO si dirección asiática es alcista)
   if(asianBreakoutDirection == 1) {
   ...
   // Breakout bajista (SOLO si dirección asiática es bajista)
   if(asianBreakoutDirection == -1) {
...
void OpenTrade(ENUM_ORDER_TYPE orderType, double entryReference)
{
   ...
   // Ejecutar orden
   string comment = StringFormat("Asian_V12.2_RR%.1f", InpRiskReward);
   
   if(orderType == ORDER_TYPE_BUY) {
      if(trade.Buy(lotSize, _Symbol, entryPrice, sl, tp, comment)) {
         Print("✓ LONG abierto: Lote=", lotSize, " Entry=", entryPrice, " SL=", sl, " TP=", tp);
         Print("  Dirección confirmada por breakout asiático alcista");
         tradesToday++;
      ...
      if(trade.Sell(lotSize, _Symbol, entryPrice, sl, tp, comment)) {
         ...
         Print("  Dirección confirmada por breakout asiático bajista");
         tradesToday++;
         lastTradeDate = TimeCurrent();
      }
      else {
...
{
   // Sin breakeven en V12.2
   // Solo mantener posiciones hasta TP o SL
   return;
}
...
void UpdateComment()
{
   ...
   string asianStatus = "Calculando...";
   if(asianRangeValid) {
      double rangePips = (asianRangeHigh - asianRangeLow) / (_Point * 10);
      asianStatus = StringFormat("High: %.2f | Low: %.2f (%.1f pips)", 
                                 asianRangeHigh, asianRangeLow, rangePips);
   }
   
   string breakoutDir = "Esperando breakout";
   if(asianBreakoutDirection == 1) breakoutDir = "🔼 ALCISTA (solo LONG)";
   if(asianBreakoutDirection == -1) breakoutDir = "🔽 BAJISTA (solo SHORT)";
