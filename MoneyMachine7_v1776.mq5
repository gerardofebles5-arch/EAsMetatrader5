...
//=== GESTION DE RACHA — ajustada con datos reales v17.73 ===
// Tras 3 losses: WR=20% → parar antes y más tiempo
// Tras 3 wins:   WR=73% → apostar más, aprovechar momentum
input double Streak_Reduce_Pct   = 0.80;  // ×0.80 por cada loss (antes 0.50 de golpe)
input double Streak_Boost_Pct    = 1.30;  // ×1.30 por cada win  (antes 1.10)
input double Streak_Scale_Max    = 1.50;  // techo: nunca más de ×1.5
input double Streak_Scale_Min    = 0.40;  // piso:  nunca menos de ×0.4
input int    Streak_Loss_Trigger = 3;     // CAMBIADO: 2→3 losses para activar reducción
input int    Streak_Win_Trigger  = 3;     // 3 wins consecutivos para activar boost
...
input int    Max_Consec_Losses   = 3;     // CAMBIADO: 4→3 (pausa tras 3 losses)
input int    Pause_Bars          = 8;     // CAMBIADO: 5→8 barras de pausa
...
input int    InpMagicNumber      = 177600;
...
input int    Regime_Good_Days    = 1;     // CAMBIADO: 2→1 (retomar tras 1 día bueno)
...
int OnInit()
{
   ...
   Print("MM7 v17.76 | ",g_sym,
         " | BASE=v17.75 + Partial$",Partial_USD_Trigger,"@",Partial_Close_Pct*100,"%",
         " | Horas:2,7,15,22 | HardCap=$",Hard_Loss_Cap_USD,
