...
int GetMTFConfluence(int dir)
{
   ...
   if(g_hEMA_M5 != INVALID_HANDLE)
   {
      ...
      if(CopyBuffer(g_hEMA_M5,0,1,1,em5)>=1)
      {
         double bid = SymbolInfoDouble(g_sym, SYMBOL_BID);
   ...
   if(g_hEMA_M15 != INVALID_HANDLE)
   {
      ...
      if(CopyBuffer(g_hEMA_M15,0,1,1,em15)>=1)
      {
         double bid = SymbolInfoDouble(g_sym, SYMBOL_BID);
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.1 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   // v16.1: inicializar warmup guard
   g_warmupDone      = false;
   g_warmupStartTime = TimeCurrent();
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.1 | WARMUP GUARD fix live | Mode=",ms,
         ...
         " | v16.1 FIX: warmup guard bloquea señales hasta que EMA_M5/M15 esten calientes");
...
void OnTick()
{
   ...
   // ── v16.1 WARMUP GUARD ────────────────────────────────────────────
   // Bloquea señales hasta que EMA_M5 y EMA_M15 tengan datos frescos.
   // En live los indicadores MTF arrancan con valores de hace 21+ minutos.
   // Necesitamos esperar a que cierre al menos 1 barra de M15 (15 min)
   // desde que arrancó el EA. Eso garantiza que la EMA21_M15 ya procesó
   // precio actual y GetMTFConfluence() no puntúa la dirección incorrecta.
   if(!g_warmupDone)
   {
      // Verificar que haya pasado suficiente tiempo desde el arranque
      // y que CopyBuffer de M5 y M15 devuelvan datos válidos
      int seconds_running = (int)(TimeCurrent() - g_warmupStartTime);
      
      // Necesitamos al menos 1 barra M15 cerrada = 900 segundos mínimo
      // Pero también verificamos directamente que los buffers tengan datos:
      ...
      bool time_ready = (seconds_running >= 900); // mínimo 15 minutos = 1 barra M15
      
      if(m5_ready && m15_ready && time_ready)
      {
         g_warmupDone = true;
         Print("MM7 v16.1 | WARMUP COMPLETADO tras ", seconds_running, "s | EMA_M5 y EMA_M15 listas | Señales activadas");
      }
      else
      {
         // Durante warmup: solo actualizar dashboard con estado de espera
         // No abrir ningún trade — los indicadores MTF no son confiables aún
         if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
         {
            int remaining = MathMax(0, 900 - seconds_running);
            Comment("MM7 v16.1 | WARMUP: esperando ", remaining, "s más | M5:", (m5_ready?"OK":"wait"), " M15:", (m15_ready?"OK":"wait"));
            g_lastDashTime = TimeCurrent();
         }
         return; // NO procesar señales durante warmup
      }
   }
   // ── FIN WARMUP GUARD ─────────────────────────────────────────────
