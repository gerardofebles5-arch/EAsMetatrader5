//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v16.0 — PREDICTIVE INTELLIGENCE ENGINE (BALANCED BUY/SELL)   |
//|                                                                  |
//|  FIX v16: PROBLEMA CRITICO DETECTADO EN LIVE v15               |
//|  Solo abría SELL porque GetEMAMomentum exige cruce EMA3/EMA8   |
//|  en formación — en Gold alcista ese cruce nunca ocurre para    |
//|  BUY ya que EMA3 lleva horas sobre EMA8. BUY score=0 siempre. |
//|                                                                  |
//|  SOLUCIÓN v16:                                                  |
//|  GetEMAMomentum reemplazado por GetMomentumBias: mide si EMA   |
//|  rápida es más alcista/bajista que lenta (posición relativa +  |
//|  aceleración) no solo cruce. Funciona en cualquier régimen.    |
//|                                                                  |
//|  TRAILING INTELIGENTE por Lotaje:                              |
//|  Los umbrales TP_Trail_Activate y TP_Trail_Retrace se escalan  |
//|  automáticamente según el lotaje real de la posición.          |
//|  Ej: lot=0.05 → umbral×0.5; lot=0.10 → umbral×1.0            |
//|  Trailing Stop también escala distancia por ATR × lot.        |
//|                                                                  |
...
int GetMomentumBias(int dir)
{
   ...
   // Dirección del gap (primera derivada)
   double gap_delta = gap0 - gap1; // positivo = gap creciendo, negativo = gap reduciéndose
   (void)gap_delta; // referencia futura
...
void StealthRecoverOrphans()
{
   ...
   for(int p = PositionsTotal()-1; p >= 0; p--)
   {
      ...
      Print("MM7 v16.6 | ORPHAN RECOVERED: ticket=", tk, " dir=", dir,
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      // ── Trailing Profit NO dinamico v16.6 ────────────────────────
      // v16.6 FIX: umbrales FIJOS en USD — NO se escalan por lotaje.
      // Versiones anteriores multiplicaban por (lot/0.10) lo que con lot=0.01
      // daba activate=$0.05 y retrace=$0.03 — se cerraba demasiado rapido
      // o con cualquier fluctuacion de spread. Ahora: valores directos del input.
      if(Enable_Trailing_Profit || Enable_MinUSD_TP)
      {
         double activateUSD = TP_Trail_Activate_USD; // FIJO: $0.50 sin escalar
         double retraceUSD  = TP_Trail_Retrace_USD;  // FIJO: $0.30 sin escalar
         double minUSD      = TP_Min_USD;             // FIJO sin escalar

         // MODO 1: TP por USD minimo — cierre inmediato
         if(Enable_MinUSD_TP && curProfit >= minUSD)
            cls = true;

         // MODO 2: Trailing Profit fijo
         if(!cls && Enable_Trailing_Profit)
         {
            // Actualizar maximo profit alcanzado
            if(curProfit > g_sp[i].trailMaxProfit)
               g_sp[i].trailMaxProfit = curProfit;

            // Activar trailing cuando profit alcanza umbral fijo
            if(!g_sp[i].trailActivated && curProfit >= activateUSD)
               g_sp[i].trailActivated = true;

            // Cerrar cuando el profit retrocede desde el maximo >= umbral fijo
      ...
      if(!cls && Enable_TrailingStop)
      {
         ...
         // TrailingStop: distancias basadas en ATR puro (no escaladas por lot)
         // El mercado se mueve independiente del tamaño del lot
      ...
      // v16.3 FIX CRITICO: el vSL SIEMPRE actua como red de seguridad.
      // Antes: con Enable_TrailingStop=true el vSL original se ignoraba aunque
      // el trailing aun no se habia activado → posicion sin SL real en live.
      // Ahora: vSL siempre activo. Si trail movio el vSL → ese nivel protege.
      //        Si trail no activado aun → vSL original cierra la posicion.
      // En BACKTEST: comportamiento equivalente porque el tester cierra por sus
      // propios TP/SL (que aqui son 0, stealth) y el codigo virtual era el real.
      if(!cls)
      {
         // TP fijo virtual — siempre aplica (TrailingProfit puede cerrar antes)
         if(g_sp[i].dir== 1 && g_sp[i].vTP > 0 && bid >= g_sp[i].vTP) cls = true;
         if(g_sp[i].dir==-1 && g_sp[i].vTP > 0 && ask <= g_sp[i].vTP) cls = true;
         // SL virtual — SIEMPRE actua como red de seguridad
         // Con TrailingStop activo y trail movido: vSL = nivel del trail (protege ganancia)
         // Con TrailingStop no activado aun: vSL = SL original (protege capital)
         if(!cls)
         {
...
ulong OpenOrder(ENUM_ORDER_TYPE type,string comment,double slMul)
{
   ...
   // v16.5 LIVE FIX: calcular vTP y vSL sobre BID para BUY (no sobre ASK).
   // En backtest spread≈0 → ASK≈BID → sin diferencia.
   // En live spread Gold 0.2-0.5pts → vTP sobre ASK hace el TP efectivamente 
   // 0.3-0.5pts más difícil de alcanzar → muchos trades que deberian llegar al TP no llegan.
   // Usando BID como referencia: bid_close >= bid_entry + 2.50 → condicion uniforme.
   // v16.6: vTP/vSL sobre ENTRY directo (ASK para BUY, BID para SELL).
   // Igual que v16.1 original. No compensar spread — lo importante es FIX 1.
   ...
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED)
   {
      ...
      // CAUSA DEL BUG: con filling IOC/RETURN, res.deal puede ser 0 en live.
      // Cuando res.deal==0 -> HistoryDealSelect falla -> StealthRegister nunca
      // se llama -> g_sp[] no tiene el ticket -> StealthCheckAll no cierra nunca
      // -> trades de 7min, 34min en live vs 13s promedio en backtest.

      // METODO 1: res.order -> en cuentas HEDGE el ticket de orden == ticket de posicion
      // En MQL5 HEDGE cada orden de apertura crea una posicion con el mismo ID
      if(res.order > 0 && PositionSelectByTicket(res.order))
         posTk = res.order;

      // METODO 2: res.deal -> DEAL_POSITION_ID (cuando historial ya actualizo)
      ...
      // METODO 3: buscar posicion del tipo correcto abierta hace menos de 5s
      if(posTk == 0)
      {
         datetime now = TimeCurrent();
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.6 ]",clrGold,       0*lh,x,y,co,fn);
   string warmup_str = g_warmupDone ? "" : " [WARMUP "+IntegerToString((int)(TimeCurrent()-g_warmupStartTime))+"s/~15min]";
   DashLbl("1","Mode : "+ms+(hv?" [HiVol→Sweep]":"")+warmup_str+" Trail:"+(Enable_Trailing_Profit?"ON":"off"),g_warmupDone?clrCyan:clrOrangeRed,1*lh,x,y,co,fn);
   ...
   // v16.6 FIX: Trail es FIJO (no escalado por lot) — mostrar valores reales
   string trail_str="Act:$"+DoubleToString(TP_Trail_Activate_USD,2)+" Ret:$"+DoubleToString(TP_Trail_Retrace_USD,2)+" [FIJO]";
   DashLbl("trl","Trail(fijo) "+trail_str,clrCyan,10*lh,x,y,co,fn);
   string cool_str="";
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.6 | LIVE PARITY (backtest identical) | Mode=",ms,
         " | PredScore>=",Min_Prediction_Score,"/5",
         " | MTF=",Enable_MTF_Confluence," OFlow=",Enable_OrderFlow_Bias,
         " | MomBias=",Enable_EMA_Momentum," Structure=",Enable_Price_Structure,
         " | TrailProfit=",Enable_Trailing_Profit," act=$",TP_Trail_Activate_USD,"(FIJO) ret=$",TP_Trail_Retrace_USD,"(FIJO)",
         " | TrailStop=",Enable_TrailingStop," ATR×",TS_Start_ATR_Multiplier,
         " | v16.3: (bool)MQLInfoInteger(MQL_TESTER) bifurca live vs backtest");
   ...
   if(Use_Stealth_Mode)
   {
      for(int p=0;p<PositionsTotal();p++)
      {
         ...
         Print("MM7 v16.6 | RECOVERY: ticket=",tk," dir=",posDir," entry=",posEntry," vSL=",recSL);
      }
   }
...
void OnTick()
{
   ...
   if(!g_warmupDone)
   {
      ...
      else
      {
         ...
         {
            g_warmupDone = true;
            Print("MM7 v16.6 | WARMUP LIVE COMPLETADO tras ", secs, "s | M5+M15 barras nuevas | Senales activadas");
         }
         else
         {
            if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
            {
               Comment("MM7 v16.6 | WARMUP LIVE: M5:", (m5_new?"OK":"wait"), " M15:", (m15_new?"OK":"wait"), " (", secs, "s)");
               g_lastDashTime = TimeCurrent();
            }
            return;
         }
