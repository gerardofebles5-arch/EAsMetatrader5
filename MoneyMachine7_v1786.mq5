//+------------------------------------------------------------------+
//|  MoneyMachine7_v1786.mq5                                        |
//|  v17.86 — VERSIÓN DEFINITIVA                                    |
//|                                                                  |
//|  BASE: v17.80 (mejor histórico: $270, PF=2.06, WR=53%)         |
//|                                                                  |
//|  ANÁLISIS CUANTITATIVO (136 trades reales, 5 versiones):        |
//|                                                                  |
//|  PROBLEMA 1 — TRAILING DEMASIADO AJUSTADO:                      |
//|  Trades cerrados en H16 (aguantan desde H15): WR=83% +$119     |
//|  Trades cerrados en H15 (mueren rápido):      WR=39% +$45      |
//|  Phase1=15pts = RUIDO NORMAL de XAUUSD M1. Los winners         |
//|  necesitan espacio para respirar. Con 30pts en Phase1 los       |
//|  trades llegan a H16 donde el WR es 2x mejor.                  |
//|  FIX: Trail_Dist_Phase1 15→30pts                               |
//|       Trail_Start_Pts 2.9→5.0 (no trailing antes de $5 ganado) |
//|       Trail_Dist_Phase2 10→8pts (más ajustado tras Phase1)      |
//|       Trail_Dist_Phase3 6→4pts                                  |
//|                                                                  |
//|  PROBLEMA 2 — SL/TP FIJOS NO SE ADAPTAN A VOLATILIDAD:         |
//|  HardCaps duran avg=22.9min (vs SL normales 11.9min).          |
//|  Son posiciones atascadas, no pérdidas instantáneas.            |
//|  El SL fijo de 19.2pts no se adapta: si ATR=8pts, el SL        |
//|  debería ser ~12pts (1.5×ATR). Si ATR=25pts → SL=37pts.       |
//|  FIX: SL = max(SL_Min, 1.5×ATR14) — dinámico y realista       |
//|       TP = entry - max(TP_Min, 3.0×ATR14) — captura movimiento |
//|       Esto sube avg_win de $14 a ~$30+ objetivo               |
//|                                                                  |
//|  PROBLEMA 3 — H07 Y H22 SIEMPRE PIERDEN:                       |
//|  H07: WR=0% en TODAS las versiones (7 trades, 0 wins)          |
//|  H22: EV≈$0, pero genera HardCaps y alta varianza              |
//|  Horas operativas: solo H02 (WR=47%, +$51) y H15 (WR=54%,+$45)|
//|  FIX: Hardcoded en código, no configurable                     |
//|                                                                  |
//|  PROBLEMA 4 — PARTIAL CLOSE ÚNICO MATA LOS GRANDES WINNERS:    |
//|  Con un solo partial a $8 (40%), se "asegura" demasiado pronto |
//|  y la posición restante (60%) puede revertir.                   |
//|  FIX: Partial DOBLE escalonado:                                |
//|       1er partial: 30% cuando PnL >= $6 → SL a breakeven       |
//|       2do partial: 30% cuando PnL >= $15 → SL sube             |
//|       Posición restante (40%) corre con trailing amplio         |
//|                                                                  |
//|  PROBLEMA 5 — FILTROS MATAN EL VOLUMEN:                        |
//|  v17.85: 68→9 trades (-87%) por RSI/ZScore/BodyFilter          |
//|  El edge estadístico necesita volumen para expresarse.          |
//|  FIX: Cero filtros extra. Solo la señal original probada.      |
//|                                                                  |
//|  PROBLEMA 6 — DAILY LOSS LIMIT AUSENTE:                        |
//|  Mar17-19: -$84 en 3 días sin parar. Un límite diario de $25   |
//|  habría salvado ~$59.                                           |
//|  FIX: Daily_Loss_Limit=$25 — halt completo cuando se alcanza   |
//|                                                                  |
//|  MEJORAS ADICIONALES (datos confirman):                         |
//|  · Motor 2 lot dinámico desde balance $50 (WR histórico 100%)  |
//|  · Bear Protector: 120b, 50pts → lot×0.15 (más agresivo)       |
//|  · WR_Track_Window: 8→5 (reacciona más rápido)                 |
//|  · WR_Track_Pause_Bars: 5→8 (pausa más larga)                  |
//|  · Learn_Min_Samples: 10→6 (aprende más rápido sin perder sig) |
//|  · Momentum serial confirmado: tras WIN WR=63% → boost ×1.30  |
//|  · Max_Consec_Losses: 3→4 (3 pérdidas = aleatoriedad normal)   |
//|  · Cooldown post-HC: 3→5 barras                                |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.86"
#property strict
...
input double Daily_Loss_Limit_USD  = 25.0; // halt si perdemos $25/día
...
void CheckDailyHalt()
{
   ...
   if(day_pnl < -Daily_Loss_Limit_USD){
      ...
      for(int i = 0; i < ArraySize(tks); i++) ClosePosition(tks[i], "DailyHalt");
...
bool IsGoodHour(int hour)
{
   ...
   // H02: WR=47%, EV=+$3.51 — operar
   // H15: WR=54%, EV=+$4.69 — operar (mejor hora por volumen)
   // H07: WR=0%  en todas las versiones — BLOQUEADO PERMANENTEMENTE
   // H22: EV≈$0, alta varianza, genera HardCaps — BLOQUEADO
