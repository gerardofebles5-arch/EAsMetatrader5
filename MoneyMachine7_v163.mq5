...
// En backtest estas vars no se usan porque IsTesting()==true → warmup inmediato
...
void ScanFVGs()
{
   ...
   datetime bt = IsTesting() ? iTime(g_sym,_Period,0) : iTime(g_sym,_Period,1);
...
int GetMTFConfluence(int dir)
{
   ...
   // En backtest: IsTesting()==true → sin verificacion de frescura (comportamiento v16.1)
   if(g_hEMA_M5 != INVALID_HANDLE)
   {
      double em5[]; ArraySetAsSeries(em5,true);
      bool m5_data_ok = (CopyBuffer(g_hEMA_M5,0,1,1,em5)>=1);
      bool m5_fresh = IsTesting() || (iTime(g_sym, PERIOD_M5, 1) > g_warmup_M5_bar);
   ...
   if(g_hEMA_M15 != INVALID_HANDLE)
   {
      ...
      bool m15_fresh = IsTesting() || (iTime(g_sym, PERIOD_M15, 1) > g_warmup_M15_bar);
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.3 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   // En backtest: IsTesting()==true → se salta esta lógica → warmup inmediato
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.3 | LIVE PARITY (backtest identical) | Mode=",ms,
         ...
         " | v16.3: IsTesting() bifurca live vs backtest");
...
void OnTick()
{
   ...
   // BACKTEST: IsTesting()==true → warmup inmediato (tester precarga historico)
   ...
   if(!g_warmupDone)
   {
      ...
      else
      {
         ...
         {
            g_warmupDone = true;
            Print("MM7 v16.3 | WARMUP LIVE COMPLETADO tras ", secs, "s | M5+M15 barras nuevas | Senales activadas");
         }
         else
         {
            if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
            {
               Comment("MM7 v16.3 | WARMUP LIVE: M5:", (m5_new?"OK":"wait"), " M15:", (m15_new?"OK":"wait"), " (", secs, "s)");
               g_lastDashTime = TimeCurrent();
            }
            return;
         }
