//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v17.9 — SESIÓN NY: SOLO OPERAR DESPUÉS DE 14:30 UTC           |
//|                                                                  |
//|  ANÁLISIS DEFINITIVO (todos los backtests):                     |
//|                                                                  |
//|  TRADES GANADORES (aparecen en TODAS las versiones):            |
//|  14:59 → TP | 16:46 → TP | 17:29 → TP | 18:11 → TP            |
//|  → Todos después de las 14:30 UTC (apertura NY)                 |
//|                                                                  |
//|  TRADES PERDEDORES (aparecen en TODAS las versiones):           |
//|  08:44 → SL | 11:41 → SL | 13:26 → SL | 15:41 → SL            |
//|  → Todos en sesión europea (08-15h UTC)                         |
//|                                                                  |
//|  CONCLUSIÓN: La señal EMA3/8 cruce en M1 tiene edge real        |
//|  en la sesión NY (14:30-20:00 UTC) donde XAUUSD tiene           |
//|  movimientos más limpios y direccionales.                        |
//|  En la sesión europea el mercado es más ruidoso y los           |
//|  cruces se revierten inmediatamente.                             |
//|                                                                  |
//|  CAMBIOS v17.9:                                                  |
//|  FIX 1 — SESIÓN NY ÚNICAMENTE: 14:30-20:00 UTC                 |
//|    Elimina todos los trades perdedores de la mañana             |
//|    Mantiene todos los trades ganadores de la tarde              |
//|  FIX 2 — SEÑAL LIMPIA: solo EMA3/8 cruce + EMA21 M1            |
//|    Sin filtros M5/H1 que bloquean trades buenos                 |
//|    La sesión ya es el filtro más efectivo                       |
//|  FIX 3 — COOLDOWN MÍNIMO: 3s (era 5s)                          |
//|    En NY los movimientos son rápidos, no perder entradas        |
//+------------------------------------------------------------------+
...
// +++ Sesión NY (FIX 1) +++
input int    NY_Start_Hour               = 14;    // 14:30 UTC = apertura NY
input int    NY_Start_Min                = 30;
input int    NY_End_Hour                 = 20;    // cierre liquidez NY
input int    NY_End_Min                  = 0;
// +++ Salida +++
input double MM7_TP_FIXED                = 1.00;
input double MM7_SL_FIXED                = 0.50;
// +++ Entry Control +++
input int    Entry_Cooldown_Secs         = 3;     // FIX 3
...
// FIX 2: señal limpia — solo EMA cruce + EMA21 M1, sin filtros externos
int GetSignal()
{
...
// FIX 1: solo sesión NY 14:30-20:00 UTC
bool IsNYSession()
{
...
void DrawDashboard()
{
   ...
   DashLbl("0", "[ MoneyMachine7 v17.9 ]", clrGold, 0*lh, x, y, co, fn);
   DashLbl("1", "Sesion NY "+IntegerToString(NY_Start_Hour)+":"+IntegerToString(NY_Start_Min)+"-"+IntegerToString(NY_End_Hour)+":00 UTC  "+(inNY?"ACTIVA":"inactiva"), inNY?clrLimeGreen:clrGray, 1*lh, x, y, co, fn);
...
int OnInit()
{
   ...
   Print("MM7 v17.9 | EMA(",EMA_Fast_Period,"/",EMA_Slow_Period,"/",EMA_Trend_M1_Period,") M1 | Sesion NY ",NY_Start_Hour,":",NY_Start_Min,"-",NY_End_Hour,":00 UTC | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED);
