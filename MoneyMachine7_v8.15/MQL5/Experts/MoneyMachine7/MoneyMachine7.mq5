//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.15 — Signal quality + lot fix              |
//|                                                                  |
//|  CAMBIOS vs v8.14:                                               |
//|  1. LOT FIX: round() en lugar de floor() para ~$5000 → 0.05     |
//|     Evita 0.04 cuando balance cae a $4995-$4999                  |
//|  2. COOLDOWN eliminado (bar-throttle ya protege, cooldown=3      |
//|     mataba 67% de señales legítimas)                             |
//|  3. QUALITY GATE → 1 de 2 (solo RSI O solo Volume bastan)       |
//|  4. CANDLE FILTER → soft: solo aplica en pure-stoch mode,        |
//|     se ignora en sweep/FVG (señales de mayor calidad)            |
//|  5. HORA FILTER: horas 10h y 11h requieren Sweep confirmado      |
//|     (no basta Stoch solo — son horas de alta volatilidad falsa)  |
//|  6. BOS (Break of Structure) como confirmador adicional:          |
//|     precio rompe swing reciente en dirección del trade            |
//|  7. Trailing stop corregido: aplica vTP dinámico post TP inicial  |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "8.15"
#property strict
...
// +++ v8.15 Smart Filters +++
input bool   Enable_Candle_Filter        = true;  // candle body direction filter (soft — bypassed by sweep/FVG)
input bool   Enable_BOS_Filter           = true;  // Break of Structure confirmation
input int    BOS_Lookback                = 5;     // bars to confirm BOS
input double ATR_HighVol_Multiplier      = 1.8;   // ATR > avg*this → high-vol, requires sweep
input int    G2_Min_Bar_Gap              = 5;     // min bars legacy open before G2 fires
...
// Candle body direction (bar[1])
int GetCandleDir()
{
   double o=iOpen(g_sym,_Period,1), c=iClose(g_sym,_Period,1);
   double atr=GetATR(); double bodyMin=atr*0.15;
   if(c-o>bodyMin) return 1; if(o-c>bodyMin) return -1; return 0;
}
...
// BUY BOS: most recent N bars had LOWER highs, current bar breaks above them → bullish BOS
// SELL BOS: most recent N bars had HIGHER lows, current bar breaks below → bearish BOS
//--------------------------------------------------------------------
bool HasBOS(int dir)
{
   if(!Enable_BOS_Filter) return true; // if disabled, always pass
   int lb=BOS_Lookback;
   double ask=SymbolInfoDouble(g_sym,SYMBOL_ASK);
   double bid=SymbolInfoDouble(g_sym,SYMBOL_BID);
   if(dir==1)
   {
      // Find recent swing high in bars [1..lb]
      double recentHi=0;
      for(int i=1;i<=lb;i++) { double h=iHigh(g_sym,_Period,i); if(h>recentHi) recentHi=h; }
      // BOS = ask is above recent high (price broke structure upward)
      return (ask > recentHi);
   }
   else
   {
      double recentLo=DBL_MAX;
      for(int i=1;i<=lb;i++) { double l=iLow(g_sym,_Period,i); if(l<recentLo) recentLo=l; }
      return (bid < recentLo);
   }
}
...
// MASTER SIGNAL v8.15
// Key changes:
// - High-vol hours: Sweep only (no fallback to stoch)
// - Quality gate: 1 of 2 (RSI OR Volume) — not 2 of 3
// - Candle filter: soft (bypassed if sweep or FVG source)
// - BOS filter: optional confirmator for pure-stoch entries
//============================================================
int GetSignal()
{
   ...
   // In high volatility: only accept sweep signals (highest precision)
   ...
   {
      // P1: Sweep (highest quality)
      if(Enable_Stop_Hunt_Strategy) { int s=GetSweep(); if(s!=0&&(trend==0||s==trend)) { sig=s; fromSweep=true; } }

      // P2: FVG fill
      if(sig==0 && Enable_FVG_Strategy)
      {
         if(InFVG(1)  && k1<=Stoch_Buy_Level+10  && (trend==0||trend== 1)) { sig= 1; fromFVG=true; }
         if(InFVG(-1) && k1>=Stoch_Sell_Level-10 && (trend==0||trend==-1)) { sig=-1; fromFVG=true; }
      }

      // P3: Pure Stoch (lowest quality — needs extra confirmation)
      if(sig==0)
      {
         bool sBuy=(k1<=Stoch_Buy_Level), sSell=(k1>=Stoch_Sell_Level);
         // RSI quality gate (1 of 2: RSI OR Volume)
         double rsi=GetRSI(); bool volOK=IsVolumeOK();
         bool rsiOK_buy  = (rsi < RSI_Buy_Threshold);
         bool rsiOK_sell = (rsi > RSI_Sell_Threshold);
         bool qualBuy  = sBuy  && (rsiOK_buy  || volOK);
         bool qualSell = sSell && (rsiOK_sell || volOK);
         ...
         // Pure stoch + BOS (avoids choppy market entries)
         if(sig==0)
         {
            if(qualBuy &&(trend==0||trend== 1)&&!qualSell && HasBOS(1))  sig= 1;
            if(qualSell&&(trend==0||trend==-1)&&!qualBuy  && HasBOS(-1)) sig=-1;
         }
      }
   }
   ...
   // Candle body filter — SOFT: only applied for pure-stoch (P3) entries
   // Sweep and FVG entries bypass this (they already have structural confirmation)
   if(Enable_Candle_Filter && !fromSweep && !fromFVG)
   {
      int cd=GetCandleDir();
      if(cd!=0 && cd!=sig) return 0;
   }
...
//--------------------------------------------------------------------
// LOT — FIX v8.15: round() instead of floor()
// floor(4995/1000)=4 → 0.04 BUG
// round(4995/1000)=5 → 0.05 CORRECT
// Original verified: stays at 0.05 until balance reaches ~$5500
//--------------------------------------------------------------------
double CalcLot()
{
   if(!Use_dynamic_lot_) return NormalizeDouble(Lot_,2);
   double bal=AccountInfoDouble(ACCOUNT_BALANCE);
   // Use round() to avoid lot drop on tiny balance dips below round thousands
...
void CheckG2()
{
   ...
   // Bar gap check
   int barsOpen=0;
   for(int i=0;i<G2_Min_Bar_Gap+10;i++){datetime bt=iTime(g_sym,_Period,i);if(bt<=ot)break;barsOpen++;}
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.15 ]",clrGold,       0*lh,x,y,co,fn);
   ...
   DashLbl("7","ATR  : "+DoubleToString(GetATR(),3)+" avg:"+DoubleToString(GetATRAvg(),3),clrGray,7*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.15 | Mode=",ms," | LotFix=round() | BOS=",Enable_BOS_Filter,
         " | CandleFilter=soft | HighVolSweepOnly ATR×",ATR_HighVol_Multiplier,
         " | G2MinBarGap=",G2_Min_Bar_Gap);
