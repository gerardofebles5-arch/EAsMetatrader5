//+------------------------------------------------------------------+
//|  MoneyMachine7_v1789.mq5                                        |
//|  v17.89 — 3 FIXES BASADOS EN DATOS v17.88                      |
//|                                                                  |
//|  BASE: v17.88 ($50->$267 pico, PF=1.92, WR=57%, SIN HC WR=87%)|
//|                                                                  |
//|  FIX 1: H15 Max_Positions=1 SIEMPRE (no dobles HC simultáneos) |
//|  FIX 2: ATR_SL_Mult 1.5->1.0 (SL real < HC, margen entre ellos)|
//|  FIX 3: Trail_Start $5->$10 (no cortar winners en movim. ruido)|
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.89"
#property strict
...
input double ATR_SL_Mult         = 1.0;   // FIX 2: 1.5→1.0 (SL real deja margen antes del HC)
...
input double Trail_Start_Pts     = 10.0;  // FIX 3: $5→$10 (no activar trail en ruido)
...
input double Daily_Loss_Limit_USD  = 20.0; // FIX C: $25→$20
...
// FIX 1: H15 limitada a 1 posición SIEMPRE — elimina los HCs simultáneos
// 03/18 y 03/19: 2 posiciones H15 abiertas en la misma barra → 2 HCs juntos
// H02 puede tener 2 posiciones (sus HCs nunca son simultáneos)
input int    Max_Positions_H15     = 1;   // H15 nunca abre 2da posición
...
input int    HardCap_Cooldown_Bars = 8;    // FIX D: 5→8 barras entre trades tras HC

//=== FIX A: HC DIARIO TRACKER ===
// Tras 1er HC del día → MaxPositions efectivo = 1 (no abrir 2da posición perdedora)
// Tras 2do HC del día → pausa 15 barras ese día (no halt total, permite recuperación)
input bool   Use_HC_Day_Tracker    = true;
input int    HC_Day_Pause_Bars     = 15;   // barras de pausa tras 2do HC en el día

//=== FIX B: BLOQUEO H15 TRAS HC ===
// Si HC ocurre con entrada de H15 → bloquear H15 resto del día
// El precio está en tendencia ese día, la señal H15 será incorrecta
input bool   Use_H15_Block_After_HC = true;
...
bool     g_halt_closed_today = false; // evita cerrar posiciones múltiples veces
int      g_hc_today          = 0;     // FIX A: contador HCs del día actual
bool     g_h15_blocked_today = false; // FIX B: H15 bloqueada por HC hoy
int      g_max_pos_today     = 2;     // FIX A: MaxPositions efectivo del día
...
void CheckDailyHalt()
{
   ...
   // Solo verificar el límite y setear el flag.
   // El cierre de posiciones lo hace OnTick() para evitar
   // llamadas a funciones no definidas aún en este punto del código.
...
// FIX B: Si HC ocurrió en H15 hoy → bloquear H15 el resto del día
bool IsGoodHour(int hour)
{
   if(!Use_Hour_Filter) return true;
   if(hour != 2 && hour != 15) return false;
   // FIX B: bloqueo intraday de H15 tras HC
...
int OnInit()
{
   ...
   Print("MM7 v17.89 | ", g_sym,
         " | H02(max2)+H15(max", Max_Positions_H15, ") | M2:H8+H13",
         ...
         " | HC_Cool=", HardCap_Cooldown_Bars, "b");
...
void OnTick()
{
   ...
   CheckDailyHalt();
   if(g_daily_halted){
      // Cerrar posiciones cuando se activa el halt (solo una vez)
   ...
   for(int pi = 0; pi < npos; pi++){
      ...
      if(Use_Hard_Loss_Cap && pnl < -Hard_Loss_Cap_USD){
         // Obtener hora de entrada para FIX B
         int entry_hour = 15;
         if(g_states[si].openTime > 0){
            MqlDateTime edt; TimeToStruct(g_states[si].openTime, edt);
            entry_hour = edt.hour;
         }
         ...
         // FIX A: registrar HC del día y ajustar MaxPositions efectivo
         g_hc_today++;
         if(Use_HC_Day_Tracker){
            if(g_hc_today == 1){
               g_max_pos_today = 1; // tras 1er HC → no abrir 2da posición
               Print("HC#1 hoy → MaxPos=1 para el resto del día");
            } else if(g_hc_today >= 2){
               g_pauseBarsLeft = MathMax(g_pauseBarsLeft, HC_Day_Pause_Bars);
               g_pendingDir    = 0;
               Print("HC#", g_hc_today, " hoy → pausa ", HC_Day_Pause_Bars, " barras");
            }
         }
         // FIX B: si HC fue en H15 → bloquear H15 el resto del día
         if(Use_H15_Block_After_HC && entry_hour == 15 && !g_h15_blocked_today){
            g_h15_blocked_today = true;
            Print("H15 bloqueada hoy: HC en H15 detectado");
         }
         Print("MM7 HC pnl=", DoubleToString(pnl, 2), " H", entry_hour);
         continue;
      }
   ...
   // ================================================================
   // FIX A: usar Max_Positions efectivo del día (se reduce tras 1er HC)
   // FIX 1: si la señal pendiente es de H15, aplicar límite especial H15
   {
      int cur_pos = CountByMagic();
      int max_pos = g_max_pos_today;
      // Si hay señal pendiente de H15 → máximo 1 posición abierta en H15
      // Esto elimina los HCs dobles simultáneos de raíz
      if(g_pendingDir != 0){
         MqlDateTime pdt; TimeToStruct(TimeCurrent(), pdt);
         if(pdt.hour == 15 && cur_pos >= Max_Positions_H15){
            g_pendingDir = 0; return;
         }
      }
      if(cur_pos >= max_pos){ g_pendingDir = 0; return; }
   }
   ...
   if(curBar != g_lastBarTime){
      ...
      if(closeNow >= upperZone && !TrendBlocksSell()){
         // FIX 1: verificar límite H15 ANTES de poner la señal
         // Si ya hay 1 posición abierta y la señal es de H15 → no generar señal
         if(dt.hour == 15 && CountByMagic() >= Max_Positions_H15) return;
         g_pendingDir    = -1;
