//+------------------------------------------------------------------+
//|                                           SKOLL_FVG_EA_FIXED.mq5 |
//|                           Estrategia FVG + OB + CHOCH (CORREGIDA)|
//|                             RR 1:2 con horario ajustado a VET    |
//+------------------------------------------------------------------+
...
int OnInit()
{
    Print("=== Inicializando SKOLL-FVG EA v3.0 CORREGIDO ===");
...
//+------------------------------------------------------------------+
//| Verificar horario válido de operación (CORREGIDO)              |
//+------------------------------------------------------------------+
bool IsValidTradingTime()
{
    ...
    int minute_utc = dt.min;
    
    //--- Determinar si estamos en horario de verano de EE.UU. (EDT)
    bool is_dst_usa = IsUSDaylightSaving(TimeCurrent());
    
    //--- Convertir hora de inicio/fin a UTC según horario
    int start_offset = is_dst_usa ? 4 : 5;  // EDT = UTC-4, EST = UTC-5
    int end_offset = is_dst_usa ? 4 : 5;
    
    int start_utc = InpStartHour + start_offset;
    int end_utc = InpEndHour + end_offset;
    
    if(start_utc >= 24) start_utc -= 24;
    if(end_utc >= 24) end_utc -= 24;
    
    //--- Verificar si la hora actual (UTC) está en la ventana
    int current_utc_minutes = hour_utc * 60 + minute_utc;
    int start_utc_minutes = start_utc * 60;
    int end_utc_minutes = end_utc * 60;
    
    if(end_utc < start_utc) {
        // Caso de cruce de medianoche UTC (ej: 22:00 a 04:00 UTC)
        return (current_utc_minutes >= start_utc_minutes || current_utc_minutes < end_utc_minutes);
    } else {
        return (current_utc_minutes >= start_utc_minutes && current_utc_minutes < end_utc_minutes);
    }
}
...
//+------------------------------------------------------------------+
//| Detectar si estamos en horario de verano de EE.UU. (EDT)       |
//+------------------------------------------------------------------+
bool IsUSDaylightSaving(datetime t)
{
    ...
    datetime dst_start = GetSecondSunday(year, 3);
    datetime dst_end = GetFirstSunday(year, 11);
    
    return (t >= dst_start && t < dst_end);
}

//+------------------------------------------------------------------+
//| Obtener segundo domingo de un mes                              |
//+------------------------------------------------------------------+
datetime GetSecondSunday(int year, int month)
{
    datetime first_day = StringToTime(IntegerToString(year) + "." + (month < 10 ? "0" : "") + IntegerToString(month) + ".01");
    MqlDateTime dt;
    TimeToStruct(first_day, dt);
    
    int days_to_add = (7 - dt.wday) % 7; // Días hasta el primer domingo
    if(days_to_add == 0) days_to_add = 7; // Si ya es domingo, sumar 7 para llegar al segundo
    
    return first_day + (days_to_add + 7) * 86400; // Segundo domingo
}

//+------------------------------------------------------------------+
//| Obtener primer domingo de un mes                               |
//+------------------------------------------------------------------+
datetime GetFirstSunday(int year, int month)
{
    ...
    int days_to_add = (7 - dt.wday) % 7; // Días hasta el primer domingo
    if(dt.wday == 0) days_to_add = 0; // Si ya es domingo
    
    return first_day + days_to_add * 86400;
}

//+------------------------------------------------------------------+
//| Filtro de noticias (implementado con eventos predefinidos)     |
//+------------------------------------------------------------------+
bool IsHighImpactNews()
{
    ...
    // Ejemplo de fechas clave (puedes expandir con calendario externo)
    // NFP: Primer viernes del mes a las 08:30 UTC
    // CPI: ~13:30 UTC, tercer jueves del mes
    // FOMC: Fechas específicas
    
    // Verificar si es viernes y es NFP (primero de la semana)
    if(dt.wday == 5) { // Viernes
        if(current_day <= 7) { // Primero de la semana
            if(current_hour == 8) return true; // 08:30 UTC = 04:30 VET (EDT) o 03:30 VET (EST)
        }
    }
    
    // Otros eventos clave (simplificados)
...
//+------------------------------------------------------------------+
//| Buscar señal de entrada (CORREGIDO)                            |
//+------------------------------------------------------------------+
void CheckForEntrySignal()
{
    ...
    //--- 3.1 VERIFICAR QUE EL OB SEA ANTERIOR AL FVG
    if(ob_m5.time >= fvg_h1.time) {
        Print("✗ OB posterior al FVG: rechazado");
        return;
    }
    ...
    //--- 5. Confirmar CHOCH en M3 (CORREGIDO)
...
bool DetectCHOCH(bool look_for_bullish)
{
    ...
    if(look_for_bullish) {
        // Buscar el último swing HIGH (romperlo es CHOCH alcista)
        ...
        // CHOCH alcista: rompe swing high y está por encima de EMA
        ...
        // Buscar el último swing LOW (romperlo es CHOCH bajista)
        ...
        // CHOCH bajista: rompe swing low y está por debajo de EMA
...
//+------------------------------------------------------------------+
//| Abrir posición (CORREGIDO para dos órdenes TP1/TP2)             |
//+------------------------------------------------------------------+
void OpenPosition(bool is_buy, const OB_Structure &ob)
{
    ...
    // Dividir volumen en dos partes iguales
    double vol1 = volume / 2.0;
    double vol2 = volume / 2.0;
    ...
    //--- Abrir orden TP1 (50% volumen)
    trade.SetExpertMagicNumber(InpMagicNumber);
    trade.SetDeviationInPoints(20);
    
    if(trade.PositionOpen(_Symbol, is_buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, vol1, price, sl, tp1, InpTradeComment + "_TP1")) {
        Print("✅ Orden TP1 abierta: #", trade.ResultRetcodeDescription());
    } else {
        Print("❌ Error orden TP1: ", trade.ResultRetcodeDescription());
        return;
    }
    
    //--- Abrir orden TP2 (50% volumen)
    if(trade.PositionOpen(_Symbol, is_buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, vol2, price, sl, tp2, InpTradeComment + "_TP2")) {
        Print("✅ Orden TP2 abierta: #", trade.ResultRetcodeDescription());
    } else {
        Print("❌ Error orden TP2: ", trade.ResultRetcodeDescription());
        return;
    }
    
    //--- Recuperar los tickets de las posiciones recién abiertas
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        if(PositionSelectByIndex(i)) {
            if(PositionGetInteger(POSITION_MAGIC) == InpMagicNumber) {
                string comment = PositionGetString(POSITION_COMMENT);
                if(StringFind(comment, "TP1") >= 0) {
                    g_Position.ticket_tp1 = PositionGetInteger(POSITION_TICKET);
                } else if(StringFind(comment, "TP2") >= 0) {
                    g_Position.ticket_tp2 = PositionGetInteger(POSITION_TICKET);
                }
            }
        }
    }
...
//+------------------------------------------------------------------+
//| Gestionar posición (CORREGIDO para manejo dual)                 |
//+------------------------------------------------------------------+
void ManagePosition()
{
    ...
    if(pos1_exists) {
        // Verificar si TP1 se alcanzó
        double current_price = PositionGetDouble(POSITION_PRICE_CURRENT);
        bool is_buy = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
        bool hit_tp1 = (is_buy && current_price >= g_Position.tp1) || (!is_buy && current_price <= g_Position.tp1);
        
        if(hit_tp1 && !g_Position.tp1_hit) {
            g_Position.tp1_hit = true;
            Print("🎯 TP1 alcanzado. Moviendo SL de TP2 a BE.");
            
            // Modificar SL de TP2 a break-even (entry price)
            if(pos2_exists) {
                trade.PositionModify(g_Position.ticket_tp2, g_Position.entry_price, g_Position.tp2);
            }
        }
    }
    
    if(pos2_exists) {
        // Verificar si TP2 se alcanzó
        double current_price = PositionGetDouble(POSITION_PRICE_CURRENT);
        bool is_buy = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
        bool hit_tp2 = (is_buy && current_price >= g_Position.tp2) || (!is_buy && current_price <= g_Position.tp2);
        
        if(hit_tp2) {
            Print("🎯 TP2 alcanzado. Cerrando posición.");
            trade.PositionClose(g_Position.ticket_tp2);
            ResetPosition(g_Position);
        }
    }
    
    // Si ambas posiciones se cierran, resetear
    if(!pos1_exists && !pos2_exists) {
        ResetPosition(g_Position);
    }
}
...
{
    string text = "═══ SKOLL-FVG v3.0 CORREGIDO ═══\n";
