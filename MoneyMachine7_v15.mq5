//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v15.0 — PREDICTIVE INTELLIGENCE ENGINE                        |
//|                                                                  |
//|  PROBLEMA IDENTIFICADO EN LIVE v14:                             |
//|  El Stochastic es REACTIVO, no PREDICTIVO. Genera señales       |
//|  cuando el movimiento ya ocurrió → entradas tardías → SL.       |
//|                                                                  |
//|  SOLUCIÓN v15 — MOTOR DE PREDICCIÓN MULTI-CAPA:                |
//|                                                                  |
//|  CAPA 1: Confluencia Multi-Timeframe (M1+M5+M15)               |
//|    La dirección debe coincidir en ≥2 timeframes antes de entrar |
//|    Evita entrar contra la tendencia del timeframe superior      |
//|                                                                  |
//|  CAPA 2: Order Flow Bias (Delta de Volume)                      |
//|    Detecta presión compradora/vendedora dominante               |
//|    Volumen de ticks alcistas vs bajistas en ventana reciente     |
//|                                                                  |
//|  CAPA 3: Momentum Predictivo (EMA Cross acelerado)             |
//|    EMA3 vs EMA8 cruce EN FORMACIÓN detecta giro temprano       |
//|    No espera confirmación de barra cerrada → anticipa           |
//|                                                                  |
//|  CAPA 4: Estructura de Precio (HH/HL vs LH/LL)                |
//|    Detecta si el mercado está haciendo Higher Highs/Lows        |
//|    o Lower Highs/Lows → sesgo de dirección estructural         |
//|                                                                  |
//|  CAPA 5: Smart Stochastic (solo como confirmador final)        |
//|    El Stoch ya NO es la señal principal, es un filtro de timing |
//|    Confirma que no se entra en la cima/valle del impulso        |
//|                                                                  |
//|  RESULTADO ESPERADO:                                            |
//|  - Menos operaciones (calidad > cantidad)                       |
//|  - Win Rate > 60% en tiempo real (vs ~40% live en v14)         |
//|  - Entradas en dirección del flujo real del mercado            |
//+------------------------------------------------------------------+
...
//--------------------------------------------------------------------
// CAPA 3: Momentum Predictivo — EMA3 vs EMA8 cruce EN FORMACIÓN
// Detecta el cruce mientras la barra aún está formándose (predictivo)
// A diferencia del Stoch, este actúa ANTES del movimiento completo
//--------------------------------------------------------------------
int GetEMAMomentum(int dir)
{
   ...
   double ef_prev[]; ArraySetAsSeries(ef_prev, true);
   double es_prev[]; ArraySetAsSeries(es_prev, true);

   if(CopyBuffer(g_hEMA_Fast,0,0,2,ef_now)<2) return 1;
   if(CopyBuffer(g_hEMA_Slow,0,0,2,es_now)<2) return 1;
   if(CopyBuffer(g_hEMA_Fast,0,1,2,ef_prev)<2) return 1;
   if(CopyBuffer(g_hEMA_Slow,0,1,2,es_prev)<2) return 1;

   double gap_now  = ef_now[0]  - es_now[0];   // gap actual (barra formándose)
   double gap_prev = ef_prev[0] - es_prev[0];  // gap barra anterior cerrada

   // Cruce alcista EN FORMACIÓN: gap negativo → positivo
   if(dir ==  1 && gap_prev < 0 && gap_now > 0) return 1;
   // Cruce bajista EN FORMACIÓN: gap positivo → negativo
   if(dir == -1 && gap_prev > 0 && gap_now < 0) return 1;

   // Sin cruce pero momentum alineado (ya cruzado y avanzando)
   if(dir ==  1 && gap_now > 0 && gap_now > gap_prev) return 1;
   if(dir == -1 && gap_now < 0 && gap_now < gap_prev) return 1;
...
int GetSignal()
{
   ...
   else // Hybrid (modo 1): FVG y Sweep como fallback
   {
      ...
      if(sig==0 && Enable_FVG_Strategy)
      {
         ...
         int fvg_score_buy  = GetOrderFlowBias(1)  + GetEMAMomentum(1);
         int fvg_score_sell = GetOrderFlowBias(-1) + GetEMAMomentum(-1);
      ...
      if(sig==0)
      {
         ...
         {
            int s_score = GetOrderFlowBias(1) + GetEMAMomentum(1) + GetMTFConfluence(1);
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      // ── Trailing Profit v14 ──────────────────────────────────────
      // Se ejecuta ANTES del TP/SL fijo para priorizar el trailing
      if(Enable_Trailing_Profit || Enable_MinUSD_TP)
      {
         ...
         if(Enable_MinUSD_TP && curProfit >= TP_Min_USD)
            cls = true;

         // MODO 2: Trailing Profit
         if(!cls && Enable_Trailing_Profit)
         {
            ...
            // Activar trailing cuando se alcanza el umbral
            if(!g_sp[i].trailActivated && curProfit >= TP_Trail_Activate_USD)
               g_sp[i].trailActivated = true;

            // Si trailing activo: cerrar cuando profit retrocede desde el máximo
            if(g_sp[i].trailActivated)
            {
               double retrace = g_sp[i].trailMaxProfit - curProfit;
               if(retrace >= TP_Trail_Retrace_USD)
                  cls = true;
            }
         }
      }

      // ── TP/SL Virtual original (precio) ─────────────────────────
      if(!cls)
      {
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v15.0 ]",clrGold,       0*lh,x,y,co,fn);
   DashLbl("1","Mode : "+ms+(hv?" [HiVol→Sweep]":"")+" Trail:"+(Enable_Trailing_Profit?"ON":"off"),clrCyan,1*lh,x,y,co,fn);
   ...
   string sig_type_str=(g_lastSigType==1)?"Sw":(g_lastSigType==2)?"FV":(g_lastSigType==3)?"P3":(g_lastSigType==4)?"PRD":"--";
   ...
   // v15: Predictive Engine Score display
   ...
   // MTF + OrderFlow + EMAMom
   int mtf_b=GetMTFConfluence(1); int mtf_s=GetMTFConfluence(-1);
   int ofl_b=GetOrderFlowBias(1); int ofl_s=GetOrderFlowBias(-1);
   int ema_b=GetEMAMomentum(1);   int ema_s=GetEMAMomentum(-1);
   int str_b=GetPriceStructure(1);int str_s=GetPriceStructure(-1);
   string layers_b="M"+IntegerToString(mtf_b)+"O"+IntegerToString(ofl_b)+"E"+IntegerToString(ema_b)+"S"+IntegerToString(str_b);
   string layers_s="M"+IntegerToString(mtf_s)+"O"+IntegerToString(ofl_s)+"E"+IntegerToString(ema_s)+"S"+IntegerToString(str_s);
   DashLbl("prd","PRED:"+pred_dir_str+" ["+pred_score_str+"] min="+IntegerToString(Min_Prediction_Score),pred_col,8*lh,x,y,co,fn);
   DashLbl("lyrb","B:"+layers_b+" S:"+layers_s,clrSilver,9*lh,x,y,co,fn);
   string cool_str="";
   ...
   DashLbl("8","Cool:"+cool_str+" Halt:"+(g_haltedToday?"Y":"n"),(g_haltedToday||cool_str!="OK")?clrOrangeRed:clrGray,10*lh,x,y,co,fn);
}
