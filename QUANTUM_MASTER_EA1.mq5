...
//|           Sistema MASTER - v4.25 BALANCED                        |
//|           Consistente en Todas las Condiciones                   |
//+------------------------------------------------------------------+
#property copyright "Quantum Master EA v4.25 - Balanced Consistent"
...
input int    InpMaxTradesPerDay = 5;         // Max trades/día (BALANCEADO)
input int    InpMaxConsecutiveLosses = 4;    // Pausar después de N pérdidas (MÁS ESTRICTO)
input double InpMaxDailyLossPercent = 3.0;   // Pérdida máxima diaria % (MÁS ESTRICTO)
input double InpMaxDrawdownPercent = 18.0;   // Drawdown máximo % (MÁS ESTRICTO)

input group "═══ INDICADORES BASE ═══"
input int    InpBollingerPeriod = 20;        // Bollinger período
input double InpBollingerDeviation = 2.0;    // Bollinger desviación (NORMAL)
input int    InpRSIPeriod = 14;              // RSI período
input int    InpRSIOversold = 32;            // RSI sobreventa (MÁS EXTREMO)
input int    InpRSIOverbought = 68;          // RSI sobrecompra (MÁS EXTREMO)
...
input int    InpMinPatternScore = 65;        // Score mínimo del patrón (MÁS ESTRICTO)
input bool   InpRequirePatternConfirmation = false; // NO requiere confirmación
...
input double InpMinPatternWinRate = 45.0;    // Win rate mínimo del patrón (MÁS ESTRICTO)
input bool   InpUseIntelligentScoring = true; // Scoring inteligente
input int    InpMinIntelligentScore = 70;    // Score inteligente mínimo (MÁS ESTRICTO)
...
input int    InpMaxHoursInTrade = 4;           // Máx horas sin alcanzar 50% TP (MÁS ESTRICTO)
input double InpMinADX = 12.0;                 // ADX mínimo para operar (MÁS ESTRICTO)
input double InpMinVolatility = 0.5;           // Volatilidad mínima (MÁS ESTRICTO)
input double InpMaxVolatility = 3.0;           // Volatilidad máxima (MÁS ESTRICTO)
input double InpMinOrderFlowBuy = 1.1;         // Imbalance mínimo para compra (MÁS ESTRICTO)
input double InpMaxOrderFlowSell = 0.9;        // Imbalance máximo para venta (MÁS ESTRICTO)

input group "═══ MEJORAS AVANZADAS ═══"
input bool   InpUseConfluenceScoring = true;   // Scoring por confluencia
input int    InpMinConfluenceFactors = 3;      // Factores mínimos (de 10) (MÁS ESTRICTO)
input bool   InpUseDynamicSL = true;           // Stop Loss dinámico
input double InpSLMultiplier = 1.8;            // Multiplicador SL (ATR) (MÁS PROTECCIÓN)
input bool   InpUsePartialProfits = true;      // Toma parcial de ganancias
input bool   InpUseCorrelationFilter = true;   // Filtro de correlación (ACTIVADO)
input int    InpMinMinutesBetweenTrades = 15;  // Minutos entre trades similares (MÁS ESTRICTO)

input group "═══ MEJORAS ÉLITE (MASTER) ═══"
input bool   InpUseTrendStrengthFilter = false; // Filtro de fuerza de tendencia
input double InpMinTrendStrength = 0.6;        // Fuerza mínima
input bool   InpUseNewsFilter = false;         // Evitar operar cerca de noticias
input int    InpNewsAvoidMinutes = 30;         // Minutos antes/después de noticias
input bool   InpUseSessionFilter = false;      // Filtro de sesión óptima
input bool   InpTradeOnlyBestHours = false;    // Operar solo mejores horas
input bool   InpUseDrawdownProtection = true;  // Protección adicional de drawdown
input double InpMaxDrawdownForDay = 2.5;       // Drawdown máximo diario %
...
int OnInit()
{
   ...
   Print("╔═══════════════════════════════════════════════════════════╗");
   Print("║  QUANTUM MASTER EA v4.25 BALANCED                        ║");
   Print("╠═══════════════════════════════════════════════════════════╣");
   ...
   Print("╠═══════════════════════════════════════════════════════════╣");
   Print("║  ⚡ FILTROS BALANCEADOS (CALIDAD):                        ║");
   ...
   Print("╠═══════════════════════════════════════════════════════════╣");
   Print("║  🚀 OBJETIVO: CONSISTENCIA EN TODO EL PERÍODO:           ║");
...
void UpdateDailyControls()
{
   ...
   string yearStr = IntegerToString(timeStruct.year);
   string monStr = IntegerToString(timeStruct.mon);
   string dayStr = IntegerToString(timeStruct.day);
   datetime today = StringToTime(yearStr + "." + monStr + "." + dayStr);
...
void UpdateVWAP()
{
   ...
   string monStr = IntegerToString(timeStruct.mon);
   string dayStr = IntegerToString(timeStruct.day);
   datetime todayStart = StringToTime(yearStr + "." + monStr + "." + dayStr);
