//+------------------------------------------------------------------+
//|                           XAUUSD_V6_3_PROFIT_OPTIMIZED.mq5       |
//|              V6.3: OPTIMIZADO PARA MÁXIMA RENTABILIDAD           |
//|   Combina lo mejor de Config A/B/C + Mejoras adicionales         |
//+------------------------------------------------------------------+
#property copyright "V6.3 - Profit Optimized"
...
input double SL_ATR_Multiplier = 1.3;     // Balance entre A y B
input double TP_ATR_Multiplier = 4.5;     // TP realista
input int ATR_Period = 14;

// Break-Even AGRESIVO
input bool Use_Break_Even = true;
input double BreakEven_Start_ATR = 1.6;   // Más rápido
input double BreakEven_Profit_ATR = 0.4;  // Ganancia pequeña

// Trailing Stop OPTIMIZADO
input bool Use_Trailing_Stop = true;
input double Trailing_Start_ATR = 2.6;
input double Trailing_Step_ATR = 0.9;

// Filtro de Volatilidad (NUEVO)
input bool Use_ATR_Filter = true;
input double Min_ATR_Points = 15.0;       // Solo opera con volatilidad suficiente
...
{
    Print("========================================");
    Print("XAUUSD V6.3 - PROFIT OPTIMIZED");
    Print("========================================");
    Print("RR: 1:3.46 (SL 1.3×ATR, TP 4.5×ATR)");
    Print("Break-Even: AGRESIVO (1.6×ATR)");
    Print("Trailing: OPTIMIZADO (2.6×ATR)");
    Print("Filtro ATR: ", Use_ATR_Filter ? "ACTIVO" : "DESACTIVADO");
    Print("Objetivo: PF 2.5+, Net $4,000+");
    Print("========================================");
...
void CheckForEntry()
{
    ...
    if(Use_ATR_Filter)
    {
        ...
        {
            return; // No operar con baja volatilidad
        }
    }
    ...
    // SEÑAL LONG: Precio cruza por encima de EMA Fast y EMAs alineadas alcista
    bool long_signal = false;
    if(close[1] < ema_fast[1] && close[0] > ema_fast[0])
    {
        if(ema_fast[0] > ema_medium[0] && ema_medium[0] > ema_slow[0])
        {
            // Confirmación adicional: momentum fuerte
            double ema_separation = (ema_fast[0] - ema_slow[0]) / ema_slow[0] * 100;
            if(ema_separation > 0.05) // EMAs separadas al menos 0.05%
            {
                long_signal = true;
            }
        }
    ...
    // SEÑAL SHORT: Precio cruza por debajo de EMA Fast y EMAs alineadas bajista
    ...
    if(close[1] > ema_fast[1] && close[0] < ema_fast[0])
    {
        ...
        {
            // Confirmación adicional: momentum fuerte
            double ema_separation = (ema_slow[0] - ema_fast[0]) / ema_slow[0] * 100;
    ...
    if(long_signal)
    {
        Print("📈 SEÑAL LONG - V6.3 Optimized");
        ExecuteTrade(ORDER_TYPE_BUY, current_atr);
        g_tradesToday++;
    }
    else if(short_signal)
    {
        Print("📉 SEÑAL SHORT - V6.3 Optimized");
...
void ExecuteTrade(ENUM_ORDER_TYPE orderType, double atr_value)
{
    ...
    {
        g_tradesExecuted++;
        Print("✅ TRADE #", g_tradesExecuted, " EJECUTADO");
    }
    else
    {
...
void OnDeinit(const int reason)
{
    ...
    Print("========================================");
    Print("V6.3 PROFIT OPTIMIZED FINALIZADO");
