...
input bool   Enable_RegimeFilter         = true;   // detector régimen: TREND vs RANGE
...
{
   // 1. Market Regime: no operar contra trend fuerte (resuelve 09h, 17-19h)
   if(!RegimeFilter(sig)) return false;
   // 2. K velocity mínima (0.0 = desactivado)
   if(Min_K_Velocity > 0)
   {
      double velocity = (sig==1) ? (k_now-k_prev) : (k_prev-k_now);
      if(velocity < Min_K_Velocity) return false;
   }
   // 3. Vela anterior confirma dirección (body ratio adaptativo)
   if(!PrevCandleConfirm(sig)) return false;
   // 4. RSI Trend Gate (45/55 → más permisivo, recupera tarde)
   if(!RSITrendGate(sig)) return false;
   // 5. Anti-impulso (OFF por defecto)
   if(!AntiImpulseOK(sig)) return false;
   // 6. Cooling-off direccional
   if(IsCoolingOff(sig)) return false;
   // 7. Fast-loss pause (volatilidad extrema)
