//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v10.0 — FORENSIC REPLICA: CROSSING + COOLDOWN TIMER           |
//|                                                                  |
//|  DIAGNÓSTICO v8.34 (PROBLEMA RAÍZ):                            |
//|  ✗ WR=23%, dur avg=11s, 841 trades vs 264 objetivo             |
//|  ✗ 64% trades duran <10s → entra en SL instantáneo             |
//|  ✗ K crossing filter fallido: k[0] ya está MUY dentro de zona  |
//|    cuando señal dispara → entra en medio del movimiento         |
//|  ✗ Throttle 3s insuficiente → múltiples entradas/zona Stoch    |
//|                                                                  |
//|  EVIDENCIA ORIGINAL (245 Legacy trades):                        |
//|  ✓ Avg dur = 43.2s, Median = 23.5s                             |
//|  ✓ Dur <10s = 18% (vs 64% en v8.34)                           |
//|  ✓ WR = 58.4% (vs 23% en v8.34)                               |
//|  ✓ TP move = 2.51 pts (ATR×5.0 ✓)                             |
//|  ✓ SL move = 0.76 pts (ATR×1.5 ✓)                             |
//|  ✓ ~16 trades/hora = 1 cada ~3.6 min                           |
//|                                                                  |
//|  FIXES v10:                                                     |
//|  1. STOCH CROSS + COOLDOWN: tras entrada, esperar mínimo        |
//|     G2_Min_Seconds (30s) antes de nueva señal del mismo ciclo  |
//|  2. K CROSSING ESTRICTO: k_prev DEBE estar fuera de la zona    |
//|     (k_prev > 30 para BUY, k_prev < 70 para SELL)              |
//|  3. POST-CLOSE LOCKOUT: tras cierre (TP o SL), lockout de 15s  |
//|     para evitar re-entrada inmediata en zona de baja calidad    |
//|  4. STOCH DEPTH: solo señal si K está suficientemente dentro    |
//|     de la zona (≤25 para BUY, ≥75 para SELL) = alta convicción |
//|  5. Todos los filtros extra siguen OFF por default              |
//|                                                                  |
//|  HISTORIAL:                                                     |
//|  v8.26→8.34: throttle 3s, crossing → WR=23%, dur=11s ✗        |
//|  v10.0:      crossing estricto + cooldown timer → target ~50%  |
//+------------------------------------------------------------------+
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v10.0 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v10.0 | CROSSING ESTRICTO + COOLDOWN | Mode=",ms,
         ...
         " | Depth=",Enable_Stoch_Depth_Filter," (",Stoch_Depth_Buy,"/",Stoch_Depth_Sell,")",
