//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.28 — TREND FILTER CORRECTO                 |
//|                                                                  |
//|  APRENDIDO DE v8.27:                                            |
//|  ✗ WR=28.49%, 60% SHORTS en día de fuerte uptrend               |
//|  → GetTrend() usaba MA200 en M1 = 3.3h lookback = DEMASIADO     |
//|    CORTO → price oscila arriba/abajo del MA → trend=-1 60%      |
//|                                                                  |
//|  EVIDENCIA DEL LOG ORIGINAL:                                    |
//|  ✓ 92.8% COMPRAS en March 2 (día uptrend oro +2.5%)            |
//|  ✓ Trend filter muy estable → casi siempre trend=+1              |
//|  ✓ Esto exige MA con LARGO lookback para capturar trend real     |
//|                                                                  |
//|  HISTORIAL DE VERSIONES:                                        |
//|  v8.24: Candle+LocalPA+AdaptPC → WR=28.76% (filtros nocivos)   |
//|  v8.25: ZoneFreshEntry → WR=27% (bloqueó señales buenas)        |
//|  v8.26: Sin filtros, throttle=3s → 802/día, WR=27%             |
//|  v8.27: Bar-lock → 292/día, WR=28% (trend MA broken)           |
//|  v8.28: MA trend en H1(200) → lookback 8.3 días → ESTABLE ✓   |
//|                                                                  |
//|  FIX v8.28:                                                     |
//|  iMA(sym, PERIOD_H1, 200) en lugar de iMA(sym, _Period, 200)   |
//|  H1 MA(200) = 200 horas = ~8 días lookback                      |
//|  Gold en uptrend multi-semanas: price >> H1 MA200 → trend=+1   |
//|  → 90%+ BUYS en uptrend day → WR objetivo ~50%                 |
//+------------------------------------------------------------------+
...
datetime g_lastSignalBar=0;   // throttle: TimeCurrent() cuando última señal legacy abrió
bool     g_inBuyZone    =false; // ZONE LOCK: K ya cruzó <30, esperar salida >35
bool     g_inSellZone   =false; // ZONE LOCK: K ya cruzó >70, esperar salida <65
...
int GetSignal()
{
   ...
   else // Hybrid (modo 1)
   {
      ...
      // P3: STOCH + TREND + ZONE LOCK ──────────────────────────────────────────
      // FORENSE ORIGINAL (264 trades):
      //   21.3% aperturas en mismo minuto → usa kb[0] (tiempo real) ✓
      //   13-17 señales/hora → 1 entrada por CICLO completo de Stoch
      //   K oscila 29→31→29→31 = múltiples cruces en segundos → problema v8.33
      //
      // ZONE LOCK — solución definitiva:
      //   Cuando K entra en <30 → g_inBuyZone=true (señal disparada)
      //   Bloquea nuevas señales BUY hasta que K suba de vuelta a >35 (exit hysteresis)
      //   Cuando K vuelve a cruzar <30 desde >35 → nueva señal permitida
      //   Hysteresis 5pts (30→35) evita re-entradas por rebote K=29→31→29
      //   Resultado: 1 señal por ciclo Stoch completo = ~130/día como original
      if(sig==0)
      {
         double buyExit  = Stoch_Buy_Level  + 5.0;  // 35: K debe salir de zona antes de nueva entrada
         double sellExit = Stoch_Sell_Level - 5.0;  // 65: zona sell exit

         // Actualizar estado de zona (siempre, independiente de posición abierta)
         if(g_inBuyZone  && k_now > buyExit)  g_inBuyZone  = false; // K salió zona buy
         if(g_inSellZone && k_now < sellExit) g_inSellZone = false; // K salió zona sell

         // Señal BUY: K entra en zona <30 Y no estábamos ya en zona
         bool crossBuy  = (k_now <= Stoch_Buy_Level  && !g_inBuyZone);
         bool crossSell = (k_now >= Stoch_Sell_Level && !g_inSellZone);

         if(crossBuy  && IntraFiltersPass(1, k_now,k_prev) && (trend==0||trend== 1))
         {
            sig=1; g_lastSigType=3;
            g_inBuyZone=true;   // marcar zona activa — bloquea hasta K>35
         }
         if(crossSell && IntraFiltersPass(-1,k_now,k_prev) && (trend==0||trend==-1))
         {
            sig=-1; g_lastSigType=3;
            g_inSellZone=true;  // marcar zona activa — bloquea hasta K<65
         }
      }
   }
...
void DailyReset()
{
   ...
   g_inBuyZone=false;g_inSellZone=false;
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.34 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.28 | BAR-LOCK + H1-TREND | Mode=",ms,
...
void OnTick()
{
   ...
   {
      // THROTTLE 3s: evita duplicados en el mismo tick
      // kb[0] = K tiempo real → señal cambia con cada tick → throttle controla frecuencia
      // Después de TP: precio sube → K sube de <30 a >30 → señal cesa naturalmente
      // Después de SL: precio baja poco → K sigue <30 → re-abre en <5s (como original 23.6%)
      datetime now = TimeCurrent();
      if(now - g_lastSignalBar < 3){DrawDashboard();return;}

      int sig=GetSignal();
      if(sig== 1&&CountBuys() >=Max_Buy)  sig=0;
      if(sig==-1&&CountSells()>=Max_Sell) sig=0;
      if(sig==1)  {ulong tk=OpenOrder(ORDER_TYPE_BUY, CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastSignalBar=now;g_lastLegacyTicket=tk;}}
      if(sig==-1) {ulong tk=OpenOrder(ORDER_TYPE_SELL,CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastSignalBar=now;g_lastLegacyTicket=tk;}}
   }
   DrawDashboard();
}
