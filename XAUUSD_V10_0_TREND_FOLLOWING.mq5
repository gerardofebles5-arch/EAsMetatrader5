//+------------------------------------------------------------------+
//|                     XAUUSD_V10_0_TREND_FOLLOWING.mq5             |
//|          V10.0: TREND FOLLOWING - Opera A FAVOR de tendencia     |
//+------------------------------------------------------------------+
#property copyright "V10.0 - Trend Following"
#property version   "10.00"

input int Magic_Number = 999999;
input double Risk_Percent = 0.5;
input int Max_Trades_Per_Day = 6;
input double Max_Daily_Loss_Percent = 2.0;  // NUEVO: Stop diario
...
input double Partial_Close_Percent = 50.0;  // Cerrar 50% en 1.5 ATR
input double Partial_Close_ATR = 1.5;

input bool Use_Break_Even = true;
input double BreakEven_Start_ATR = 0.8;
input double BreakEven_Profit_ATR = 0.2;
...
double g_dailyProfit = 0.0;  // NUEVO: Track daily profit
bool g_dailyLossReached = false;  // NUEVO: Flag daily loss
...
{
    Print("========================================");
    Print("V10.0 - EXTREME LOSS REDUCTION + PROFESSIONAL");
    Print("========================================");
    ...
    Print("Max Daily Loss: ", Max_Daily_Loss_Percent, "% | Max Trades/Day: ", Max_Trades_Per_Day);
    Print("========================================");
...
void OnTick()
{
    ...
    if(currentBarTime != g_lastBarTime)
    {
        ...
        {
            Print("Trades today: ", g_tradesToday, "/", Max_Trades_Per_Day, " | Checking for entry...");
            CheckForEntry();
        }
        else if(g_tradesToday >= Max_Trades_Per_Day)
        {
            Print("Max trades per day reached: ", g_tradesToday, "/", Max_Trades_Per_Day);
        }
    ...
    {
        if(Use_Partial_Close) ManagePartialClose();
...
void CheckForEntry()
{
    ...
    {
        Print("ERROR: Failed to copy indicator/price data");
        return;
    }
    ...
    Print("Price: ", close[0], " | EMA21: ", ema_fast[0], " | EMA50: ", ema_trend[0], " | EMA200: ", ema_filter[0]);
    Print("ATR: ", current_atr);
    
    // Tendencia alcista: Precio y EMA50 por encima de EMA200
    bool uptrend = (close[0] > ema_filter[0] && ema_trend[0] > ema_filter[0]);
    
    // Tendencia bajista: Precio y EMA50 por debajo de EMA200
    bool downtrend = (close[0] < ema_filter[0] && ema_trend[0] < ema_filter[0]);
    
    Print("Uptrend: ", uptrend ? "YES" : "NO", " | Downtrend: ", downtrend ? "YES" : "NO");
    
    // LONG: Tendencia alcista + Pullback a EMA21
    if(uptrend)
    {
        ...
        Print("LONG Check - Pullback: ", pullback ? "YES" : "NO", 
              " | Bounce: ", bounce ? "YES" : "NO", 
              " | Trend Intact: ", trend_intact ? "YES" : "NO");
        ...
        {
            Print("LONG conditions not met");
        }
    }
    
    // SHORT: Tendencia bajista + Pullback a EMA21
    if(downtrend)
    {
        ...
        Print("SHORT Check - Pullback: ", pullback ? "YES" : "NO", 
              " | Bounce: ", bounce ? "YES" : "NO", 
              " | Trend Intact: ", trend_intact ? "YES" : "NO");
        ...
        {
            Print("SHORT conditions not met");
        }
    }
    
    if(!uptrend && !downtrend)
    {
        Print("NO TREND - Waiting for clear trend...");
    }
}
...
void ExecuteTrade(ENUM_ORDER_TYPE orderType, double atr_value)
{
    ...
    Print("Price: ", price, " | SL: ", sl, " (", sl_distance, ") | TP: ", tp, " (", tp_distance, ")");
    ...
    request.comment = "V10.0";
    request.type_filling = ORDER_FILLING_IOC;
    
    bool sent = OrderSend(request, result);
    
    Print("Order sent: ", sent ? "YES" : "NO");
    ...
    if(sent && result.retcode == TRADE_RETCODE_DONE)
    {
        g_tradesExecuted++;
        Print("========================================");
        Print("TRADE #", g_tradesExecuted, " EXECUTED SUCCESSFULLY");
...
void ManageTrailingStop()
{
    ...
    if(should_modify)
    {
        ...
        {
            Print("Trailing Stop modify failed: ", result.retcode);
        }
    }
}
...
void ManageBreakEven()
{
    ...
    if(should_modify)
    {
        ...
        if(!OrderSend(request, result))
        {
            Print("Break Even modify failed: ", result.retcode);
        }
    }
}
...
void ManagePartialClose()
{
    ...
    double minLot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
    if(posVolume <= minLot * 1.5) return; // Already closed or too small
    ...
    {
        Print("Partial Close: ", closeVolume, " lots at ", Partial_Close_ATR, " ATR profit");
    }
}
...
void OnDeinit(const int reason)
{
    ...
    Print("V10.0 FINALIZADO - Trades: ", g_tradesExecuted);
}
//+------------------------------------------------------------------+
