...
{
   // 1. EMA50 como BIAS (no blocker): si contra EMA exigimos mayor velocity
   int ema_gate = GetEMA20Gate();
   double vel_required = Min_K_Velocity;
   if(ema_gate != 0 && sig != ema_gate)
      vel_required = Min_K_Velocity * EMA_Against_KVel_Factor; // contra EMA: 5x más exigente

   // 2. K velocity adaptativa
   if(!KVelocityOK(k_now, k_prev, sig))
   {
      // si no pasa velocidad base → rechazar siempre
      // si pasa velocidad base pero no la ampliada → rechazar solo si contra EMA
      double velocity = (sig==1) ? (k_now-k_prev) : (k_prev-k_now);
      if(velocity < vel_required) return false;
   }

   // 3. D-confirm: solo para P3 (controlado externamente via parámetro)
   // Para Sweep/FVG, DConfirm_P3Only=true lo saltamos → ver abajo en P3
