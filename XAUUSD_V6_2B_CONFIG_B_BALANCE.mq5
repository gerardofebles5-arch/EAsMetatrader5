//+------------------------------------------------------------------+
//|                        XAUUSD_V6_2B_CONFIG_B_BALANCE.mq5         |
//|                    CONFIGURACIÓN B: RR 1:2.67 BALANCE            |
//|    SL 1.5×ATR, TP 4.0×ATR - Objetivo: PF 2.0+                   |
//+------------------------------------------------------------------+
#property copyright "V6.2B Config B - Balance"
...
// Risk Management - CONFIGURACIÓN B
input double SL_ATR_Multiplier = 1.5;     // BALANCE
input double TP_ATR_Multiplier = 4.0;     // RR 1:2.67
input int ATR_Period = 14;

// Break-Even - CONFIGURACIÓN B
input bool Use_Break_Even = true;
input double BreakEven_Start_ATR = 1.8;
input double BreakEven_Profit_ATR = 0.4;

// Trailing Stop - CONFIGURACIÓN B
input bool Use_Trailing_Stop = true;
input double Trailing_Start_ATR = 2.8;
...
{
    Print("========================================");
    Print("XAUUSD V6.2B - CONFIGURACIÓN B BALANCE");
    Print("========================================");
    Print("RR: 1:2.67 (SL 1.5×ATR, TP 4.0×ATR)");
    Print("Break-Even: 1.8×ATR → +0.4×ATR");
    Print("Trailing: Start 2.8×ATR, Step 1.0×ATR");
    Print("Objetivo: PF 2.0+");
    Print("========================================");
...
void CheckForEntry()
{
    ...
    if(long_signal)
    {
        Print("📈 SEÑAL LONG - Config B");
        ExecuteTrade(ORDER_TYPE_BUY, current_atr);
        g_tradesToday++;
    }
    else if(short_signal)
    {
        Print("📉 SEÑAL SHORT - Config B");
...
void ExecuteTrade(ENUM_ORDER_TYPE orderType, double atr_value)
{
    ...
    {
        g_tradesExecuted++;
        Print("✅ TRADE #", g_tradesExecuted, " | RR 1:2.67");
    }
}
