...
//|  por lotaje para mantener equivalencia en PUNTOS, no en USD.   |
//|  Backtest lot=0.05 → $0.50 = 1.0 pto. Live lot=0.01 → $0.10  |
//|  = 1.0 pto. Mismos PUNTOS, diferente USD.                      |
//|  En backtest: usa umbrales originales (sin escalar).           |
//|                                                                  |
...
// g_lastOpenAttemptTime se actualiza en OpenOrder() independientemente del cierre de la posición
// Evita re-entrada inmediata tras trades de 0s que resetean CountLegacyOpen al instante
...
int GetPredictiveSignal()
{
   ...
   score_buy  += GetStochTiming(1);

   score_sell += GetMTFConfluence(-1);
   score_sell += GetOrderFlowBias(-1);
   score_sell += GetMomentumBias(-1);
   score_sell += GetPriceStructure(-1);
   score_sell += GetStochTiming(-1);

   // El Stochastic Timing actúa como VETO si retorna 0
   // (evita entrar absolutamente contra el momentum extremo)
   bool stoch_veto_buy  = (GetStochTiming(1)  == 0);
   bool stoch_veto_sell = (GetStochTiming(-1) == 0);
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      // BACKTEST: usa umbrales directos — identico a v16.6 (Trail_Lot_Ref/lot=1.0)
      // LIVE:     escala para mantener mismos PUNTOS de activación/retroceso
      //           Con lot=0.05 (ref): $0.50 activate = 1.0 pto (x1 escalar)
      //           Con lot=0.01 (live): sin escalar = $0.50 = 0.5 pto → demasiado sensible
      //                                con escalar = $0.50*(0.05/0.01) = $2.50 = mismos 0.5 pto
      //           Fórmula: activateUSD_scaled = TP_Trail_Activate_USD * (Trail_Lot_Ref / posLot)
      //           Cap: no escalar más de 5x para evitar umbrales irreales con lots muy pequeños
      if(Enable_Trailing_Profit || Enable_MinUSD_TP)
      {
         double lotScale = 1.0;
         if(isLive && posLot > 0 && Trail_Lot_Ref > 0)
            lotScale = MathMax(0.5, MathMin(Trail_Lot_Ref / posLot, 5.0));
      ...
      if(!cls && Enable_TrailingStop)
      {
         ...
         if(posType == POSITION_TYPE_BUY)
         {
            ...
            if(!slGuardActive && g_sp[i].vSL > 0 && bid <= g_sp[i].vSL)
               cls = true;
         }
         else // SELL
         {
            ...
            if(!slGuardActive && g_sp[i].vSL > 0 && ask >= g_sp[i].vSL)
               cls = true;
         }
      }
      ...
      if(!cls)
      {
         if(g_sp[i].dir== 1 && g_sp[i].vTP > 0 && bid >= g_sp[i].vTP) cls = true;
         if(g_sp[i].dir==-1 && g_sp[i].vTP > 0 && ask <= g_sp[i].vTP) cls = true;
         if(!cls)
         {
            // SL: respeta SL_Guard en live
            if(!slGuardActive)
            {
               if(g_sp[i].dir== 1 && g_sp[i].vSL > 0 && bid <= g_sp[i].vSL) cls = true;
               if(g_sp[i].dir==-1 && g_sp[i].vSL > 0 && ask >= g_sp[i].vSL) cls = true;
            }
         }
      }
...
ulong OpenOrder(ENUM_ORDER_TYPE type,string comment,double slMul)
{
   ...
   // v16.7 FIX 3: guardar tiempo de apertura INDEPENDIENTEMENTE del cierre
   // Evita que trades de 0s salten el cooldown al resetear CountLegacyOpen
   // g_lastOpenAttemptTime se actualiza aquí ANTES de enviar la orden
   if(!(bool)MQLInfoInteger(MQL_TESTER))
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.7 ]",clrGold,       0*lh,x,y,co,fn);
   ...
   // v16.7: trail escalado por lot en live
   double curLot = CalcLot();
   double lScaleD = (!(bool)MQLInfoInteger(MQL_TESTER) && curLot>0 && Trail_Lot_Ref>0)
                    ? MathMax(0.5, MathMin(Trail_Lot_Ref/curLot, 5.0)) : 1.0;
   string trail_str="Act:$"+DoubleToString(TP_Trail_Activate_USD*lScaleD,2)+" Ret:$"+DoubleToString(TP_Trail_Retrace_USD*lScaleD,2)+(lScaleD!=1.0?" [×"+DoubleToString(lScaleD,1)+"]":" [FIJO]");
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.7 | LIVE PARITY ENGINE | Mode=",ms,
         ...
         " | TrailProfit=",Enable_Trailing_Profit," act=$",TP_Trail_Activate_USD," ret=$",TP_Trail_Retrace_USD,
         " | TrailStop=",Enable_TrailingStop," ATR×",TS_Start_ATR_Multiplier,
         " | v16.7: MQL_TESTER bifurca 5 fixes live vs backtest");
