...
input double Min_K_Velocity             = 0.2;    // K debe moverse mínimo X pts entre barras (era 0.5)
input int    G2_Legacy_Threshold        = 13;     // G2 cada N legacies (forense: 13, param: 4)
input bool   Enable_AntiImpulse          = true;   // bloquea entrada si vela actual es impulso fuerte contrario
...
{
   // 1. EMA50 gate: bloquea dirección contra tendencia intraday (50 min)
   ...
   // 3. K velocity: señal debe tener momentum mínimo
   if(!KVelocityOK(k_now, k_prev, sig)) return false;

   // 4. Anti-impulso: no entrar contra vela actual con fuerza
   if(!AntiImpulseOK(sig)) return false;

   // 5. Cooling-off: no operar en dirección pausada
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.19 | Mode=",ms," | EMA50=",Enable_EMA20_Gate,"(p=",EMA20_Period,") | KVel=",Min_K_Velocity,
