//+------------------------------------------------------------------+
//|  MoneyMachine7_v1794.mq5                                        |
//|  v17.94 — MÁXIMO VOLUMEN + MEJOR CALIDAD                  |
//|                                                                  |
//|  BASE: v17.91 (WR=59%, PF=2.36, +$290, R:R=1.64)              |
//|  DIAGNÓSTICO: 44 trades reales analizados                      |
//|                                                                  |
//|  BUG A — BITMASK INCORRECTO (miércoles no se bloqueó):         |
//|  111 decimal = 0b01101111 → bloquea bit4=JUEVES, no Miércoles  |
//|  119 decimal = 0b01110111 → bloquea bit3=MIÉRCOLES ← CORRECTO  |
//|  FIX A: DayOfWeek_Mask 111→119                                 |
//|                                                                  |
//|  BUG B — H22 SIGUE ACTIVA (WR=25%, -$24, 3 SL_Caps):          |
//|  H22 historial: v17.80 WR=56%(-$4), v17.91 WR=25%(-$24)       |
//|  Demasiado volátil — en días malos es catastrófico             |
//|  FIX B: H22 eliminada permanentemente del código               |
//|                                                                  |
//|  BUG C — SL AMPLIO CREA 16 SL_CAPS (paradoja matemática):     |
//|  ATR_SL_Mult=1.5 + SL_Min=19.2 → SL de mercado a 19pts=$19   |
//|  El precio oscila libremente hasta $12 → activa SL_Cap         |
//|  16 SL_Caps × $12 = -$197 (68% del profit bruto destruido)    |
//|  Con ATR_SL_Mult=0.8: SL≈6-8pts≈$6-8 < SL_Cap=$12            |
//|  → SL de mercado cierra ANTES que el SL_Cap → caps desaparecen|
//|  FIX C: ATR_SL_Mult 1.5→0.8, SL_Min 19.2→10.0                |
//|                                                                  |
//|  CONFIRMADO QUE FUNCIONA (mantener igual):                     |
//|  ✓ Trail_Start=$13: avg_win $13.89→$19.34 (+39%) EXCELENTE    |
//|  ✓ R:R: 0.81→1.64 (casi se duplicó, en dirección correcta)    |
//|  ✓ DoW scaling: Lunes +$172 (59% del profit), Jueves fuerte   |
//|  ✓ SL_Cap=$12 (mantener como último escudo)                   |
//|  ✓ Señal: Zone=0.555, Confirm=4.6 → no tocar                  |
//|  ✓ TP dinámico ATR×3: 9 TPs avg $30.56                        |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.94"
#property strict
...
input double Trail_Dist_Phase2   = 12.0;  // v17.94: 8→12 (más espacio para winners H15)
...
input double Daily_Loss_Limit_USD  = 25.0;
// v17.94: Weekly equity trail — proteger semanas extraordinariamente malas
// 15% desde el peak semanal es un umbral muy liberal: solo activa en crash
// No afecta semanas normales de operación
input bool   Use_Weekly_Trail      = true;
input double Weekly_DD_Max         = 0.15; // $25 (con HC off las pérdidas son menores)
...
double   g_week_trail_peak   = 0.0;
bool     g_week_trail_halt   = false;
datetime g_week_trail_start  = 0;
...
void InitLearning()
{
   ...
   g_week_trail_peak  = AccountInfoDouble(ACCOUNT_EQUITY);
   g_week_trail_halt  = false;
   g_week_trail_start = 0;
   for(int h = 0; h < 24; h++){
      g_hourStats[h].n = 0;
...
// v17.94: Weekly equity trail — activa solo en crashes semanales (>15% DD)
// Protege semanas extraordinariamente buenas sin limitar operación normal
void UpdateWeeklyTrail()
{
   if(!Use_Weekly_Trail) return;
   MqlDateTime dt; TimeToStruct(TimeCurrent(), dt);
   int days_back = (dt.day_of_week == 0) ? 6 : (dt.day_of_week - 1);
   datetime wk = StringToTime(StringFormat("%04d.%02d.%02d 00:00",
                              dt.year, dt.mon, dt.day - days_back));
   if(g_week_trail_start != wk){
      g_week_trail_start = wk;
      g_week_trail_halt  = false;
      g_week_trail_peak  = AccountInfoDouble(ACCOUNT_EQUITY);
   }
   if(g_week_trail_halt) return;
   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
   if(eq > g_week_trail_peak) g_week_trail_peak = eq;
   if(g_week_trail_peak > 0){
      double dd = (g_week_trail_peak - eq) / g_week_trail_peak;
      if(dd > Weekly_DD_Max){
         g_week_trail_halt = true;
         g_pendingDir = 0;
         Print("WEEKLY TRAIL HALT: DD=", DoubleToString(dd*100,1),
               "% desde $", DoubleToString(g_week_trail_peak,2));
      }
   }
}
...
void OnTick()
{
   ...
   // v17.94: Weekly equity trail check
   if(Use_Weekly_Trail){ UpdateWeeklyTrail(); if(g_week_trail_halt) return; }
