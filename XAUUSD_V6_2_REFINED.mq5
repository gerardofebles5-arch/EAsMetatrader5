//+------------------------------------------------------------------+
//|                                XAUUSD_V6_2_REFINED.mq5           |
//|                V6.2B: REFINADO + BREAK-EVEN + RR OPTIMIZADO      |
//|    Mejoras: Break-Even, RR 1:3, Trailing mejorado, Sin filtros  |
//+------------------------------------------------------------------+
#property copyright "V6.2B - Refined + Break-Even"
...
// Risk Management
input double SL_ATR_Multiplier = 1.5;     // SL más ajustado
input double TP_ATR_Multiplier = 4.5;     // TP más cercano (RR 1:3)
...
input double Trailing_Step_ATR = 1.0;      // Más ajustado

// Filtro de Sesión
input bool Filter_Session = false;        // Desactivado para más trades
...
{
    Print("========================================");
    Print("XAUUSD V6.2B - REFINED + BREAK-EVEN");
    Print("========================================");
    Print("RR: 1:3 (SL 1.5×ATR, TP 4.5×ATR)");
    Print("Break-Even: ACTIVO");
    Print("Trailing Stop: ACTIVO (más ajustado)");
    Print("Filtro Sesión: DESACTIVADO");
...
void CheckForEntry()
{
    ...
    if(long_signal)
    {
        Print("📈 SEÑAL LONG - Ejecutando");
        ExecuteTrade(ORDER_TYPE_BUY, current_atr);
        g_tradesToday++;
    }
    else if(short_signal)
    {
        Print("📉 SEÑAL SHORT - Ejecutando");
