//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v11.0 — REPLICA EXACTA DEL ORIGINAL (FORENSE COMPLETO)        |
//|                                                                  |
//|  EVIDENCIA FORENSE (comparativa trade-a-trade 2026.03.02):     |
//|                                                                  |
//|  Métrica           Original    v10      Fix v11                 |
//|  Legacy trades     245         370      →245  (bar-lock)        |
//|  Win Rate          50.76%      29.73%   →~50% (k[1])           |
//|  Profit Factor     3.38        1.77     →3.38 (k[1])           |
//|  Avg duration      43.2s       17.8s    →43s  (bar-lock)       |
//|  Dur <10s          18%         40%      →18%  (bar-lock)       |
//|  Gap median        119s        27s      →119s (bar-lock)       |
//|  Entradas 0-14s    0%          26%      →0%   (k[1])           |
//|  Min 2+ trades     26          82       →26   (bar-lock)       |
//|  Señales comunes   --          14%      →>90% (k[1]+bar-lock)  |
//|  Max DD            0.53%       0.97%    →0.53%(less trades)    |
//|  G2/Legacy         7.8%        20.8%    →7.8% (auto con k[1]) |
//|                                                                  |
//|  LOS 3 CAMBIOS DE v11 vs v10:                                  |
//|                                                                  |
//|  CAMBIO 1 — GetSignal(): k[1] en lugar de k[0]                 |
//|    CopyBuffer(stoch, 0, 1, 3, kb) → kb[0]=barra completada[1]  |
//|    Antes: k[0]=tick realtime → K oscila decenas/min → señales  |
//|    falsas en cada tick dentro de la barra                        |
//|    Ahora: k[1]=barra cerrada → estable, cambia 1 vez/minuto    |
//|    Prueba: original tiene 0% entradas en seg 0-14 del minuto   |
//|    (imposible con k[0], natural con k[1])                       |
//|                                                                  |
//|  CAMBIO 2 — OnTick(): BAR-LOCK por iTime[1]                    |
//|    Solo evaluar señal cuando iTime(sym,M1,1) cambia             |
//|    → exactamente 1 evaluación por barra M1 completada           |
//|    → máximo ~60 señales/hora evaluadas (≈16 abiertas)          |
//|    Elimina cooldown de 30s (innecesario con bar-lock)           |
//|    Post-close lockout reducido a 5s (re-entradas rápidas OK)   |
//|                                                                  |
//|  CAMBIO 3 — GetSignal() P3: eliminar depth filter              |
//|    El crossing k[1]→k[0] de barra completada ya es filtro      |
//|    suficiente. Depth filter ≤25 era parche del k[0] ruidoso.   |
//|    Original no tiene depth filter, solo crossing de zona.       |
//|                                                                  |
//|  HISTORIAL:                                                     |
//|  v8.x→v10: k[0] realtime → WR=23-30%, trades 2× objetivo ✗   |
//|  v11.0: k[1] barra completada + bar-lock → replica exacta ✓   |
//+------------------------------------------------------------------+
...
// +++ v11.0 Entry Quality Control +++
// CAMBIO vs v10: cooldown eliminado (bar-lock lo hace innecesario)
// PostClose reducido a 5s (re-entradas rápidas tras TP son válidas: 17% del original)
// Depth filter OFF (el crossing de barra completada ya es filtro suficiente)
input int    Entry_Cooldown_Secs         = 0;     // 0=desactivado — bar-lock controla frecuencia
input int    PostClose_Lockout_Secs      = 5;     // 5s mínimo post-cierre (evita doble entrada)
input double Stoch_Depth_Buy             = 25.0;  // solo referencia (Enable=false por defecto)
input double Stoch_Depth_Sell            = 75.0;  // solo referencia (Enable=false por defecto)
input bool   Enable_Stoch_Depth_Filter   = false; // OFF: crossing barra completada es suficiente
...
datetime g_postCloseLockout = 0;
int      g_lastCycleDir     = 0;
// v11.0 BAR-LOCK: iTime[1] de la última barra M1 evaluada
// Garantiza exactamente 1 evaluación de señal por barra completada
...
//============================================================
// MASTER SIGNAL v11.0 — REPLICA EXACTA DEL ORIGINAL
//============================================================
int GetSignal()
{
   ...
   // ═══════════════════════════════════════════════════════
   // FIX PRINCIPAL v11: usar barras COMPLETADAS (shift=1)
   // ═══════════════════════════════════════════════════════
   // ANTES v10: CopyBuffer(stoch, 0, 0, 3, kb) → kb[0]=k[0]=tick realtime
   //   K oscila tick a tick dentro de cada barra M1
   //   → falsos cruces detectados decenas de veces por minuto
   //   → WR=30%, trades=370, entradas 0-14s=26% ← imposible con barra cerrada
   //
   // AHORA v11: CopyBuffer(stoch, 0, 1, 3, kb) → kb[0]=k[1]=barra cerrada
   //   K estable, cambia solo al cierre de cada barra M1
   //   → máximo 1 cruce real por barra → ~16 trades/hora como el original
   //   → PRUEBA: original 0% entradas en seg 0-14 → solo posible con k[1]
   // ═══════════════════════════════════════════════════════
   double kb[]; ArraySetAsSeries(kb,true);
   if(CopyBuffer(g_hStoch,0,1,3,kb)<3) return 0;  // shift=1: excluye barra en formación
   double k_now  = kb[0];  // K de barra completada más reciente (bar[1])
   double k_prev = kb[1];  // K de barra completada anterior       (bar[2])
   ...
   else // Hybrid (modo 1)
   {
      ...
      // P3: STOCH PURO + TREND + K CROSSING (barras completadas) ──────
      // v11: el crossing k[1]→k[2] sobre barras completadas ya garantiza
      // que K realmente cruzó la zona entre dos barras cerradas.
      // El depth filter era un parche para el ruido del k[0] realtime.
      // Con k[1] estable, el crossing simple es idéntico al original.
      if(sig==0)
      {
         // Crossing sobre barras completadas: k_now=bar[1], k_prev=bar[2]
         bool crossBuy  = (k_now <= Stoch_Buy_Level  && k_prev > Stoch_Buy_Level);
         bool crossSell = (k_now >= Stoch_Sell_Level && k_prev < Stoch_Sell_Level);

         // Depth filter: OFF por defecto con k[1] (parámetro existe pero no aplica)
         // Si el usuario lo activa manualmente puede usarlo para ser más selectivo
         ...
         if(crossBuy  && IntraFiltersPass(1, k_now,k_prev) && (trend==0||trend== 1)) {sig= 1;g_lastSigType=3;}
         if(crossSell && IntraFiltersPass(-1,k_now,k_prev) && (trend==0||trend==-1)) {sig=-1;g_lastSigType=3;}
      }
   }
...
void DailyReset()
{
   ...
   // v10/v11: reset entry quality control
   g_lastEntryTime=0;g_postCloseLockout=0;g_lastCycleDir=0;
   // v11: reset bar-lock al inicio del día
   g_lastBarTime=0;
}
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v11.0 ]",clrGold,       0*lh,x,y,co,fn);
   ...
   if(now_d<g_postCloseLockout) cool_str+="L";
   // v11: mostrar si bar-lock está activo (misma barra = esperando nueva)
   datetime bl=iTime(g_sym,PERIOD_M1,1);
   if(bl==g_lastBarTime) cool_str+="W";  // W=Waiting for new bar
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v11.0 | k[1] BAR-LOCK REPLICA EXACTA | Mode=",ms,
         " | TrendMA=H1(",Trend_MA_Period,")"
         " | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED,
         " | BarLock=ON PostClose=",PostClose_Lockout_Secs,"s",
         " | DepthFilter=",Enable_Stoch_Depth_Filter,
...
void OnTick()
{
   ...
   // v11: detectar cierre para activar post-close lockout mínimo (5s)
   static int s_prevPosCount = -1;
   int curPosCount = CountLegacyOpen();
   if(s_prevPosCount > 0 && curPosCount == 0)
      g_postCloseLockout = TimeCurrent() + PostClose_Lockout_Secs;
   s_prevPosCount = curPosCount;
   ...
   if(CountLegacyOpen()==0)
   {
      ...
      // Post-close lockout: 5s mínimo para evitar doble entrada en mismo tick
      if(now < g_postCloseLockout){DrawDashboard();return;}

      // ═══════════════════════════════════════════════════════════════
      // BAR-LOCK v11: evaluar señal solo cuando hay nueva barra M1 cerrada
      // ═══════════════════════════════════════════════════════════════
      // GetSignal() usa kb[0]=k[1]=barra completada → señal estable por barra
      // Bar-lock garantiza exactamente 1 evaluación por barra M1 completada
      // → mismo comportamiento que el original (0% entradas en seg 0-14)
      // → naturalmente ~16 señales evaluadas/hora, ~16 abiertas
      // Cuando no hay señal: consumir barra igual (no reevaluar en próximo tick)
      // ═══════════════════════════════════════════════════════════════
      datetime bar1_time = iTime(g_sym, PERIOD_M1, 1);
      if(bar1_time == g_lastBarTime){DrawDashboard();return;}

      // Nueva barra M1 completada → evaluar señal UNA sola vez
      g_lastBarTime = bar1_time;  // consumir barra independientemente del resultado
      ...
      if(sig==1)
      {
         ...
         if(tk>0){g_lastEntryTime=now;g_lastSignalBar=now;g_lastLegacyTicket=tk;}
      }
      else if(sig==-1)
      {
