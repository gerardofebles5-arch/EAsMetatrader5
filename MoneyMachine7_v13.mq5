//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v13.0 — 3 FIXES FORENSES SOBRE v12                            |
//|                                                                  |
//|  DIAGNÓSTICO v12 (análisis trade-a-trade 2026.03.02):          |
//|                                                                  |
//|  PROBLEMA 1: 100% BUYs (0 SELLs)                               |
//|    Enable_Trend_Filter=true + MA200 H1 en día alcista           |
//|    → precio > MA200 todo el día → bloquea todos los SELLs       |
//|    Original: 48% BUY / 52% SELL → trend filter DESACTIVADO     |
//|    FIX: Enable_Trend_Filter = false (default)                   |
//|                                                                  |
//|  PROBLEMA 2: PostClose lockout bloquea re-entradas válidas      |
//|    Original tiene 39 re-entradas en ≤5s tras cierre (gap=1s)   |
//|    Ocurren cuando TP hit → señal sigue activa → re-entra        |
//|    inmediatamente. PostClose_Lockout_Secs=15 las bloqueaba.     |
//|    FIX: PostClose_Lockout_Secs = 0                              |
//|                                                                  |
//|  PROBLEMA 3: 370 trades vs 226 (gap open→open: 27s vs 179s)    |
//|    EMA crossing genera señales repetidas en el mismo ciclo      |
//|    Original: gap open→open median=179s, p50=179s               |
//|    FIX: Entry_Cooldown_Secs = 179 (mediana del original)       |
//|    EXCEPCIÓN: cooldown reset si SignalDir cambia (BUY→SELL)     |
//|    Esto permite re-entradas rápidas cuando el mercado revierte  |
//|                                                                  |
//|  HISTORIAL:                                                     |
//|  v10: MODE_SMA, cooldown 30s → 370 trades, 0% sells ✗         |
//|  v12: MODE_EMA → mejora timing pero sigue 370t, 0% sells ✗    |
//|  v13: +Trend_OFF +PostClose=0 +Cooldown=179s → target 226t ✓  |
//+------------------------------------------------------------------+
...
// v13 FIX: Enable_Trend_Filter = false
// Original: 48% BUY / 52% SELL → sin restricción de dirección
// Con true + MA200 H1 en día alcista → bloquea 100% de los SELLs
...
// v13 FIX2: PostClose_Lockout_Secs = 0
//   Original tiene 39 re-entradas en ≤5s (gap=1s tras TP)
//   El lockout de 15s las bloqueaba — en v13 se elimina
// v13 FIX3: Entry_Cooldown_Secs = 179
//   Original: gap open→open median=179s (p50)
//   EMA crossing repite señal en el mismo ciclo → cooldown desde apertura
//   Cooldown se resetea si la dirección de señal CAMBIA (BUY↔SELL)
input int    Entry_Cooldown_Secs         = 179;   // mediana gap open→open del original
input int    PostClose_Lockout_Secs      = 0;     // 0=sin lockout (original re-entra en 1s)
input double Stoch_Depth_Buy             = 25.0;
input double Stoch_Depth_Sell            = 75.0;
input bool   Enable_Stoch_Depth_Filter   = true;
...
int GetSignal()
{
   ...
   double k_now  = kb[0];  // K tiempo real (barra actual en formación)
   double k_prev = kb[1];  // barra completada anterior
   ...
   else // Hybrid (modo 1)
   {
      ...
      // P3: STOCH PURO + TREND + K CROSSING ESTRICTO v10 ─────────────
      // DIAGNÓSTICO v8.34: WR=23%, dur avg=11s, 841 trades
      //   → K crossing fallido: k_prev >30 pero K ya estaba profundo
      //   → throttle 3s: insuficiente cuando K oscila múltiples ciclos
      //
      // SOLUCIÓN v10 (objetivo: 245 trades, WR ~58%, dur avg ~43s):
      //   CROSSING ESTRICTO: k_prev DEBE estar FUERA (>30 / <70)
      //   DEPTH FILTER: k_now en zona PROFUNDA (≤25 / ≥75) = alta convicción
      //   COOLDOWN: Entry_Cooldown_Secs entre entradas consecutivas
      if(sig==0)
      {
         // Crossing ESTRICTO: k_prev claramente fuera de zona
         bool crossBuy  = (k_now <= Stoch_Buy_Level  && k_prev > Stoch_Buy_Level);
         bool crossSell = (k_now >= Stoch_Sell_Level && k_prev < Stoch_Sell_Level);

         // v10: Depth filter — zona profunda = señal de mayor convicción
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v13.0 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   // Esto replica el delay natural de ~18s del original:
   // con EMA el K[0] tarda ~15-20 ticks en converger → entradas en :15-:59
   // con SMA el K[0] reacciona en el primer tick → entradas falsas en :00-:14
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v13.0 | MODE_EMA+TrendOFF+Cooldown179s | Mode=",ms,
         " | TrendFilter=",Enable_Trend_Filter,
         " | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED,
         " | Cooldown=",Entry_Cooldown_Secs,"s PostClose=",PostClose_Lockout_Secs,"s",
...
void OnTick()
{
   ...
   if(CountLegacyOpen()==0)
   {
      ...
      // PostClose lockout: 0s en v13 (original re-entra en 1s tras TP)
      if(now < g_postCloseLockout){DrawDashboard();return;}

      // Entry cooldown v13: 179s desde última apertura (mediana gap open→open original)
      // EXCEPCIÓN: el cooldown se ignora si la señal actual es DIRECCIÓN OPUESTA
      // al último trade. El original re-entra inmediatamente cuando el mercado revierte
      // (BUY→SELL o SELL→BUY) porque es una señal genuinamente nueva.
      if(Entry_Cooldown_Secs > 0 && (now - g_lastEntryTime) < Entry_Cooldown_Secs)
      {
         // Evaluar señal solo para verificar si la dirección cambió
         int peekSig = GetSignal();
         bool dirChanged = (peekSig != 0 && g_lastCycleDir != 0 && peekSig != g_lastCycleDir);
         if(!dirChanged){DrawDashboard();return;}
         // Dirección cambió → cooldown ignorado, señal procesada abajo
      }

      int sig=GetSignal();
      if(sig== 1&&CountBuys() >=Max_Buy)  sig=0;
      if(sig==-1&&CountSells()>=Max_Sell) sig=0;
      if(sig==1)  {ulong tk=OpenOrder(ORDER_TYPE_BUY, CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastEntryTime=now;g_lastSignalBar=now;g_lastLegacyTicket=tk;g_lastCycleDir=1;}}
      if(sig==-1) {ulong tk=OpenOrder(ORDER_TYPE_SELL,CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastEntryTime=now;g_lastSignalBar=now;g_lastLegacyTicket=tk;g_lastCycleDir=-1;}}
   }
   DrawDashboard();
}
