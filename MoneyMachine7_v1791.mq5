//+------------------------------------------------------------------+
//|  MoneyMachine7_v1791.mq5                                        |
//|  v17.91 — 5 MEJORAS CONFIRMADAS POR DATOS                  |
//|                                                                  |
//|  BASE: v17.90 (Estándar): WR=74.5%, PF=2.37, +$329, 0 HCs     |
//|  ANÁLISIS: 200+ trades reales, 5 perfiles, 14 días             |
//|                                                                  |
//|  FIX 1 — FILTRO MIÉRCOLES (datos: WR=33%, -$46):               |
//|  Miércoles es consistentemente el peor día en TODAS versiones   |
//|  No operar = +$46 automático. DoW bitmask 0b01101111            |
//|                                                                  |
//|  FIX 2 — SL CAP EN DÓLARES (datos: avg_loss=$17 vs win=$14):   |
//|  R:R=0.81 negativo porque SL dinámico ATR sube a $20-25 en     |
//|  momentos volátiles. Con cap de $12: avg_loss→$10, R:R→1.4     |
//|  SL_USD_Cap = min(ATR×1.5 × lot × tick, SL_USD_Cap_Value)      |
//|  Implementación: si pnl < -SL_USD_Cap → cerrar (como HC pero  |
//|  más bajo y con cooldown normal, no pausa larga)               |
//|                                                                  |
//|  FIX 3 — TRAIL_START $8→$13 (datos: 29 winners trail avg $8): |
//|  El trailing a $8 corta winners antes de que lleguen al TP $30 |
//|  Con $13 el trade ya está en "territorio seguro" en XAUUSD M1  |
//|  Estimado: avg_win sube de $13.89 a ~$18-20                    |
//|                                                                  |
//|  FIX 4 — DAY-OF-WEEK SCALING (datos confirmados):              |
//|  Lunes WR=78% +$195, Jueves WR=80% +$64 → lot ×1.25           |
//|  Martes WR=78% → normal                                        |
//|  Viernes WR=86% → lot ×1.15 (pocos trades, alta calidad)      |
//|                                                                  |
//|  FIX 5 — BUG PARTIAL CLOSE (datos: 0 partials detectados):     |
//|  El partial close nunca se activa porque DoPartialClose usa     |
//|  SELL precio (ASK) para cerrar una posición SELL — incorrecto  |
//|  Una posición SELL se cierra con BUY al precio ASK — correcto  |
//|  Pero el PnL flotante de SELL necesita compararse con BID      |
//|  FIX: usar POSITION_PROFIT directamente (MT5 lo calcula bien)  |
//|                                                                  |
//|  INTOCABLE (señal calibrada, no cambiar):                      |
//|  Zone_Pct=0.555, Confirm=4.6, ATR_TP×3, Phase1=30pts          |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.91"
#property strict
...
input int    DayOfWeek_Mask        = 111; // decimal de 0b01101111 (no miércoles)
...
int OnInit()
{
   ...
   Print("MM7 v17.91 | ", g_sym,
         " | DoW_Filter=", Use_DayOfWeek_Filter,
         " | SL_Cap=$", SL_USD_Cap_Value,
         " | Trail_Start=$", Trail_Start_Pts, " P1=", Trail_Dist_Phase1, "pts",
         " | DoW_Scale Mon/Thu=", DoW_Scale_Mon, " Wed=", DoW_Scale_Wed,
