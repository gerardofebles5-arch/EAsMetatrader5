//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v18.0 — REPLICA EXACTA DEL BACKTEST DE PRODUCCION            |
//|                                                                  |
//|  BACKTEST DE REFERENCIA (2026.02.02 - 2026.03.11, 37 dias):   |
//|  Deposito: $20 | Beneficio: $2,935,264 | WR: 67.1%            |
//|  22,997 trades | BUY: 10,911 / SELL: 12,086 (casi 50/50)      |
//|  Profit Factor: 2.99 | Sharpe: 140 | MaxDD: 0.17%             |
//|  Duracion media: 15s | Lotaje dinamico: 0.01 - 5.0            |
//|                                                                  |
//|  BRECHAS IDENTIFICADAS ENTRE BACKTEST Y LIVE:                  |
//|                                                                  |
//|  BRECHA 1 (CRITICA): Motor predictivo v16 bloquea direccion    |
//|  GetPredictiveSignal() compara score_buy vs score_sell y       |
//|  siempre gana el mismo lado (el que la tendencia favorece).    |
//|  Backtest: 10,911 BUY + 12,086 SELL = equilibrado.            |
//|  Live v16: solo BUY o solo SELL segun el dia.                 |
//|  FIX: GetSignal() vuelve al Stoch puro como unica fuente de   |
//|  señal (P1 Sweep, P2 FVG, P3 Stoch crossing) exactamente      |
//|  como estaba cuando se genero el backtest.                     |
//|  Min_Prediction_Score=0 = bypass total del motor (default).   |
//|                                                                  |
//|  BRECHA 2: Trailing Profit no escalaba por lotaje             |
//|  Con lot 5.0 (balance >$500k) los $0.50 de activacion son     |
//|  insignificantes. El trailing ahora escala con el lot real.    |
//|  (fix de v16 mantenido)                                        |
//|                                                                  |
//|  BRECHA 3: Trailing Stop en Stealth no movia el vSL virtual   |
//|  Ahora mueve el SL virtual tick a tick conforme el precio      |
//|  avanza a favor. (fix de v16 mantenido)                       |
//|                                                                  |
//|  PARAMETROS CRITICOS DEL BACKTEST (NO CAMBIAR):               |
//|  Deposito=$20 | Free_margin=1000 | Max_Lot=5.0                 |
//|  TP_FIXED=2.5 | SL_FIXED=0.77 | G2_SL=0.5                    |
//|  TP_Trail_Activate=0.5 | TP_Trail_Retrace=0.3                 |
//|  Entry_Cooldown=30s | PostClose_Lockout=15s                    |
//|  Stoch(14,3,3) Buy<30 Sell>70 | Depth_Buy=25 Depth_Sell=75   |
//+------------------------------------------------------------------+
...
input int    Min_Prediction_Score        = 0;      // 0=REPLICA EXACTA BACKTEST (Stoch puro). 3=filtro activo (menos trades)
// +++ Forensic Core +++
...
// v18: REPLICA EXACTA DEL BACKTEST
// Stochastic puro es la fuente de señal (como en el backtest de produccion).
// El motor predictivo es OPCIONAL: si Min_Prediction_Score=0 → bypass total.
// Esto produce BUY y SELL equilibrados exactamente como en el backtest.
//============================================================
int GetSignal()
{
   ...
   else // Hybrid modo 1 — EXACTAMENTE como backtest: Sweep → FVG → Stoch
   {
      ...
      // P3: Stoch cruce — señal principal, igual que en el backtest
      if(sig==0)
      {
   ...
   // ── FILTRO PREDICTIVO OPCIONAL (v16) ──────────────────────────
   // Min_Prediction_Score=0 → bypass total → replica exacta del backtest
   // Min_Prediction_Score=3 → filtro activo → menos trades, más selectivo
   // RECOMENDADO PARA REPLICA: Min_Prediction_Score=0
   if(sig != 0 && Min_Prediction_Score > 0)
   {
      // Motor como filtro de calidad — NO decide direccion, solo valida
      int score_dir = GetMTFConfluence(sig) + GetOrderFlowBias(sig) +
                      GetMomentumBias(sig)  + GetPriceStructure(sig) + GetStochTiming(sig);
      g_lastPredScore = score_dir;
      g_lastPredDir   = sig;
      if(score_dir < Min_Prediction_Score) return 0;
   }
   else if(sig != 0)
   {
      // Bypass: calcular score solo para dashboard (no filtra)
      g_lastPredScore = GetMTFConfluence(sig) + GetOrderFlowBias(sig) +
                        GetMomentumBias(sig)  + GetPriceStructure(sig) + GetStochTiming(sig);
      g_lastPredDir   = sig;
   }
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v18.0 ]",clrGold,       0*lh,x,y,co,fn);
   string replica_str = (Min_Prediction_Score==0) ? " [REPLICA BT]" : " [Filter="+IntegerToString(Min_Prediction_Score)+"]";
   DashLbl("1","Mode : "+ms+replica_str+" Trail:"+(Enable_Trailing_Profit?"ON":"off"),clrCyan,1*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v18.0 | REPLICA EXACTA BACKTEST | Mode=",ms,
         " | PredFilter=",Min_Prediction_Score,(Min_Prediction_Score==0?" (BYPASS=replica exacta)":" (filtro activo)"),
         " | TP_FIXED=",MM7_TP_FIXED," SL_FIXED=",MM7_SL_FIXED,
         " | Trail=",Enable_Trailing_Profit," act=$",TP_Trail_Activate_USD,"(xlot) ret=$",TP_Trail_Retrace_USD,"(xlot)",
         " | TrailStop=",Enable_TrailingStop," G2c/",G2_Legacy_Threshold," G2s/",G2_Min_Seconds,
         " | BUY+SELL equilibrado como backtest 02.02-03.11");
