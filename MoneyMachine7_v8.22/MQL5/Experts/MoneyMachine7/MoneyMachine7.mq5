...
input bool   Enable_PrevCandle_Confirm   = true;   // vela anterior cerró en dirección correcta (reduce 87% pérdidas rápidas)
input double PrevCandle_Min_Body_Ratio   = 0.25;   // cuerpo vela anterior >= rango*ratio (0.25=body>=25% del rango)
input bool   Enable_FastLoss_Cooldown    = true;   // pausa extra si 2+ pérdidas ultrarrápidas en ventana
...
input bool   Enable_RSI_Trend_Gate       = true;   // RSI>50 para BUY, RSI<50 para SELL (confirma tendencia)
input double RSI_Trend_Min               = 48.0;   // RSI mínimo para BUY (48=permisivo, evita falsos)
input double RSI_Trend_Max               = 52.0;   // RSI máximo para SELL (52=permisivo)
...
bool PrevCandleConfirm(int dir)
{
   ...
   double range=h1-l1;
   if(range < g_point*2) return true; // doji/micro-vela → no filtrar
   double body=MathAbs(c1-o1);
   if(dir== 1) return (c1>o1 && body>=range*PrevCandle_Min_Body_Ratio);
   if(dir==-1) return (c1<o1 && body>=range*PrevCandle_Min_Body_Ratio);
   return true;
}
...
bool IntraFiltersPass(int sig, double k_now, double k_prev)
{
   ...
   // 2. Vela anterior confirma dirección (ataca 87% pérdidas rápidas)
   if(!PrevCandleConfirm(sig)) return false;
   // 3. RSI Trend Gate (confirma momentum, umbrales permisivos 48/52)
   if(!RSITrendGate(sig)) return false;
   // 4. Anti-impulso (OFF por defecto)
   if(!AntiImpulseOK(sig)) return false;
   // 5. Cooling-off direccional
   if(IsCoolingOff(sig)) return false;
   // 6. Fast-loss pause (volatilidad extrema)
...
int GetSignal()
{
   ...
   else // Hybrid (modo 1)
   {
      ...
      if(sig==0)
      {
         ...
         // Filtros P3: estricto (D-confirm + LocalPA)
...
void DrawDashboard()
{
   ...
   DashLbl("7","EMA:"+ema_str+" LPA:"+lpa_str+" ATR:"+DoubleToString(GetATR(),3),clrGray,7*lh,x,y,co,fn);
   string cool_str="";
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.20 | Mode=",ms," | EMA50=",Enable_EMA20_Gate,"(p=",EMA20_Period,") | KVel=",Min_K_Velocity,
