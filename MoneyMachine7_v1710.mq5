//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v17.10 — ANÁLISIS v17.9 → MEJORAS QUIRÚRGICAS                 |
//|                                                                  |
//|  RESULTADOS v17.9: Net=+$9.20 | PF=1.60 | WR=44% | 9 trades   |
//|                                                                  |
//|  PROBLEMAS IDENTIFICADOS:                                        |
//|  1. Después de SL, el siguiente trade también pierde:           |
//|     15:34 SL → 15:41 SL (30s cooldown insuficiente)            |
//|     18:44 SL → 18:48 SL (mismo patrón)                         |
//|  2. SELLs en NY muy malos: 1 TP de 4 (25% WR)                  |
//|     Solo el SELL de 14:59 ganó (tendencia bajista clara)        |
//|  3. Trade 18:44 BUY→SL siempre pierde (cierre sesión tarde)    |
//|                                                                  |
//|  FIXES v17.10:                                                   |
//|  FIX 1 — SL COOLDOWN 300s (era 30s):                           |
//|    Evita entrar inmediatamente después de un SL                 |
//|    15:34 SL → espera hasta 15:39 → evita 15:41 SL             |
//|    18:44 SL → espera hasta 18:49 → evita 18:48 SL             |
//|  FIX 2 — CIERRE SESIÓN 18:30 UTC (era 20:00):                  |
//|    Elimina trades 18:44 y 18:48 que siempre pierden            |
//|    Los 4 ganadores clásicos ocurren antes de las 18:30         |
//|  FIX 3 — FILTRO SELL EN NY: solo si EMA21 bajando              |
//|    SELLs solo cuando EMA21[0] < EMA21[1] (pendiente negativa)  |
//|    El único SELL ganador (14:59) fue en tendencia bajista clara |
//+------------------------------------------------------------------+
...
// +++ Sesión NY (FIX 2) +++
input int    NY_Start_Hour               = 14;    // 14:30 UTC
input int    NY_Start_Min                = 30;
input int    NY_End_Hour                 = 18;    // FIX 2: 18:30 (era 20:00)
input int    NY_End_Min                  = 30;    // elimina trades 18:44+
// +++ Salida +++
input double MM7_TP_FIXED                = 1.00;
input double MM7_SL_FIXED                = 0.50;
// +++ Entry Control +++
input int    Entry_Cooldown_Secs         = 3;
input int    SL_Cooldown_Secs            = 300;   // FIX 1: 300s (era 30s)
...
// FIX 3: filtro SELL — solo si EMA21 tiene pendiente negativa
bool IsTrendDown()
{
   if(g_hEMAt == INVALID_HANDLE) return false;
   int start = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
   double et[];
   ArraySetAsSeries(et, true);
   if(CopyBuffer(g_hEMAt, 0, start, 2, et) < 2) return false;
   return (et[0] < et[1]); // EMA21 bajando
}
...
bool IsTrendUp()
{
   ...
   ArraySetAsSeries(et, true);
   if(CopyBuffer(g_hEMAt, 0, start, 2, et) < 2) return false;
   return (et[0] > et[1]); // EMA21 subiendo
}

// Señal: EMA3/8 cruce + EMA21 posición (precio vs EMA21)
int GetSignal()
{
   ...
   // precio debe estar del lado correcto de EMA21
   if(crossUp   && bid <= et_now) return 0;
   if(crossDown && bid >= et_now) return 0;

   // FIX 3: SELLs solo si EMA21 tiene pendiente bajista
   if(crossDown && !IsTrendDown()) return 0;

   return crossUp ? 1 : -1;
}

// Sesión NY con cierre a las 18:30 (FIX 2)
bool IsNYSession()
{
                        ...
                        const MqlTradeResult      &result)
{
   ...
   if(reason == DEAL_REASON_SL)
   {
      ...
      Print("MM7 SL hit → cooldown ", SL_Cooldown_Secs, "s (", SL_Cooldown_Secs/60, " min)");
   }
}
...
void DrawDashboard()
{
   ...
   DashLbl("0", "[ MoneyMachine7 v17.10 ]", clrGold, 0*lh, x, y, co, fn);
   DashLbl("1", "NY "+IntegerToString(NY_Start_Hour)+":"+IntegerToString(NY_Start_Min)+"-"+IntegerToString(NY_End_Hour)+":"+IntegerToString(NY_End_Min)+" UTC  "+(inNY?"ACTIVA":"inactiva"), inNY?clrLimeGreen:clrGray, 1*lh, x, y, co, fn);
   ...
   DashLbl("8", "Cool:"+cdStr+" SLcd:"+IntegerToString(SL_Cooldown_Secs/60)+"min Halt:"+(g_haltedToday?"Y":"n"), clrGray, 8*lh, x, y, co, fn);
}
...
int OnInit()
{
   ...
   Print("MM7 v17.10 | EMA(",EMA_Fast_Period,"/",EMA_Slow_Period,"/",EMA_Trend_M1_Period,") M1",
         " | NY ",NY_Start_Hour,":",NY_Start_Min,"-",NY_End_Hour,":",NY_End_Min," UTC",
         " | SL_CD=",SL_Cooldown_Secs,"s | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED,
         " | FIX: SL_CD 300s + cierre 18:30 + filtro SELL pendiente");
...
void OnTick()
{
   ...
   // FIX 2: solo sesión NY 14:30-18:30 UTC
   ...
   datetime now = TimeCurrent();
   // FIX 1: SL cooldown 300s — evita trade inmediato después de SL
