...
//--------------------------------------------------------------------
// MOTOR PREDICTIVO — FILTRO DE CALIDAD (no decide dirección)
// FIX LIVE: el motor ya NO compite score_buy vs score_sell.
// Esa competición siempre elegía el lado que la tendencia favorecía
// ese día, bloqueando la dirección opuesta por horas/días completos.
// Ahora: recibe la dirección que el Stochastic quiere operar y
// responde si esa señal tiene calidad suficiente (score >= mínimo).
// El Stochastic sigue siendo quien decide BUY o SELL — igual que
// en el backtest. El motor solo filtra señales de baja calidad.
//--------------------------------------------------------------------
int GetPredictiveSignal()
{
   // Este wrapper ya no se usa como generador de dirección.
   // GetSignal() llama directamente a CalcPredScore(dir).
   // Se mantiene para compatibilidad con llamadas de dashboard.
   double kb[]; ArraySetAsSeries(kb,true);
   if(CopyBuffer(g_hStoch,0,0,1,kb)<1) return 0;
   double k = kb[0];
   int guess = (k <= Stoch_Buy_Level) ? 1 : (k >= Stoch_Sell_Level) ? -1 : 0;
   if(guess != 0)
   {
      g_lastPredScore = CalcPredScore(guess);
      g_lastPredDir   = guess;
   }
   return 0; // nunca genera señal propia — solo informa al dashboard
}

// Calcula el score predictivo para UNA dirección dada
// Retorna 0-5 (cuántas capas confirman esa dirección)
int CalcPredScore(int dir)
{
   int score = 0;
   score += GetMTFConfluence(dir);
   score += GetOrderFlowBias(dir);
   score += GetMomentumBias(dir);
   score += GetPriceStructure(dir);
   score += GetStochTiming(dir);
   return score;
}
...
int GetSignal()
{
   ...
   else // Hybrid modo 1 — flujo idéntico al backtest
   {
      ...
      // P3: Stoch crossing — señal principal, idéntico al backtest
      if(sig==0)
      {
   ...
   // Motor predictivo como filtro de calidad (NO como generador de dirección)
   // Min_Prediction_Score=3 → igual que el backtest v16
   if(sig!=0 && Min_Prediction_Score>0)
   {
      int quality = CalcPredScore(sig);
      g_lastPredScore = quality;
      g_lastPredDir   = sig;
      if(quality < Min_Prediction_Score) return 0;
   }
   else if(sig!=0)
   {
      g_lastPredScore = CalcPredScore(sig);
      g_lastPredDir   = sig;
   }
