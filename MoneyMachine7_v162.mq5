...
// v16.2 — WARMUP GUARD REAL
// FIX: v16.1 aceptaba datos VIEJOS de bar[1] como "frescos" porque CopyBuffer(bar1)
// siempre devuelve algo aunque sea de hace 14+ min. Solucion real: guardar
// iTime(M5,1) y iTime(M15,1) al arrancar. Warmup termina SOLO cuando esos
// timestamps hayan avanzado = barra NUEVA cerrada en cada timeframe.
// En backtest: tester precarga datos → iTime ya avanza desde tick 1 → OK inmediato.
// En live: espera que cierre 1 barra nueva de M5 (5min) Y 1 de M15 (15min).
bool     g_warmupDone      = false; // true cuando los indicadores estan listos
datetime g_warmupStartTime = 0;     // momento en que arranco el EA
datetime g_warmup_M5_bar   = 0;     // iTime(M5,1) al arrancar - esperar que cambie
datetime g_warmup_M15_bar  = 0;     // iTime(M15,1) al arrancar - esperar que cambie
...
void ScanFVGs()
{
   // v16.2 FIX: usar bar[1] (cerrada) como trigger de scan en lugar de bar[0]
   // En live, el primer tick de una barra nueva puede llegar antes de que iTime(bar0)
   // se estabilice. Usar bar[1] garantiza que escaneamos cuando la barra anterior
   // ya cerro completamente. En backtest: sin diferencia porque el tester es deterministico.
   datetime bt=iTime(g_sym,_Period,1); if(bt==g_fvgLastScan) return; g_fvgLastScan=bt;
...
int GetMTFConfluence(int dir)
{
   ...
   // M5: tendencia — v16.2 FIX: verificar que la barra M5[1] sea reciente
   // Si iTime(M5,1) tiene mas de 2 periodos de antiguedad → datos obsoletos → bypass
   if(g_hEMA_M5 != INVALID_HANDLE)
   {
      datetime m5_bar_time = iTime(g_sym, PERIOD_M5, 1);
      bool m5_fresh = (TimeCurrent() - m5_bar_time <= PeriodSeconds(PERIOD_M5) * 2);
      double em5[]; ArraySetAsSeries(em5,true);
      if(m5_fresh && CopyBuffer(g_hEMA_M5,0,1,1,em5)>=1)
      {
         double bid = SymbolInfoDouble(g_sym, SYMBOL_BID);
         int d5 = (bid > em5[0]) ? 1 : -1;
         if(d5 == dir) score++;
      }
      else if(!m5_fresh) score++; // datos obsoletos → bypass (neutral, no penaliza)
   }

   // M15: tendencia — v16.2 FIX: verificar que la barra M15[1] sea reciente
   if(g_hEMA_M15 != INVALID_HANDLE)
   {
      datetime m15_bar_time = iTime(g_sym, PERIOD_M15, 1);
      bool m15_fresh = (TimeCurrent() - m15_bar_time <= PeriodSeconds(PERIOD_M15) * 2);
      double em15[]; ArraySetAsSeries(em15,true);
      if(m15_fresh && CopyBuffer(g_hEMA_M15,0,1,1,em15)>=1)
      {
         double bid = SymbolInfoDouble(g_sym, SYMBOL_BID);
         int d15 = (bid > em15[0]) ? 1 : -1;
         if(d15 == dir) score++;
      }
      else if(!m15_fresh) score++; // datos obsoletos → bypass (neutral, no penaliza)
   }
...
int GetMomentumBias(int dir)
{
   ...
   // v16.2 FIX: leer 4 barras cerradas (start=1) en lugar de 3 desde bar0
   // Antes: CopyBuffer(start=0, count=3) leía bar0 (en formacion) + bar1 + bar2
   // Ahora: CopyBuffer(start=1, count=4) lee bar1+bar2+bar3+bar4 (todas cerradas)
   // Resultado en live: señales estables, no cambian tick a tick dentro del mismo minuto
   // Resultado en backtest: identico porque bar[0] final = bar[1] del tick siguiente
   if(CopyBuffer(g_hEMA_Fast,0,1,4,ef_now)<4) return 1;
   if(CopyBuffer(g_hEMA_Slow,0,1,4,es_now)<4) return 1;

   // v16.2 FIX: usar barras CERRADAS [1,2,3] en lugar de [0,1,2]
   // bar[0] = barra en formacion, cambia CADA TICK en live → senales inestables
   // bar[1,2,3] = barras cerradas = deterministas = mismo comportamiento que backtest
   // Nota: en backtest bar[0] al cierre = bar[1] del tick siguiente → sin diferencia
   double gap0 = ef_now[1] - es_now[1]; // gap barra cerrada (era bar0 = en formacion)
   double gap1 = ef_now[2] - es_now[2]; // gap barra anterior cerrada
   double gap2 = ef_now[3] - es_now[3]; // gap 2 barras atras (necesita 4 barras)
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      // v16.2 FIX: El bug principal de "no cierra" en live es que con
      // Enable_TrailingStop=true, el vSL original se ignoraba COMPLETAMENTE
      // aunque el TrailingStop aun no se habia activado (precio no subio lo
      // suficiente). Resultado: la posicion no tenia SL real ni virtual → queda
      // abierta indefinidamente perdiendo. Broker SL=0 (stealth) + sin vSL = sin proteccion.
      //
      // Solucion: el vSL original SIEMPRE actua como red de seguridad.
      // Si TrailingStop esta activo Y ya movio el vSL → ese vSL nuevo protege.
      // Si TrailingStop NO se ha activado aun → el vSL original es la proteccion.
      // El TP virtual fijo SIEMPRE aplica (TrailingProfit puede cerrar antes).
      if(!cls)
      {
         // TP fijo virtual: aplica siempre (TrailingProfit puede cerrar antes)
         if(g_sp[i].dir== 1 && g_sp[i].vTP > 0 && bid >= g_sp[i].vTP) cls = true;
         if(g_sp[i].dir==-1 && g_sp[i].vTP > 0 && ask <= g_sp[i].vTP) cls = true;

         // SL virtual: SIEMPRE aplica como red de seguridad
         // Con TrailingStop activo: vSL puede haber sido actualizado al nivel del trail
         // Con TrailingStop inactivo (precio no subio lo suficiente): vSL = SL original
         // En AMBOS casos protege la cuenta
         if(!cls)
         {
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.2 ]",clrGold,       0*lh,x,y,co,fn);
   int warmup_secs = (int)(TimeCurrent() - g_warmupStartTime);
   string warmup_str = g_warmupDone ? "" : " [WARMUP "+IntegerToString(warmup_secs)+"s]";
...
int OnInit()
{
   ...
   // v16.2: inicializar warmup guard REAL
   // Guardar los timestamps de las barras cerradas actuales de M5 y M15.
   // Warmup termina cuando AMBOS timestamps hayan avanzado (= barra nueva cerrada).
   // En backtest el tester precarga historico completo -> avanza inmediatamente.
   g_warmupDone      = false;
   g_warmupStartTime = TimeCurrent();
   // Capturar baseline de barras cerradas al arrancar
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.2 | LIVE PARITY FIX | Mode=",ms,
         ...
         " | FIXES: WarmupGuard(bar-timestamp)+MomBias(closedBars)+MTF(freshness)+SL(safetyNet)+FVG(bar1)");
...
void OnTick()
{
   ...
   // ── v16.2 WARMUP GUARD REAL ──────────────────────────────────────
   // FIX CRITICO: v16.1 usaba CopyBuffer para verificar datos "frescos",
   // pero CopyBuffer(bar1) siempre devuelve datos aunque tengan 14+ min.
   // Resultado: warmup terminaba con datos VIEJOS → score_sell dominaba → solo SELLs.
   //
   // SOLUCION v16.2: comparar iTime(M5,1) e iTime(M15,1) con el baseline
   // capturado en OnInit. Si el timestamp avanzó → se cerró una barra NUEVA
   // → los indicadores ahora tienen datos que incluyen precio post-arranque.
   //
   // En BACKTEST: el tester precarga el historico completo antes del primer tick
   // → iTime ya es distinto al baseline desde el primer tick → g_warmupDone=true
   // inmediatamente → comportamiento identico a v16.1 en backtest.
   if(!g_warmupDone)
   {
      datetime cur_M5_bar  = (Enable_MTF_Confluence && g_hEMA_M5  != INVALID_HANDLE) ? iTime(g_sym, PERIOD_M5,  1) : TimeCurrent();
      datetime cur_M15_bar = (Enable_MTF_Confluence && g_hEMA_M15 != INVALID_HANDLE) ? iTime(g_sym, PERIOD_M15, 1) : TimeCurrent();

      // Warmup completo cuando AMBAS barras base hayan sido superadas por una nueva
      bool m5_new_bar  = (!Enable_MTF_Confluence || g_hEMA_M5  == INVALID_HANDLE || cur_M5_bar  > g_warmup_M5_bar);
      bool m15_new_bar = (!Enable_MTF_Confluence || g_hEMA_M15 == INVALID_HANDLE || cur_M15_bar > g_warmup_M15_bar);

      int seconds_running = (int)(TimeCurrent() - g_warmupStartTime);

      if(m5_new_bar && m15_new_bar)
      {
         g_warmupDone = true;
         Print("MM7 v16.2 | WARMUP COMPLETADO tras ", seconds_running, "s | M5_bar OK M15_bar OK | Senales activadas");
      }
      else
      {
         if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
         {
            Comment("MM7 v16.2 | WARMUP: esperando barra nueva | M5:", (m5_new_bar?"OK":"wait"), " M15:", (m15_new_bar?"OK":"wait"), " (", seconds_running, "s)");
            g_lastDashTime = TimeCurrent();
         }
         return; // NO procesar senales durante warmup
      }
   }
   // ── FIN WARMUP GUARD ─────────────────────────────────────────────
