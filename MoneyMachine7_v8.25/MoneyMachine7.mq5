//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.25                                         |
//|  CAMBIOS vs v8.24 — basados en análisis REAL del HTML original:  |
//|  1. TP/SL basado en ATR (no fijos en USD)                        |
//|     Original: TP=ATR*5.0=250pts, SL=ATR*1.5=75pts               |
//|     G2 SL: ATR*1.0=50pts (más ajustado)                          |
//|  2. Trailing Stop IMPLEMENTADO                                    |
//|     Activa en profit>=ATR*5, trail distance=ATR*5                |
//|  3. Candle Filter OFF por default (no existe en original)         |
//|  4. LocalPA OFF por default (no existe en original)               |
//|  5. AdaptivePrevCandle OFF (no existe en original)                |
//|  6. CalcLot corregida: MathRound(bal/1000)*0.1                   |
//|     $10→0.01lot, $1000→0.10lot, $5000→0.50lot                    |
//|  HALLAZGOS del HTML original (2026.01.01-2026.02.27):            |
//|  - 9272 trades, WR=44.8%, PF=2.52, Net=$171,080                  |
//|  - Legacy 8314 trades WR=48.4%, G2 958 trades WR=12.9%           |
//|  - Win pts: 250-252 (TP hit), Loss pts: -75/-76 (SL hit)         |
//|  - G2 Loss pts: -50 (SL más ajustado), G2 Win: +276              |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "8.25"
#property strict
...
input bool   Enable_Zone_Exit_Filter     = false; // Zone-exit: K saliendo de zona (OFF=stoch puro como original)
input bool   Enable_Candle_Filter        = false; // vela confirma (OFF=no existe en original, mejora WR 44.8%)
...
input bool   Enable_EMA20_Gate           = false;  // EMA50 gate (OFF=como original)
input int    EMA20_Period                = 50;     // periodo EMA intraday
input bool   Enable_Stoch_D_Confirm     = false;  // K>D/K<D confirm (OFF=como original)
input double Min_K_Velocity             = 0.0;    // K mínimo movimiento (0.0=sin restricción, como original)
input int    G2_Legacy_Threshold        = 4;      // G2 cada N legacies cerrados (4=Overlap_AFTER_X_trades_=4)
input bool   Enable_AntiImpulse          = false;  // anti-impulso (OFF=como original)
input double AntiImpulse_ATR_Ratio       = 0.5;
input bool   Enable_CoolingOff           = false;  // pausa dirección (OFF=como original, opera sin delay)
input int    CoolingOff_Losses           = 3;
input int    CoolingOff_Bars             = 5;
// +++ v8.20 Smart Entry +++
input bool   Enable_LocalPA              = false;  // LocalPA (OFF=no existe en original)
input int    LocalPA_Lookback            = 5;
input double LocalPA_ATR_Tolerance       = 0.4;
input bool   DConfirm_P3Only             = false;  // D-confirm (OFF=no existe en original)
input double EMA_Against_KVel_Factor     = 5.0;
// +++ v8.23 Market Regime & Adaptive +++
input bool   Enable_RegimeFilter         = false;  // régimen (OFF=como original)
input int    Regime_Lookback             = 20;
input double Regime_ADX_Threshold        = 25.0;
input bool   Enable_AdaptivePrevCandle   = false;  // body ratio adaptativo (OFF=no existe en original)
input double AdaptPC_ATR_Ratio_Calm      = 0.10;
input double AdaptPC_ATR_Ratio_Normal    = 0.20;
input double AdaptPC_ATR_Ratio_Active    = 0.30;
input bool   Enable_DynamicTP            = false;  // TP dinámico (OFF=usa ATR*5.0 fijo del original)
...
int GetSignal()
{
   ...
   else // Hybrid (modo 1)
   {
      ...
      // P3: STOCH PURO + TREND — idéntico al original ─────────────────────────
      // "El Stochastic actúa como filtro de dirección, NO de frecuencia"
      // El original: K≤30 + trend confirma → BUY. K≥70 + trend confirma → SELL.
      // Sin zone-exit, sin BOS, sin D-confirm, sin RSI gate adicional.
      if(sig==0)
      {
         bool stochBuy  = (k_now <= Stoch_Buy_Level);
         bool stochSell = (k_now >= Stoch_Sell_Level);

         // Zone-exit opcional (OFF por default en v8.24)
         if(Enable_Zone_Exit_Filter)
         {
         ...
         // Aplicar filtros intraday opcionales (todos OFF por default)
...
// LOT: fórmula original: NormalizeLot(Balance/Free_margin * Lot_)
// Free_margin=1000, Lot_=0.1 → $1000→0.10lot, $5000→0.50lot
// Confirmado por análisis del HTML: día 1 $10→0.01lot, final $17100→1.71lot
double CalcLot()
{
   if(!Use_dynamic_lot_) return NormalizeDouble(Lot_,2);
   double bal=AccountInfoDouble(ACCOUNT_BALANCE);
   double lot=MathFloor(bal/Free_margin_for_each_Lots_)*Lot_;
...
void StealthCheckAll()
{
   ...
   double bid=SymbolInfoDouble(g_sym,SYMBOL_BID),ask=SymbolInfoDouble(g_sym,SYMBOL_ASK);
   double atr = GetATR();
   if(atr <= 0) atr = MM7_ATR_FALLBACK;
   double tsStart = atr * TS_Start_ATR_Multiplier;    // activar trail cuando profit >= ATR*5
   double tsDist  = atr * TS_Distance_ATR_Multiplier; // distancia del trail = ATR*5
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      if(g_sp[i].dir==1)
      {
         // Trailing Stop para BUY
         if(Enable_TrailingStop && (bid - entryPrice) >= tsStart)
         {
            double newSL = bid - tsDist;
            if(newSL > g_sp[i].vSL) g_sp[i].vSL = newSL;  // solo sube el SL
         }
         if(g_sp[i].vTP>0&&bid>=g_sp[i].vTP)cls=true;
         else if(g_sp[i].vSL>0&&bid<=g_sp[i].vSL)cls=true;
      }
      else
      {
         // Trailing Stop para SELL: precio baja = nuestro favor
         // profit = entryPrice - ask (en precio)
         if(Enable_TrailingStop && (entryPrice - ask) >= tsStart)
         {
            double newSL = ask + tsDist;  // SL virtual está POR ENCIMA del precio actual (sell)
            // Solo actualizamos si el nuevo SL es MÁS BAJO que el anterior (beneficio al vendedor)
            if(g_sp[i].vSL <= 0 || newSL < g_sp[i].vSL) g_sp[i].vSL = newSL;
         }
         if(g_sp[i].vTP>0&&ask<=g_sp[i].vTP)cls=true;
         else if(g_sp[i].vSL>0&&ask>=g_sp[i].vSL)cls=true;
      }
      if(cls&&StealthClosePos(g_sp[i].ticket,g_sp[i].dir)) g_sp[i].active=false;
   }
}

ulong OpenOrder(ENUM_ORDER_TYPE type,string comment,double slMulParam)
{
   double lot=CalcLot();
   double ask=SymbolInfoDouble(g_sym,SYMBOL_ASK),bid=SymbolInfoDouble(g_sym,SYMBOL_BID);
   double entry=(type==ORDER_TYPE_BUY)?ask:bid;
   int dir=(type==ORDER_TYPE_BUY)?1:-1;
   // ATR-based TP/SL: confirmado por análisis HTML real del original
   // TP = ATR(7) * Auto_TP_Ratio(5.0) ≈ 250 pts → ~$2.50 por 0.01 lot
   // SL Legacy = ATR(7) * Auto_SL_Ratio(1.5) ≈ 75 pts
   // SL G2     = ATR(7) * 1.0            ≈ 50 pts (más ajustado, WR 12.9%)
   double atr = GetATR();
   if(atr <= 0) atr = MM7_ATR_FALLBACK;
   double tpPts, slPts;
   bool isG2 = (slMulParam <= 1.0);  // G2 llama con Auto_SL_Ratio=1.0 aprox
   if(Use_Auto_TP) tpPts = atr * Auto_TP_Ratio;
   else            tpPts = (Take_Profit_ > 0) ? Take_Profit_*g_point : atr * 5.0;
   if(Use_Auto_SL) slPts = isG2 ? atr * 1.0 : atr * Auto_SL_Ratio;
   else            slPts = (Stop_Loss_ > 0) ? Stop_Loss_*g_point : atr * 1.5;
   double vTP=entry+dir*tpPts, vSL=entry-dir*slPts;
   MqlTradeRequest req={};MqlTradeResult res={};
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.25 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.25 | ATR-based TP/SL | TP=ATR*",Auto_TP_Ratio," SL=ATR*",Auto_SL_Ratio," G2SL=ATR*1.0 | Mode=",ms,
         " | Trailing=",Enable_TrailingStop,"(start=",TS_Start_ATR_Multiplier,"x,dist=",TS_Distance_ATR_Multiplier,"x)",
         " | ZoneExit=",Enable_Zone_Exit_Filter," | G2every=",G2_Legacy_Threshold);
...
void OnTick()
{
   ...
   {
      // Throttle: mínimo 3 segundos entre señales Legacy (evita duplicados, permite frecuencia del original)
      // El original opera ~1 trade cada 3.6 min avg pero puede ser más rápido en ráfagas
      datetime now=TimeCurrent();
