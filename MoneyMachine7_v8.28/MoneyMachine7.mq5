...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.28 ]",clrGold,       0*lh,x,y,co,fn);
...
void OnTick()
{
   ...
   {
      // BAR-LEVEL LOCK: solo 1 señal por barra M1 cerrada (bar[1])
      // El original evalúa Stoch en bar completada → mismo valor todos los ticks del bar actual
      // Sin bar-lock: misma señal repite cada tick → 802 trades/día (v8.26)
      // Con bar-lock: máx 1 señal por barra = ~8-10/hora = ~132/día (como original)
      datetime curBar1 = iTime(g_sym,_Period,1);
      if(curBar1 == g_lastSignalBar){DrawDashboard();return;}  // ya operamos en esta barra
