//+------------------------------------------------------------------+
//|  MoneyMachine7_v1787.mq5                                        |
//|  v17.87 — CIRUGÍA ANTI-HARDCAP                                  |
//|                                                                  |
//|  BASE: v17.86 ($50→$416 pico, PF=2.40, WR=62%)                 |
//|                                                                  |
//|  DIAGNÓSTICO v17.86 (48 trades reales):                         |
//|                                                                  |
//|  SIN HardCaps: WR=88.2% | CON HardCaps: WR=62.5%               |
//|  14 HardCaps = -$173 = 85% del drawdown desde el pico          |
//|                                                                  |
//|  PATRÓN 1: HCs repetidos mismo día en H15                       |
//|  03/18: 3 HCs H15 en 20min. 03/19: 2 HCs H15 simultáneos      |
//|  FIX B: Tras 1er HC en H15 → bloquear H15 ese día             |
//|                                                                  |
//|  PATRÓN 2: 2 posiciones abiertas contra tendencia               |
//|  Max_Positions=2 → 2 HCs simultáneos = -$24 en 1 tick          |
//|  FIX A: Tras 1er HC del día → Max_Positions=1 ese día          |
//|         Tras 2do HC → Daily Halt inmediato                      |
//|                                                                  |
//|  PATRÓN 3: NY Open los primeros 20min son el momento peligroso  |
//|  Todos los HCs H15 ocurren entre 15:20-15:45 UTC               |
//|  FIX C: No entrar en H15 antes de minuto :20 (esperar momentum)|
//|                                                                  |
//|  FIX D: Daily Halt $25→$20 (recuperamos en el día siguiente)   |
//|                                                                  |
//|  TODO LO DEMÁS SE MANTIENE IGUAL (funciona muy bien):           |
//|  ✓ Trail Phase1=30pts, Phase2=8pts, Phase3=4pts                |
//|  ✓ SL dinámico ATR×1.5 / TP dinámico ATR×3                     |
//|  ✓ Partial doble $6+$15                                         |
//|  ✓ Señal original intacta                                       |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.87"
#property strict
...
input double ATR_SL_Mult         = 1.5;   // SL = 1.5 × ATR14
...
input double Trail_Start_Pts     = 5.0;   // no trail antes de $5 ganado
...
input double Daily_Loss_Limit_USD  = 20.0; // FIX D: $25→$20 (recuperamos al día siguiente)

//=== FIX A: HC DIARIO TRACKER ===
// Tras 1er HC → reducir Max_Positions a 1 ese día (no doblar apuestas malas)
// Tras 2do HC → Daily Halt inmediato (el mercado está contra nosotros)
input bool   Use_HC_Daily_Tracker  = true;

//=== FIX B: BLOQUEO H15 POST-HC ===
// Si HC ocurre en H15 → bloquear H15 el resto del día
// La señal H15 ya demostró ser incorrecta ese día
input bool   Use_H15_HC_Block      = true;

//=== FIX C: NY OPEN DELAY ===
// No entrar en H15 antes del minuto :20
// Los primeros 20min de NY (15:00-15:19) son los más peligrosos
// Todos los HCs H15 ocurren en ese ventana
input bool   Use_NY_Open_Delay     = true;
input int    NY_Open_Delay_Min     = 20;   // esperar 20min tras apertura H15
...
// FIX A+B: HC intraday trackers
int      g_hc_count_today   = 0;   // HardCaps ocurridos hoy
bool     g_h15_blocked_today = false; // H15 bloqueada tras HC hoy
int      g_max_pos_effective = 2;  // Max_Positions efectivo (reduce tras HC)
...
void InitLearning()
{
   ...
   g_hc_count_today    = 0;
   g_h15_blocked_today = false;
   g_max_pos_effective = Max_Positions;
   for(int h = 0; h < 24; h++){
      g_hourStats[h].n = 0;
...
void CheckDailyHalt()
{
   if(!Use_Daily_Loss_Limit && !Use_HC_Daily_Tracker) return;
   MqlDateTime dt;
   ...
   if(g_today_date != today){
      ...
      g_today_start_bal   = AccountInfoDouble(ACCOUNT_BALANCE);
      g_daily_halted      = false;
      // FIX A+B: resetear trackers diarios
      g_hc_count_today    = 0;
      g_h15_blocked_today = false;
      g_max_pos_effective = Max_Positions;
   }
   ...
   if(Use_Daily_Loss_Limit){
      ...
      if(day_pnl < -Daily_Loss_Limit_USD){
         ...
         for(int i = 0; i < ArraySize(tks); i++)
            ClosePosition(tks[i], "DailyHalt");
         if(Use_Motor2) M2_CloseAll();
         Print("DAILY HALT: pérdida=$", DoubleToString(-day_pnl, 2),
...
//+------------------------------------------------------------------+
// FIX A + B: Registrar un HardCap y aplicar consecuencias intraday
void RegisterHardCap(int hour)
{
   if(!Use_HC_Daily_Tracker && !Use_H15_HC_Block) return;
   g_hc_count_today++;
   Print("HC #", g_hc_count_today, " del día H", hour,
         " | max_pos_eff=", g_max_pos_effective);

   // FIX A: tras 1er HC → Max_Positions efectivo = 1 (no doblar en mal día)
   if(Use_HC_Daily_Tracker && g_hc_count_today == 1){
      g_max_pos_effective = 1;
      Print("HC TRACKER: 1er HC → Max_Positions=1 para hoy");
   }
   // FIX A: tras 2do HC → Daily Halt (mercado claramente contra nosotros)
   if(Use_HC_Daily_Tracker && g_hc_count_today >= 2 && !g_daily_halted){
      g_daily_halted = true;
      g_pendingDir   = 0;
      ulong tks[]; GetTickets(tks);
      for(int i = 0; i < ArraySize(tks); i++)
         ClosePosition(tks[i], "HC2-DailyHalt");
      if(Use_Motor2) M2_CloseAll();
      Print("HC TRACKER: 2do HC → DAILY HALT");
   }
   // FIX B: si HC ocurrió en H15 → bloquear H15 el resto del día
   if(Use_H15_HC_Block && hour == 15 && !g_h15_blocked_today){
      g_h15_blocked_today = true;
      Print("H15 BLOCK: HC en H15 → H15 bloqueada el resto del día");
   }
}
...
// FIX B: Si hubo HC en H15 hoy → bloquear H15
bool IsGoodHour(int hour)
{
   if(!Use_Hour_Filter) return true;
   // Solo H02 y H15 tienen edge positivo demostrado
   if(hour != 2 && hour != 15) return false;
   // FIX B: si H15 fue bloqueada por HC hoy → no operar en H15
   if(hour == 15 && g_h15_blocked_today) return false;
...
// FIX C: Verificar si estamos dentro de la ventana segura de H15
// No entrar en H15 antes del minuto :20 (NY Open momentum)
bool IsNYOpenDelayOK(datetime bar_time)
{
   if(!Use_NY_Open_Delay) return true;
   MqlDateTime dt;
   TimeToStruct(bar_time, dt);
   if(dt.hour != 15) return true; // solo aplica a H15
   return (dt.min >= NY_Open_Delay_Min);
}
...
void UpdateDailyRegime(double profit)
{
   ...
   if(g_today_date != today){
      ...
      g_today_start_bal = AccountInfoDouble(ACCOUNT_BALANCE);
      g_daily_halted    = false;
      // FIX A+B: resetear trackers HC al nuevo día
      g_hc_count_today    = 0;
      g_h15_blocked_today = false;
      g_max_pos_effective = Max_Positions;
   }
...
int OnInit()
{
   ...
   Print("MM7 v17.86 DEFINITIVA | ", g_sym,
         " | M1:SELL H02+H15 ONLY | M2:Breakout H8+H13",
         " | SL=ATR×", ATR_SL_Mult, " TP=ATR×", ATR_TP_Mult,
         " | Trail Phase1=", Trail_Dist_Phase1, "pts",
         " | Partial $", Partial1_USD_Trigger, "+$", Partial2_USD_Trigger,
         " | DailyLimit=$", Daily_Loss_Limit_USD,
         " | Bear lot×", Bear_Lot_Scale,
         " | WR_Win=", WR_Track_Window, "b pause=", WR_Track_Pause_Bars, "b");
...
void OnTick()
{
   ...
   for(int pi = 0; pi < npos; pi++){
      ...
      if(Use_Hard_Loss_Cap && pnl < -Hard_Loss_Cap_USD){
         // Obtener hora de apertura del trade para FIX B
         int open_hour = 15; // default a H15 (hora más frecuente)
         if(g_states[si].openTime > 0){
            MqlDateTime open_dt;
            TimeToStruct(g_states[si].openTime, open_dt);
            open_hour = open_dt.hour;
         }
         ClosePosition(tk, "HardCap");
         if(Use_HardCap_Cooldown)
            g_hardcap_cooldown = HardCap_Cooldown_Bars;
         // FIX A+B: registrar HC y aplicar consecuencias intraday
         RegisterHardCap(open_hour);
   ...
   // ================================================================
   // FIX A: usar Max_Positions efectivo (se reduce tras 1er HC del día)
   if(CountByMagic() >= g_max_pos_effective){ g_pendingDir = 0; return; }
   ...
   if(curBar != g_lastBarTime){
      ...
      MqlDateTime dt; TimeToStruct(curBar, dt);
      if(dt.day_of_week == 5 && dt.hour >= FriClose_Hour_UTC) return;
      // FIX 3: IsGoodHour ya excluye H07, H22 y H15 bloqueada por HC
      if(!IsGoodHour(dt.hour)) return;
      // FIX C: NY Open Delay — no entrar en H15 antes del minuto :20
      if(!IsNYOpenDelayOK(curBar)) return;
