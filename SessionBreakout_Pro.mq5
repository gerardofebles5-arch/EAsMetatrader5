...
//|  1. London Open concentra ~38% del volumen forex global (BIS)   |
//|  2. Asian Range breakout con confirmacion de EMA + RSI          |
//|  3. ATR-based SL/TP fijos por sesion (robusto y simple)        |
//|  4. Riesgo 0.5-1% por operacion con gestion de drawdown         |
//|  5. Una operacion maxima por par por sesion (no sobretrading)   |
//+------------------------------------------------------------------+

#property copyright "SessionBreakout Pro v1.0"
...
int OnInit()
{
   ...
   Comment("SESSION BREAKOUT PRO - Inicializado correctamente");
   Print("SESSION BREAKOUT PRO v1.0 - Iniciado | Balance: ", DoubleToString(g_init_balance, 2));
...
void TryBreakout(int idx, string sym, string session)
{
   ...
   // Calcular SL y TP fijos basados en ATR
...
void ManageOpenPosition(int idx)
{
   ...
   double be_trigger = MathAbs(current_tp - open_price) * 0.5; // 50% del recorrido al TP
...
void UpdateDashboard()
{
   ...
   string dash = "\n";
   dash += "╔══════════════════════════════════════╗\n";
   dash += "║     SESSION BREAKOUT PRO v1.0        ║\n";
   dash += "╠══════════════════════════════════════╣\n";
