//+------------------------------------------------------------------+
//|  MoneyMachine7_v1777.mq5                                        |
//|  v17.76 — v17.75 + partial close en $ flotantes (no en % TP)   |
//|                                                                  |
//|  POR QUÉ FALLÓ v17.76 ANTERIOR:                                 |
//|  - Doble partial al 20%/50% del TP = 61pts/153pts de distancia  |
//|    Con SL_Min=19.2 y TP_Ratio=16 → TP=307pts inalcanzable       |
//|    Resultado: 0 partials ejecutados en toda la prueba            |
//|  - EarlyExit cortó trades que aún podían recuperar              |
//|  - Lot dinámico: sigue sin funcionar a $30 de balance           |
//|                                                                  |
//|  ÚNICO CAMBIO EN v17.76:                                        |
//|  Partial basado en GANANCIA FLOTANTE en $, no en % del TP       |
//|  Si PnL flotante >= Partial_USD_Trigger → cerrar 40%            |
//|  En v17.75: 21 de 28 winners (75%) llegaron a $8+              |
//|  → 21 partials adicionales → curva más suave sin cambiar edge   |
//|  → Net total similar, pero con 49 eventos positivos vs 28       |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.77"
#property strict
...
// PARTIAL CLOSE — basado en $ flotantes (no en % del TP)
// En v17.75: 75% de winners llegaron a $8+ → garantiza ejecución real
// Cierra 40% cuando la ganancia flotante supera el umbral en $
input bool   Use_Partial_Close   = true;
input double Partial_USD_Trigger = 8.0;  // cerrar parcial cuando PnL >= $8
...
//=== FIX 2: AUTOAPRENDIZAJE MÁS CONSERVADOR ===
// WR_Block 40%→30% y Min_Samples 5→10
// Evita bloquear horas con 5 trades malos → necesita 10 muestras sólidas
input int    Learn_Min_Samples   = 10;   // FIX: 5→10 (más evidencia para bloquear)
input double Learn_WR_Block      = 0.30; // FIX: 40%→30% (menos agresivo)
...
//=== FIX 3: PAUSA MÁS CORTA — menos tiempo bloqueado ===
...
input int    Max_Consec_Losses   = 4;    // FIX: 3→4 (más tolerante)
input int    Pause_Bars          = 3;    // FIX: 8→3 barras (pausa mínima)
...
input int    InpMagicNumber      = 177700;
input int    InpSlippagePoints   = 10;

//=== FIX 1: RÉGIMEN — solo reduce lot, NUNCA pausa completamente ===
// Deadlock anterior: día malo → pausa → no puede generar día bueno → DEADLOCK
// Solución: el régimen solo escala el lot hacia abajo, nunca bloquea
// Regime_Bad_Days y Regime_Good_Days se mantienen para el scaling pero
// la variable g_regime_paused nunca se pone en true
input int    Regime_Trend_Bars      = 60;
input double Regime_Trend_Pts       = 8.0;
input double Regime_Lot_Scale       = 0.50;  // días malos → lot×0.5
input int    Regime_Bad_Days        = 2;     // FIX: 1→2 (más tolerante)
input int    Regime_Good_Days       = 1;
input bool   Regime_Never_Pause     = true;  // FIX: nunca pausar completamente
...
void UpdateDailyRegime(double profit)
{
   ...
   if(g_today_date != today){
      ...
      if(g_today_date != 0){
         ...
         // Nivel 2: ¿pausar?
         // FIX 1: con Regime_Never_Pause=true → nunca bloquear completamente
         // Solo escalar el lot via g_regime_lot_factor
         if(!Regime_Never_Pause){
            ...
            // Nunca pausar — solo ajustar lot factor
            if(g_consec_neg_days >= Regime_Bad_Days && g_regime_lot_factor > Regime_Lot_Scale){
               g_regime_lot_factor = Regime_Lot_Scale;
               Print("RÉGIMEN: Lot reducido×",Regime_Lot_Scale," tras ",g_consec_neg_days," días malos");
            }
            if(g_consec_pos_days >= Regime_Good_Days && g_regime_lot_factor < 1.0){
               g_regime_lot_factor = 1.0;
               g_consec_neg_days = 0;
               Print("RÉGIMEN: Lot normal restaurado");
            }
            g_regime_paused = false; // garantizar que nunca esté pausado
         }
      }
      g_today_date = today;
      g_today_pnl  = 0.0;
...
int OnInit()
{
   ...
   Print("MM7 v17.77 | ",g_sym,
         " | NeverPause=",Regime_Never_Pause,
         " | AutoLearn samples=",Learn_Min_Samples," WR_Block=",Learn_WR_Block,
         " | Pausa ",Pause_Bars,"b@",Max_Consec_Losses,"losses",
...
void OnTick()
{
   ...
   for(int pi=0;pi<npos;pi++){
      ...
      // 2. PARTIAL CLOSE basado en $ flotantes — garantiza ejecución real
      // En v17.75: 75% de winners llegan a $8+ → este partial se ejecuta
      // Umbral en % del TP nunca llegaba (TP=307pts, necesitaba 61pts mínimo)
      if(Use_Partial_Close && !g_states[si].beMovedOnce && PositionSelectByTicket(tk)){
         ...
         if(pnl_float >= Partial_USD_Trigger){
            ...
            if(vc<cv){
               ...
               Print("MM7 PARTIAL pnl=",DoubleToString(pnl_float,2)," vol=",vc);
