//+------------------------------------------------------------------+
//|                     XAUUSD_V6_2B_CONFIG_C_CONSERVADOR.mq5        |
//|                  CONFIGURACIÓN C: RR 1:2 CONSERVADOR             |
//|    SL 1.8×ATR, TP 3.6×ATR - Objetivo: PF 1.8+, WR 48%+          |
//+------------------------------------------------------------------+
#property copyright "V6.2B Config C - Conservador"
...
// Risk Management - CONFIGURACIÓN C
input double SL_ATR_Multiplier = 1.8;     // CONSERVADOR
input double TP_ATR_Multiplier = 3.6;     // RR 1:2 (TP más cercano)
input int ATR_Period = 14;

// Break-Even - CONFIGURACIÓN C
input bool Use_Break_Even = true;
input double BreakEven_Start_ATR = 2.0;
input double BreakEven_Profit_ATR = 0.5;

// Trailing Stop - CONFIGURACIÓN C
...
{
    Print("========================================");
    Print("XAUUSD V6.2B - CONFIGURACIÓN C CONSERVADOR");
    Print("========================================");
    Print("RR: 1:2 (SL 1.8×ATR, TP 3.6×ATR)");
    Print("Break-Even: 2.0×ATR → +0.5×ATR");
    Print("Trailing: Start 3.0×ATR, Step 1.2×ATR");
    Print("Objetivo: PF 1.8+, WR 48%+");
    Print("========================================");
...
void CheckForEntry()
{
    ...
    if(long_signal)
    {
        Print("📈 SEÑAL LONG - Config C");
        ExecuteTrade(ORDER_TYPE_BUY, current_atr);
        g_tradesToday++;
    }
    else if(short_signal)
    {
        Print("📉 SEÑAL SHORT - Config C");
