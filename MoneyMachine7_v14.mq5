//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  v14.0 — TRAILING PROFIT SYSTEM (tiempo real)                  |
//|                                                                  |
//|  PROBLEMA DETECTADO EN LIVE:                                    |
//|  El TP fijo de 2.50pts es demasiado ambicioso en tiempo real    |
//|  → el precio llega a 1-2pts de ganancia y revierte antes de TP |
//|  → cierra en SL → pérdidas consistentes                        |
//|                                                                  |
//|  SOLUCIÓN — TRAILING PROFIT (dos modos seleccionables):        |
//|                                                                  |
//|  MODO 1: TP por USD fijo                                        |
//|    Si profit en USD >= TP_Min_USD → cierra inmediatamente       |
//|    Ej: lot=0.01, TP_Min_USD=1.0 → cierra cuando gana $1        |
//|    Simple, mecánico, predecible                                  |
//|                                                                  |
//|  MODO 2: Trailing Profit (seguimiento de máximo)                |
//|    Cuando profit >= TP_Trail_Activate_USD → activa el trailing  |
//|    Trailing sigue el máximo profit alcanzado                    |
//|    Cierra cuando profit cae TP_Trail_Retrace_USD desde el máx   |
//|    Ej: activa en $1, retroceso $0.30 → cierra en $0.70+        |
//|    Asegura ganancias y permite rides más largos                  |
//|                                                                  |
//|  Ambos modos operan en STEALTH (virtual, no enviado al broker)  |
//|  Compatible con Use_Stealth_Mode existente                      |
//|                                                                  |
//|  BASE: v12 (MODE_EMA Stochastic)                               |
//+------------------------------------------------------------------+
...
// Ejemplo con lot=0.01 XAUUSD: activar trailing en $0.50, cerrar si retrocede $0.30 desde máx
// +++ Forensic Core +++
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v14.0 ]",clrGold,       0*lh,x,y,co,fn);
