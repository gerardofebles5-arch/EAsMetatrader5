//+------------------------------------------------------------------+
//|  MoneyMachine7_v1788.mq5                                        |
//|  v17.88 — BASE v17.86 + ANTI-HC QUIRÚRGICO                     |
//|                                                                  |
//|  BASE: v17.86 ($416 pico, PF=2.40, WR=62%, SIN HCs WR=88%)    |
//|                                                                  |
//|  DIAGNÓSTICO v17.86 (48 trades reales):                         |
//|  Sin HCs: WR=88% | 14 HCs = -$173 = 85% del drawdown           |
//|  Patrón: HCs se repiten el mismo día por H15 y doble posición  |
//|  FIX A: Tras 1er HC hoy → MaxPositions=1, tras 2do → pausa 15b|
//|  FIX B: HC en H15 → bloquear H15 el resto del día             |
//|  FIX C: Daily Halt $25→$20 | FIX D: HC cooldown 5→8 barras    |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.88"
#property strict
...
int OnInit()
{
   ...
   Print("MM7 v17.88 ANTI-HC | ", g_sym,
         " | M1:SELL H02+H15 | M2:Breakout H8+H13",
         " | SL=ATR×", ATR_SL_Mult, " TP=ATR×", ATR_TP_Mult,
         " | Trail P1=", Trail_Dist_Phase1, "pts",
         " | Partial $", Partial1_USD_Trigger, "+$", Partial2_USD_Trigger,
         " | DailyLimit=$", Daily_Loss_Limit_USD,
         " | HC_Tracker=", Use_HC_Day_Tracker,
         " | H15_Block=", Use_H15_Block_After_HC,
         " | HC_Cooldown=", HardCap_Cooldown_Bars, "b");
...
void OnTick()
{
   ...
   if(CountByMagic() >= g_max_pos_today){ g_pendingDir = 0; return; }
