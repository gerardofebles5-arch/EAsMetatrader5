//+------------------------------------------------------------------+
//|  MoneyMachine7_v1780.mq5                                        |
//|  v17.80 — MOTOR DUAL: Reversión + Breakout                      |
//|                                                                  |
//|  APRENDIZAJE DE v17.79:                                         |
//|  ✓ Sin deadlock — opera todos los días                          |
//|  ✗ WR<30% pausa 3b era muy corto → Mar17-19 WR=0%, -$84        |
//|  FIX: WR<20% pausa 5b → sim $350 (+$52), pausa 6 losses        |
//|       sin tocar ningún winner                                    |
//|                                                                  |
//|  MOTOR 1 (original): Reversión a la media                       |
//|  Horas: 2, 7, 15, 22 | Magic: 177900                           |
//|  SELL en top del rango de 50 barras                             |
//|  Funciona en: mercado de RANGO                                  |
//|                                                                  |
//|  MOTOR 2 (nuevo): Breakout de apertura de sesión                |
//|  Horas: 8, 13 (Londres y NY overlap) | Magic: 178000           |
//|  BUY/SELL cuando precio rompe el rango previo + umbral          |
//|  Funciona en: mercado en TENDENCIA                              |
//|                                                                  |
//|  COMPLEMENTARIEDAD NATURAL:                                     |
//|  Motor 1 falla cuando hay tendencia (Mar17-19)                  |
//|  Motor 2 funciona exactamente cuando hay tendencia              |
//|  → Los dos motores se equilibran: cuando uno pierde el otro gana|
//|                                                                  |
//|  Motor 2 parámetros calibrados:                                 |
//|  Breakout_Bars=20, Breakout_Pts=8, SL=12, TP=20, MaxHold=90min |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.80"
#property strict

//=== SEÑAL (v17.75 — no tocar) ===
...
//=== PARTIAL CLOSE EN $ FLOTANTES (v17.76 — funciona en 75% wins) ===
input bool   Use_Partial_Close    = true;
input double Partial_USD_Trigger  = 8.0;  // cerrar si PnL_flotante >= $8
input double Partial_Close_Pct    = 0.40; // cerrar 40% de la posición

//=== HARD LOSS CAP (v17.75) ===
...
//=== AUTOAPRENDIZAJE (conservador — lección de v17.77) ===
input int    Learn_Min_Samples   = 10;   // 10 muestras antes de bloquear
input double Learn_WR_Block      = 0.30; // 30% (no 40% que bloqueaba horas buenas)
...
//=== RÉGIMEN (v17.75 — no pausa completa) ===
input int    Regime_Trend_Bars   = 60;
input double Regime_Trend_Pts    = 8.0;
input double Regime_Lot_Scale    = 0.80;  // día malo → lot×0.80 suave
input int    Regime_Bad_Days     = 1;
input int    Regime_Good_Days    = 1;

//=== SISTEMA 1: BALANCE TRAIL — desactivado (reduce ganancias igual que pérdidas) ===
...
//=== SISTEMA 2: WR TRACKER — pausa por BARRAS, no por días (sin deadlock) ===
input bool   Use_WR_Tracker      = true;
input int    WR_Track_Window     = 8;
input double WR_Track_Low        = 0.20;  // FIX: 0.30→0.20 (sim: $350 vs $268)
input double WR_Track_High       = 0.70;
input double WR_Track_Scale_Low  = 0.30;
input double WR_Track_Scale_High = 1.50;
input int    WR_Track_Pause_Bars = 5;     // FIX: 3→5 barras (pausa 6 losses, 0 wins)

//=== MOTOR 2: BREAKOUT DE APERTURA DE SESIÓN ===
// Complementario al motor 1: funciona en TENDENCIA (cuando motor 1 falla)
// Horas: H8 (apertura Londres), H13 (overlap NY-Londres)
// Señal: precio rompe el rango de las últimas N barras + umbral
// BUY si rompe HIGH + Breakout_Pts | SELL si rompe LOW - Breakout_Pts
input bool   Use_Motor2          = true;
input int    M2_Magic            = 178000; // separado del motor 1
input int    M2_Breakout_Bars    = 20;     // rango de las últimas 20 barras
input double M2_Breakout_Pts     = 8.0;   // umbral de ruptura en pts
input double M2_SL_Pts           = 12.0;  // SL en pts
input double M2_TP_Pts           = 20.0;  // TP en pts (ratio 1.67x)
input int    M2_MaxHold_Sec      = 5400;  // 90min máximo
input double M2_Lot              = 0.01;  // lot fijo independiente
...
// SISTEMA 1: BALANCE TRAILING STOP
double   g_balance_peak      = 30.0; // pico máximo del balance
double   g_bal_trail_scale   = 1.0;  // factor actual (1.0/0.5/0.25)

// SISTEMA 2: WR TRACKER
double   g_wr_history[20];   // historial de resultados recientes (1=win, 0=loss)
int      g_wr_hist_idx = 0;  // índice circular
int      g_wr_hist_n   = 0;  // cantidad de datos
double   g_wr_track_scale= 1.0; // factor actual del WR tracker

// MOTOR 2: estado de operaciones
datetime g_m2_lastBarTime   = 0;
int      g_m2_pendingDir    = 0;  // 1=BUY, -1=SELL, 0=nada
double   g_m2_pendingEntry  = 0;
double   g_m2_pendingSL     = 0;
double   g_m2_pendingTP     = 0;
datetime g_m2_openTime      = 0;
double   g_m2_openEntry     = 0;
bool     g_m2_timeExitDone  = false;
...
//+------------------------------------------------------------------+
// Lot dinámico por racha — gradual desde el primer trade
// Datos v17.73: tras 3 wins WR=73%, tras 3 losses WR=20%
...
//+------------------------------------------------------------------+
// CAMBIO 1: H19 ELIMINADA (WR=27%, EV=-$3.81 en datos v17.73)
// H7  MANTENIDA: WR=71%, EV=+$10.16
// H22 MANTENIDA: WR=62%, EV=+$7.84
...
void UpdateDailyRegime(double profit)
{
   ...
   if(g_today_date != today){
      ...
      if(g_today_date != 0){
         ...
         // SOLUCIÓN DEFINITIVA: el régimen NUNCA pausa
         // Solo ajusta g_regime_lot_factor suavemente
         // g_regime_paused = false SIEMPRE → sin deadlock
         if(g_consec_neg_days >= Regime_Bad_Days && g_regime_lot_factor > Regime_Lot_Scale){
            g_regime_lot_factor = Regime_Lot_Scale;
            Print("REGIMEN: lot×",Regime_Lot_Scale," tras ",g_consec_neg_days," días malos");
         }
         if(g_consec_pos_days >= Regime_Good_Days && g_regime_lot_factor < 1.0){
            g_regime_lot_factor = 1.0;
            g_consec_neg_days = 0;
            Print("REGIMEN: lot normal restaurado");
         }
...
// ================================================================
// SISTEMA 1: BALANCE TRAILING STOP
// Actualiza el pico del balance y calcula el factor de escala
// Llamar después de cada trade cerrado
// ================================================================
void UpdateBalanceTrail(double current_balance)
{
   if(!Use_Balance_Trail) return;
   if(current_balance > g_balance_peak){
      g_balance_peak = current_balance;
      g_bal_trail_scale = 1.0; // nuevo pico → lot normal
   }
   double dd = (g_balance_peak > 0) ?
               (g_balance_peak - current_balance) / g_balance_peak : 0;
   double prev_scale = g_bal_trail_scale;
   if(dd >= BalTrail_DD2_Pct)      g_bal_trail_scale = BalTrail_Scale2; // DD>15%
   else if(dd >= BalTrail_DD1_Pct) g_bal_trail_scale = BalTrail_Scale1; // DD>10%
   else                             g_bal_trail_scale = 1.0;
   if(g_bal_trail_scale != prev_scale)
      Print("BAL_TRAIL: bal=$",DoubleToString(current_balance,2),
            " peak=$",DoubleToString(g_balance_peak,2),
            " DD=",DoubleToString(dd*100,1),"% → lot×",g_bal_trail_scale);
}

// ================================================================
// SISTEMA 2: WR TRACKER EN TIEMPO REAL
// Circular buffer de últimas WR_Track_Window operaciones
// Más rápido que autoaprendizaje por hora — reacciona en 5 trades
// ================================================================
void UpdateWRTracker(bool is_win)
{
   ...
   if(wr_roll <= WR_Track_Low){
      ...
      // Pausa de BARRAS (no días) — se desbloquea solo → sin deadlock
...
double CalcLot(double sl_dist)
{
   ...
   // Combinar todos los factores de escala
   // g_lot_scale = racha | g_regime_lot_factor = tendencia diaria
   // g_bal_trail_scale = balance trailing | g_wr_track_scale = WR rolling
...
// ================================================================
// MOTOR 2 — BREAKOUT DE APERTURA
// Funciona en tendencia: cuando motor 1 falla, motor 2 gana
// ================================================================
...
void M2_OpenTrade(int dir, double entry, double sl, double tp)
{
   ...
   MqlTradeRequest req={}; MqlTradeResult res={};
   req.action=TRADE_ACTION_DEAL; req.symbol=g_sym; req.volume=M2_Lot;
   ...
   if(!ok){req.type_filling=ORDER_FILLING_RETURN;ok=OrderSend(req,res);}
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED){
      g_m2_openTime=TimeCurrent(); g_m2_openEntry=price; g_m2_timeExitDone=false;
      Print("M2 OPEN ",((dir==1)?"BUY":"SELL")," price=",price,
            " SL=",DoubleToString(sl,digs)," TP=",DoubleToString(tp,digs));
...
void RunMotor2(double bid, double ask, datetime now)
{
   ...
   // Time exit: si posición lleva más de MaxHold_Sec → cerrar
   ...
   if(curBar!=g_m2_lastBarTime){
      ...
      double breakH = rH + M2_Breakout_Pts; // nivel de ruptura alcista
      double breakL = rL - M2_Breakout_Pts; // nivel de ruptura bajista

      // Señal de ruptura: precio cerró fuera del rango
      if(closeNow >= breakH){
         // Ruptura alcista → BUY
         g_m2_pendingDir = 1;
   ...
   // Entrada en tick: si hay señal pendiente y precio confirma
                        ...
                        const MqlTradeRequest &req, const MqlTradeResult &res)
{
   ...
   // SISTEMA 2: actualizar WR Tracker
   UpdateWRTracker(is_win);
   // Actualizar régimen diario
   UpdateDailyRegime(profit);
   // SISTEMA 1: actualizar Balance Trailing Stop
...
int OnInit()
{
   ...
   Print("MM7 v17.80 DUAL | ",g_sym,
         " | M1:Reversion H2,7,15,22 | M2:Breakout H8,13",
         " | WR<",WR_Track_Low*100,"% pausa ",WR_Track_Pause_Bars,"b",
         " | M2 ",M2_Breakout_Pts,"pts SL=",M2_SL_Pts," TP=",M2_TP_Pts);
...
void OnTick()
{
   ...
   // MOTOR 2: corre en paralelo, independiente del motor 1
   ...
   for(int pi=0;pi<npos;pi++){
      ...
      // 2. PARTIAL CLOSE en $ flotantes — funciona en 75% de los winners
      // Trigger: PnL_flotante >= $8 (no % del TP que nunca llega)
