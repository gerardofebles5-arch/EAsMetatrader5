//+------------------------------------------------------------------+
//|                     XAUUSD_V10_2_REFINED.mq5                     |
//|          V10.2: REFINAMIENTO de V10.0 - Ajustes mínimos         |
//+------------------------------------------------------------------+
#property copyright "V10.2 - Refined"
#property version   "10.02"

input int Magic_Number = 999999;
input double Risk_Percent = 1.0;
input int Max_Trades_Per_Day = 4;  // 3→4 (más oportunidades)
...
// Risk - RR intermedio entre V10.0 y V10.1
input double SL_ATR_Multiplier = 1.5;
input double TP_ATR_Multiplier = 3.0;  // RR 1:2 (antes 1:3)
input int ATR_Period = 14;

input bool Use_Break_Even = true;
input double BreakEven_Start_ATR = 1.5;  // Más rápido (antes 2.0)
input double BreakEven_Profit_ATR = 0.4;

input bool Use_Trailing_Stop = true;
input double Trailing_Start_ATR = 2.5;  // Más rápido (antes 3.0)
...
int OnInit()
{
    Print("V10.2 - REFINED (RR 1:2)");
    Print("Ajustes mínimos sobre V10.0");
...
void CheckForEntry()
{
    ...
    if(downtrend)
    {
        ...
        if(pullback && bounce && trend_intact)
        {
            Print("SHORT: Downtrend + Pullback");
...
void ExecuteTrade(ENUM_ORDER_TYPE orderType, double atr_value)
{
    ...
    {
        g_tradesExecuted++;
        Print("Trade #", g_tradesExecuted, " RR 1:2");
    }
}
...
void OnDeinit(const int reason)
{
    ...
    Print("V10.2 FINALIZADO - Trades: ", g_tradesExecuted);
}
//+------------------------------------------------------------------+
