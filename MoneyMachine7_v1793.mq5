//+------------------------------------------------------------------+
//|  MoneyMachine7_v1793.mq5                                        |
//|  v17.93 — ELIMINAR SEÑALES FALSAS                  |
//|                                                                  |
//|  BASE: v17.91 (WR=59%, PF=2.36, +$290, R:R=1.64)              |
//|  DIAGNÓSTICO: 44 trades reales analizados                      |
//|                                                                  |
//|  BASE: v17.92 (WR=57%, PF=2.56, R:R=1.92, profit=$295)        |
//|  ANÁLISIS: 42 trades, 73% días positivos                       |
//|                                                                  |
//|  CONFIRMADO DE v17.92 (mantener exactamente igual):            |
//|  ✓ SL_Caps: 16→1 (ATR×0.8 + SL_Min=10 funciona perfecto)     |
//|  ✓ R:R: 0.81→1.64→1.92 (mejora continua, dirección correcta) |
//|  ✓ avg_win: $13.89→$19.34→$20.18 (+45% vs v17.90)            |
//|  ✓ PF=2.56 (mejor en toda la historia del EA)                 |
//|  ✓ Trail_Start=$13, Phase1=30pts, ATR_TP×3, SL_Cap=$12        |
//|                                                                  |
//|  PROBLEMAS IDENTIFICADOS CON DATOS REALES:                     |
//|                                                                  |
//|  FIX 1 — SEÑALES FALSAS <5min (WR=30%, -$74 de -$10 c/u):    |
//|  10 trades cierran en <5min: 7 pérdidas (-$70), 3 wins (+$91) |
//|  Los losers de <5min: precio sube 10pts INMEDIATAMENTE         |
//|  Causa: la señal SELL entra cuando el momentum ya es alcista   |
//|  FIX: verificar que las últimas 3 velas NO sean ascendentes    |
//|  Si close[1]>close[2] Y close[2]>close[3] → no entrar         |
//|  (3 velas consecutivas al alza = momentum contrario activo)    |
//|                                                                  |
//|  FIX 2 — MARTES BLOQUEAR (WR=33% en todas las versiones):     |
//|  Martes v17.90: WR=78%+$35 | v17.91: WR=43%+$8 | v17.92=33%+$2|
//|  Tendencia decreciente. El patrón es inconsistente.            |
//|  Añadir Martes al bloqueo DoW (mask 119→115)                  |
//|  Simulación: -$2 impacto mínimo pero reduce varianza           |
//|                                                                  |
//|  FIX 3 — TRAIL PHASE2: 8→12pts                                |
//|  13 winners por trailing avg $11.63 vs 11 TPs avg $30.29      |
//|  Phase2=8pts después de 10min es demasiado ajustado            |
//|  Con 12pts los trades H15 tienen más espacio para desarrollarse|
//|                                                                  |
//|  FIX 4 — PARTIAL CLOSE: corregir bug definitivamente           |
//|  0 partials en 42 trades - nunca se activa                     |
//|  El pnl flotante de SELL = POSITION_PROFIT es CORRECTO en MT5  |
//|  Pero en backtester el partial puede fallar por timing          |
//|  FIX: añadir verificación de volumen mínimo antes de intentar  |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.93"
#property strict
...
input double Trail_Dist_Phase2   = 12.0;  // FIX 3: 8→12pts (más espacio en Phase2 para H15)
...
// FIX 1: Filtro de momentum — no SELL si últimas N velas son ascendentes
// Datos: WR=30% en trades <5min porque el precio ya sube cuando entramos
// Si 3 velas consecutivas cierran al alza = momentum alcista activo = mala señal SELL
input bool   Use_Momentum_Filter   = true;
input int    Momentum_Bars         = 3;    // velas consecutivas al alza para bloquear
...
input int    DayOfWeek_Mask        = 115; // FIX 2: 119→115 (bloquear Martes=bit2 Y Miércoles=bit3)
...
int OnInit()
{
   ...
   Print("MM7 v17.93 | ", g_sym,
         " | H02+H07+H15(noH22) | Mié BLOQUEADA",
         " | SL=ATR×", ATR_SL_Mult, " min=", SL_Min_Static, " Cap=$", SL_USD_Cap_Value,
         " | TP=ATR×", ATR_TP_Mult, " Trail=$", Trail_Start_Pts, "→P1=", Trail_Dist_Phase1,
         " | DoW Mon/Thu=×", DoW_Scale_Mon, " Mié=×", DoW_Scale_Wed,
...
void OnTick()
{
   ...
   for(int pi = 0; pi < npos; pi++){
      ...
      // 2. PARTIAL CLOSE DOBLE — FIX 4: verificar volumen mínimo
      // El partial no puede ejecutarse si el lote es exactamente el mínimo
      // Verificar que vol > 2×min_lot antes de intentar el cierre parcial
      double cur_vol = PositionSelectByTicket(tk) ? PositionGetDouble(POSITION_VOLUME) : 0;
      double min_lot = SymbolInfoDouble(g_sym, SYMBOL_VOLUME_MIN);
      bool can_partial = (cur_vol > min_lot * 2.0);
      if(Use_Partial_Close && !g_states[si].partial1Done &&
         pnl >= Partial1_USD_Trigger && can_partial){
   ...
   if(curBar != g_lastBarTime){
      ...
      // FIX 1: Filtro momentum — no entrar SELL si precio tiene impulso alcista
      // WR=30% en trades <5min porque el precio ya sube cuando se genera la señal
      // 3 velas consecutivas al alza = momentum alcista fuerte = señal SELL falsa
      if(Use_Momentum_Filter){
         bool all_rising = true;
         for(int mb = 1; mb <= Momentum_Bars; mb++){
            double c_curr = iClose(g_sym, _Period, mb);
            double c_prev = iClose(g_sym, _Period, mb + 1);
            if(c_curr <= 0 || c_prev <= 0 || c_curr <= c_prev){
               all_rising = false;
               break;
            }
         }
         if(all_rising) return; // momentum alcista activo → no SELL
      }
