...
int GetSignal()
{
   ...
   else // Hybrid (modo 1)
   {
      ...
      // P3: STOCH PURO + TREND + K CROSSING FILTER ──────────────────────
      // FORENSE LOG ORIGINAL (264 trades reales):
      //   WR Legacy puro = 54.1% | 21.3% aperturas en mismo minuto M1
      //   → confirma kb[0] (tiempo real) con throttle 3s
      //   Problema v8.30: 663/día vs 132 = K oscila <30 toda la barra
      //   → throttle 3s abre en cada sub-ciclo dentro de la barra
      //
      // SOLUCIÓN: K CROSSING FILTER
      //   Solo señal cuando K CRUZA hacia <30 (no cuando YA está <30)
      //   k_now <= 30 AND k_prev > 30 → primer momento del oversold
      //   Elimina re-entradas repetidas dentro del mismo ciclo Stoch
      //   Log: 13-17 barras M1 distintas/hora → 1 cruce por ciclo ✓
      if(sig==0)
      {
         // Crossing: k_now acaba de entrar en zona (k_prev estaba fuera)
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.33 ]",clrGold,       0*lh,x,y,co,fn);
