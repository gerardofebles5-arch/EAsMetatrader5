...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.4 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.4 | LIVE PARITY (backtest identical) | Mode=",ms,
...
void OnTick()
{
   ...
   if(!g_warmupDone)
   {
      ...
      else
      {
         ...
         {
            g_warmupDone = true;
            Print("MM7 v16.4 | WARMUP LIVE COMPLETADO tras ", secs, "s | M5+M15 barras nuevas | Senales activadas");
         }
         else
         {
            if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
            {
               Comment("MM7 v16.4 | WARMUP LIVE: M5:", (m5_new?"OK":"wait"), " M15:", (m15_new?"OK":"wait"), " (", secs, "s)");
               g_lastDashTime = TimeCurrent();
            }
            return;
         }
