//+------------------------------------------------------------------+
//|  MoneyMachine7_v1782.mq5                                        |
//|  v17.82 — BASE v17.80 + MEJORAS QUIRÚRGICAS                     |
//|                                                                  |
//|  AUTOPSIA DE v17.81 (CATÁSTROFE -94.7%):                        |
//|  ✗ Motor 3 BUY operaba en las MISMAS horas que Motor 1 SELL     |
//|    → posiciones contradictorias simultáneas, se destruían solas  |
//|  ✗ TrendBlocksBuy usaba 100 barras M1 = solo ~1.6h de datos     |
//|    → nunca detectó la caída real de -400pts                      |
//|  ✗ SL_Min 19.2→14 hizo los SL demasiado cortos para XAUUSD M1  |
//|    → stops saltaban en ruido normal, WR 36%                      |
//|  ✗ Motor 2 lot dinámico sin historial suficiente = sobreoperar   |
//|  CONCLUSIÓN: el Motor 1 estaba BIEN. No tocar su señal.         |
//|                                                                  |
//|  APRENDIZAJE DE v17.80 (datos reales +$270, Factor Beneficio=2.06):|
//|  ✓ H2 WR~60%, H7 WR=71%, H15 WR~55%, H22 WR=62% → mantener    |
//|  ✓ Señal de reversión funciona en rango → NO tocar              |
//|  ✗ Mar17-19: -$84 en 3 días por tendencia bajista prolongada     |
//|    Motor 1 seguía operando contra tendencia fuerte               |
//|  ✗ Partial close a $8 flotantes → muchos trades cerraban sin llegar|
//|  ✗ Motor 2 con lot fijo 0.01 no escalaba con el balance          |
//|  ✗ WR bajó a 0% en Mar17-19 pero pausa 5 barras fue insuficiente |
//|                                                                  |
//|  MEJORAS v17.82 (solo sobre lo que SABEMOS que falla):          |
//|  FIX 1: TENDENCIA DIARIA — detectar caída sostenida y reducir   |
//|          lot Motor 1 agresivamente (no pausar — sin deadlock)    |
//|          Si precio cae >X pts en Y barras → lot×0.3 en M1       |
//|  FIX 2: WR TRACKER más reactivo: ventana 5 barras (antes 8)     |
//|          Pausa 8 barras cuando WR<=20% (antes 5)                 |
//|  FIX 3: Partial close $8→$6 asegurar antes en winners           |
//|  FIX 4: Motor 2 lot dinámico SOLO cuando balance>$80 (seguro)   |
//|          Si balance<=$80 usar lot 0.01 fijo (no sobreoperar)     |
//|  FIX 5: COOLDOWN POST-LOSS: tras Hard Cap, esperar 3 barras M1  |
//|  FIX 6: Trail Phase1 15→13 puntos (retener más en primeros 10min)|
//|  FIX 7: Tendencia detectada por ADX simplificado (3 EMAs)        |
//|          Si EMA5>EMA20>EMA60 bajista → reducir lote M1           |
//|                                                                  |
//|  LO QUE NO SE TOCA (funciona bien en v17.80):                   |
//|  ✓ Señal M1: Range=50, Zone=0.555, Confirm=4.6 — no tocar       |
//|  ✓ SL_Min=19.2, SL_Max=85.2 — calibrado para XAUUSD M1          |
//|  ✓ TP_Ratio=16 — aunque alto, el trailing es quien cierra        |
//|  ✓ Horas: 2,7,15,22 — todas con WR>50% histórico                |
//|  ✓ Motor 2: Breakout H8,13 — complementario y funcionando        |
//|  ✓ Autoaprendizaje por hora — bloquea horas malas                |
//|  ✓ WR<20% pausa 5b — buena base, solo refinar                   |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.82"
#property strict

//=== SEÑAL MOTOR 1 (v17.75 — NO TOCAR, calibrado) ===
...
input double SL_Min              = 19.2;   // NO TOCAR — calibrado para XAUUSD M1
input double SL_Max              = 85.2;
input double TP_Ratio            = 16.0;   // NO TOCAR — el trailing cierra, no el TP

//=== TRAILING ESCALONADO ===
input bool   Use_Trail           = true;
input double Trail_Start_Pts     = 2.9;
input double Trail_Dist_Phase1   = 13.0;  // FIX: 15→13 retener más en primeros 10min
...
input double Partial_USD_Trigger  = 6.0;   // FIX: 8→6 asegurar antes
...
//=== WR TRACKER (FIX: más reactivo) ===
input bool   Use_WR_Tracker      = true;
input int    WR_Track_Window     = 5;     // FIX: 8→5 reacciona en 5 trades (antes 8)
...
input int    WR_Track_Pause_Bars = 8;    // FIX: 5→8 pausa más larga cuando WR cae

//=== FIX 1: DETECTOR DE TENDENCIA BAJISTA SOSTENIDA ===
// Si el precio lleva bajando X puntos en Y barras → reducir lot Motor 1
// Esto es lo que falló Mar17-19: precio cayó -400pts y M1 seguía vendiendo
// en rango (correcto) pero el mercado ya no era de rango
input bool   Use_Bear_Protect    = true;
input int    Bear_Bars           = 180;   // 3h en M1 = ventana suficiente
input double Bear_Drop_Pts       = 60.0; // si cae >60pts en 3h → modo defensivo
input double Bear_Lot_Scale      = 0.30; // lot Motor 1 × 0.30 en tendencia bajista
input int    Bear_EMA_Fast       = 5;    // EMA rápida para confirmar dirección
input int    Bear_EMA_Slow       = 20;   // EMA lenta

//=== FIX 5: COOLDOWN POST-HARD-CAP ===
// Tras un HardCap, esperar N barras antes de volver a operar
// Evita entrar inmediatamente después de una pérdida por tendencia
input bool   Use_HardCap_Cooldown = true;
input int    HardCap_Cooldown_Bars = 3;

//=== MOTOR 2: BREAKOUT (igual que v17.80) ===
...
// FIX 4: lot dinámico M2 solo si balance > umbral
input double M2_Lot_Fixed        = 0.01;     // usado cuando balance <= umbral
input double M2_Risk_Pct         = 0.015;    // usado cuando balance > umbral
input double M2_DynLot_MinBal    = 80.0;     // umbral: solo dinámico si bal>$80
...
int    g_totalTrades   = 0;
int    g_hardcap_cooldown = 0; // FIX 5: barras de cooldown tras HardCap
...
// DETECTOR BAJISTA (FIX 1)
double   g_bear_lot_factor = 1.0; // 1.0=normal, 0.30=tendencia bajista detectada
...
//+------------------------------------------------------------------+
// FIX 1: Detector de tendencia bajista sostenida
// Compara precio hace Bear_Bars barras con precio actual
// Si caída > Bear_Drop_Pts Y EMA_fast < EMA_slow → mercado bajista
// En ese caso reducir lot Motor 1 a Bear_Lot_Scale (0.30)
void UpdateBearDetector()
{
   ...
   double drop = price_ago - price_now; // positivo si bajó

   // Calcular EMA simple para confirmar dirección
   // EMA_fast
   double ema_fast = 0, ema_slow = 0;
   for(int i=Bear_EMA_Fast; i>=1; i--)
      ema_fast += iClose(g_sym,_Period,i);
   ema_fast /= Bear_EMA_Fast;
   for(int i=Bear_EMA_Slow; i>=1; i--)
      ema_slow += iClose(g_sym,_Period,i);
   ema_slow /= Bear_EMA_Slow;

   double prev_factor = g_bear_lot_factor;

   if(drop >= Bear_Drop_Pts && ema_fast < ema_slow){
      // Tendencia bajista confirmada → motor 1 (SELL en rango) ya no está
      // en contra de la tendencia, pero el mercado puede no ser de rango
      // → reducir exposición a 30%
      if(g_bear_lot_factor != Bear_Lot_Scale){
         g_bear_lot_factor = Bear_Lot_Scale;
         Print("BEAR PROTECT: caída=",DoubleToString(drop,1),"pts en ",Bear_Bars,"b",
               " EMA",Bear_EMA_Fast,"=",DoubleToString(ema_fast,2),
               " < EMA",Bear_EMA_Slow,"=",DoubleToString(ema_slow,2),
               " → lot×",Bear_Lot_Scale);
      }
   } else {
      if(g_bear_lot_factor != 1.0){
         g_bear_lot_factor = 1.0;
         Print("BEAR PROTECT: desactivado — mercado normalizado");
      }
   }
}

//+------------------------------------------------------------------+
void RecordTrade(int hour, double sl_dist, double profit, double hold_sec, bool from_hardcap)
{
   if(g_histCount >= 500) {
   ...
   // FIX 5: Si fue HardCap → activar cooldown
   if(from_hardcap && Use_HardCap_Cooldown){
      if(g_hardcap_cooldown < HardCap_Cooldown_Bars){
         g_hardcap_cooldown = HardCap_Cooldown_Bars;
         Print("COOLDOWN: ",HardCap_Cooldown_Bars," barras tras HardCap");
      }
   }
   ...
   if(price_now > 0 && price_ago > 0){
      ...
      if(move > Regime_Trend_Pts){
         if(g_regime_lot_factor != Regime_Lot_Scale){
            ...
            Print("RÉGIMEN ALCISTA detectado → lot×",Regime_Lot_Scale);
         }
      } else {
...
void UpdateDailyRegime(double profit)
{
   ...
   if(g_today_date != today){
      if(g_today_date != 0){
         ...
         g_regime_paused = false; // NUNCA pausar por régimen
      }
...
double CalcLot(double sl_dist)
{
   ...
   // FIX 1: incorporar factor bajista al cálculo de lot
...
// FIX 4: Lot Motor 2 — dinámico solo si balance >= umbral
double CalcLotM2(double sl_dist)
{
   double bal = MathMin(AccountInfoDouble(ACCOUNT_BALANCE), AccountInfoDouble(ACCOUNT_EQUITY));
   if(bal <= M2_DynLot_MinBal){
      // balance bajo → lot fijo para no sobreoperar
      return M2_Lot_Fixed;
   }
   double cap_usd = bal * Cap_Pct_Of_Balance;
...
void OpenSell(double sl_dist)
{
   ...
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED){
      ...
      Print("MM7 SELL bid=",bid," SL=",sl," TP=",tp," lot=",lot,
            " bear_factor=",DoubleToString(g_bear_lot_factor,2),
...
// ================================================================
// MOTOR 2 — BREAKOUT (igual que v17.80 con FIX 4 en lot)
// ================================================================
...
void M2_OpenTrade(int dir, double entry, double sl, double tp)
{
   ...
   double lot = CalcLotM2(sl_dist); // FIX 4: lot dinámico solo si bal>$80
   MqlTradeRequest req={}; MqlTradeResult res={};
   req.action=TRADE_ACTION_DEAL; req.symbol=g_sym; req.volume=lot;
                        ...
                        const MqlTradeRequest &req, const MqlTradeResult &res)
{
   ...
   bool was_hardcap = (StringFind(comment,"HardCap")>=0 || StringFind(comment,"hardcap")>=0);
...
int OnInit()
{
   ...
   Print("MM7 v17.82 | ",g_sym,
         " | M1:SELL H2,7,15,22 | M2:Breakout H8,13",
         " | WR<",WR_Track_Low*100,"% pausa ",WR_Track_Pause_Bars,"b",
         " | BearProtect=",Use_Bear_Protect," drop>",Bear_Drop_Pts,"pts/",Bear_Bars,"b→lot×",Bear_Lot_Scale,
         " | Partial@$",Partial_USD_Trigger,
         " | M2 MinBal=$",M2_DynLot_MinBal);
...
void OnTick()
{
   ...
   for(int pi=0;pi<npos;pi++){
      ...
      if(Use_Hard_Loss_Cap && PositionSelectByTicket(tk)){
         ...
         if(pnl < -Hard_Loss_Cap_USD){
            ...
            // FIX 5: cooldown inmediato
            if(Use_HardCap_Cooldown) g_hardcap_cooldown = HardCap_Cooldown_Bars;
            continue;
         }
      }

      // 2. PARTIAL CLOSE (FIX 3: trigger $6 en lugar de $8)
   ...
   // FIX 5: no operar durante cooldown o pausa
