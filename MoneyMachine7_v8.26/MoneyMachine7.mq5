//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.26 — DIAGNÓSTICO DEFINITIVO                |
//|                                                                  |
//|  APRENDIDO DE v8.24/v8.25:                                      |
//|  ✗ v8.24: Candle+LocalPA+AdaptivePrevCandle → WR=28.76%         |
//|     Estos 3 filtros INVIERTEN calidad de señal (peor que nada)  |
//|  ✗ v8.25: ZoneFreshEntry → WR=27.03% (bloqueó buenas señales)   |
//|     FVG/Sweep sin filtrar → más Legacy, más G2 explosivo        |
//|                                                                  |
//|  DATOS REALES DEL ORIGINAL (log 264 trades):                    |
//|  ✓ WR=50.76% con CERO filtros extra                             |
//|  ✓ Solo: Stoch K≤30/K≥70 + MA200 + 1 posición a la vez         |
//|  ✓ TP=2.50pts, SL=0.77pts son CORRECTOS                         |
//|  ✓ Re-abre INMEDIATAMENTE tras close (sin throttle mínimo)      |
//|  ✓ WR 50% = matemática pura: R:R=3.1:1 con señal de momentum   |
//|  ✓ Trades/día natural: ~132 (controlado por duración ATR-based) |
//|                                                                  |
//|  v8.26 CAMBIOS vs v8.24:                                        |
//|  1. ALL custom filters → OFF by default                         |
//|     (Candle=F, LocalPA=F, AdaptivePrevCandle=F, ZoneExit=F)     |
//|  2. P3 usa IntraFiltersPass() puro (no _P3 con D-confirm)       |
//|  3. Min_K_Velocity = 0.0 (desactivado)                          |
//|  4. G2_Legacy_Threshold = 4 (de Overlap_AFTER_X_trades_=4)      |
//|  5. Lot fix: floor(bal/1000)*Lot_ (no MathRound)                |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "8.26"
#property strict
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.26 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.26 | PURE-SIGNAL | Mode=",ms,
         " | Candle=",Enable_Candle_Filter," LocalPA=",Enable_LocalPA,
         " AdaptPC=",Enable_AdaptivePrevCandle," ZoneExit=",Enable_Zone_Exit_Filter,
