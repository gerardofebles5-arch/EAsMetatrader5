//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.21                                         |
//|  MEJORAS vs v8.18:                                               |
//|  1. EMA50 M1 (era EMA20: más estable, menos sobre-filtro)        |
//|  cae dentro de zona. Ahora requiere K cruzar DE VUELTA hacia     |
//|  zona neutra (señal de reversión real, no zona activa)           |
//|  4. K_Velocity→0.2 (era 0.5: recupera señales graduales válidas) |
//|  CAMBIOS vs v8.15:                                               |
...
input int    SwingLookback               = 8;     // barras para detectar swing H/L (era hardcoded 20)
input double Sweep_ATR_Margin            = 0.6;   // margen ATR para confirmar sweep (era 0.5)
input bool   Enable_P3_PureStoch         = false; // P3 stoch puro (OFF=solo Sweep+FVG, mayor WR)
input bool   Enable_SignalTag            = true;  // tag en comentario [Sw][FV][P3] para diagnóstico
// Trade Filters
...
input double Min_K_Velocity             = 0.1;    // K mínimo movimiento entre barras (0.1=cualquier movimiento)
input int    G2_Legacy_Threshold        = 8;      // G2 cada N legacies cerrados (8=~19 G2 en sesión)
...
input bool   Enable_CoolingOff           = true;   // pausa dirección tras N pérdidas consecutivas
input int    CoolingOff_Losses           = 3;      // pérdidas consecutivas antes de pausa
input int    CoolingOff_Bars             = 3;      // barras de pausa por dirección
// +++ v8.20 Smart Entry +++
input bool   Enable_LocalPA              = true;   // Local Price Action: precio en extremo local (OB touch)
input int    LocalPA_Lookback            = 5;      // barras hacia atrás para detectar extremo local
input double LocalPA_ATR_Tolerance       = 0.4;    // precio dentro de ATR*ratio del extremo (0.4=permisivo)
input bool   DConfirm_P3Only             = true;   // Stoch D confirmación solo en P3 (no en Sweep/FVG)
input double EMA_Against_KVel_Factor     = 5.0;    // factor K_velocity si señal va contra EMA (5x más exigente)
...
int      g_lastSigType   = 0; // 1=Sweep, 2=FVG, 3=P3 (para diagnóstico)
...
void UpdateSwings()
{
   ...
   int sw_n=MathMax(3,SwingLookback);
   for(int i=1;i<=sw_n;i++){double h=iHigh(g_sym,_Period,i),l=iLow(g_sym,_Period,i);if(h>hi)hi=h;if(l<lo)lo=l;}
...
int GetSweep()
{
   ...
   UpdateSwings();
   double atr=GetATR(), sw=atr*Sweep_ATR_Margin;
   double bid=SymbolInfoDouble(g_sym,SYMBOL_BID), ask=SymbolInfoDouble(g_sym,SYMBOL_ASK);

   // Leer K actual y anterior para zone-exit dentro del sweep
   double kb[]; ArraySetAsSeries(kb,true);
   if(CopyBuffer(g_hStoch,0,1,3,kb)<3) return 0;
   double k_now=kb[0], k_prev=kb[1];

   // BUY sweep: precio cerca del mínimo del swing + K en zona + K subiendo (reversión)
   if(bid>=g_swingLo-sw && ask<=g_swingLo+sw && kb[0]<=Stoch_Buy_Level)
   {
      // Confirmar zone-exit si está activado
      if(Enable_Zone_Exit_Filter && k_now<=k_prev) return 0; // K aún bajando, esperar
      return 1;
   }
   // SELL sweep: precio cerca del máximo del swing + K en zona + K bajando (reversión)
   if(bid>=g_swingHi-sw && bid<=g_swingHi+sw && kb[0]>=Stoch_Sell_Level)
   {
      if(Enable_Zone_Exit_Filter && k_now>=k_prev) return 0; // K aún subiendo, esperar
      return -1;
   }
   return 0;
}
...
// IntraFiltersPass: versión base (Sweep y FVG — menos restrictivo)
// IntraFiltersPass: filtros para Sweep y FVG (mínimos, no bloquear señales buenas)
bool IntraFiltersPass(int sig, double k_now, double k_prev)
{
   // 1. K velocity mínima (0.0 = desactivado)
   if(Min_K_Velocity > 0)
   {
      double velocity = (sig==1) ? (k_now-k_prev) : (k_prev-k_now);
      if(velocity < Min_K_Velocity) return false;
   }
   // 2. Anti-impulso (OFF por defecto en v8.21)
   if(!AntiImpulseOK(sig)) return false;
   // 3. Cooling-off direccional
...
int GetSignal()
{
   ...
   g_lastSigType=0; // 1=Sweep, 2=FVG, 3=P3
   ...
   {
      // ── P1: SWEEP (señal de mayor calidad — WR estimada 45-55%) ──────────
      // Precio toca extremo del swing local + K en zona + K revirtiendo
      // SwingLookback=8 → detecta OBs de los últimos 8 minutos
      if(Enable_Stop_Hunt_Strategy)
      {
         int s=GetSweep();
         if(s!=0&&(trend==0||s==trend)&&IntraFiltersPass(s,k_now,k_prev))
         { sig=s; fromSweep=true; g_lastSigType=1; }
      }

      // ── P2: FVG FILL (señal estructural — WR estimada 40-45%) ────────────
      // Precio entrando en un Fair Value Gap previamente detectado
      if(sig==0&&Enable_FVG_Strategy)
      {
         // FVG: stoch en zona o cerca (±15) — señal estructural, menos exigente
         bool stOKbuy  = (k_now<=Stoch_Buy_Level+15);
         bool stOKsell = (k_now>=Stoch_Sell_Level-15);
         if(InFVG(1) &&stOKbuy &&(trend==0||trend== 1)&&IntraFiltersPass(1,k_now,k_prev)){sig= 1;fromFVG=true;g_lastSigType=2;}
         if(InFVG(-1)&&stOKsell&&(trend==0||trend==-1)&&IntraFiltersPass(-1,k_now,k_prev)){sig=-1;fromFVG=true;g_lastSigType=2;}
      }

      // ── P3: STOCH PURO (señal base — WR ~32%) ────────────────────────────
      // Solo activo si Enable_P3_PureStoch=true (OFF por defecto en v8.21)
      // Responsable histórico de la mayoría de las pérdidas consecutivas
      if(sig==0&&Enable_P3_PureStoch)
      {
         int sz=GetStochZone(k_now,k_prev);
         if(sz==0) return 0;

         // P3 requiere filtros estrictos: D-confirm + LocalPA + CoolingOff
         if(!IntraFiltersPass_P3(sz,k_now,k_prev)) return 0;
         ...
         if(sig==0)
         {
            if(qualBuy &&(trend==0||trend== 1)&&HasBOS(1))  {sig= 1;g_lastSigType=3;}
            if(qualSell&&(trend==0||trend==-1)&&HasBOS(-1)) {sig=-1;g_lastSigType=3;}
         }
      }
   }
...
void DrawDashboard()
{
   ...
   DashLbl("7","Sig:"+sig_type_str+" EMA:"+ema_str+" ATR:"+DoubleToString(GetATR(),3),clrGray,7*lh,x,y,co,fn);
   string cool_str="";
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.21 | Mode=",ms," SwLB=",SwingLookback," SwATR=",Sweep_ATR_Margin," P3=",Enable_P3_PureStoch," | EMA50=",Enable_EMA20_Gate,"(p=",EMA20_Period,") | KVel=",Min_K_Velocity,
         " | AntiImp=",Enable_AntiImpulse,"(",AntiImpulse_ATR_Ratio,") | Cool=",Enable_CoolingOff,"(",CoolingOff_Losses,"L/",CoolingOff_Bars,"bar) | G2every=",G2_Legacy_Threshold,
...
void OnTick()
{
   ...
   if(CountLegacyOpen()==0)
   {
      ...
      string sigTag=Enable_SignalTag?((g_lastSigType==1)?"[Sw]":(g_lastSigType==2)?"[FV]":"[P3]"):"";
      if(sig==1)  {ulong tk=OpenOrder(ORDER_TYPE_BUY, CommentOrder+" [Legacy]"+sigTag,Auto_SL_Ratio);if(tk>0){g_lastSignalBarTime=bOpen;g_lastLegacyTicket=tk;}}
      if(sig==-1) {ulong tk=OpenOrder(ORDER_TYPE_SELL,CommentOrder+" [Legacy]"+sigTag,Auto_SL_Ratio);if(tk>0){g_lastSignalBarTime=bOpen;g_lastLegacyTicket=tk;}}
   }
   DrawDashboard();
}
