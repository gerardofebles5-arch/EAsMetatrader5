...
ulong OpenOrder(ENUM_ORDER_TYPE type,string comment,double slMul)
{
   ...
   double vTP_ref = (type==ORDER_TYPE_BUY) ? bid : ask; // referencia BID-based
   double vTP = vTP_ref + dir*MM7_TP_FIXED;
   double vSL = vTP_ref - dir*slPts;
   MqlTradeRequest req={};MqlTradeResult res={};
   ...
   {
      ulong posTk=0;
      // v16.5 LIVE FIX: en live MT5 actualiza historial de forma asincrona.
      // HistoryDealSelect puede fallar en el mismo tick aunque el deal ya existe.
      // Solucion: en live esperar hasta 3 intentos con 50ms entre ellos.
      // En backtest: MQL_TESTER → sin Sleep (prohibido en tester), 1 intento directo.
      bool dealFound = false;
      if(res.deal > 0)
      {
         int maxTries = (bool)MQLInfoInteger(MQL_TESTER) ? 1 : 3;
         for(int attempt=0; attempt<maxTries && !dealFound; attempt++)
         {
            if(attempt > 0) Sleep(50); // solo en live, entre reintentos
            if(HistoryDealSelect(res.deal))
            {
               ulong pid = (ulong)HistoryDealGetInteger(res.deal, DEAL_POSITION_ID);
               if(pid > 0) { posTk = pid; dealFound = true; }
            }
         }
         if(!dealFound) posTk = res.deal; // fallback: usar deal como ticket
      }
      // Fallback final: buscar la posicion abierta mas reciente con nuestro magic
      if(posTk==0||!PositionSelectByTicket(posTk))
      {
         ...
         for(int p=PositionsTotal()-1;p>=0;p--)
         {
            ...
            if(PositionGetString(POSITION_SYMBOL)!=g_sym||PositionGetInteger(POSITION_MAGIC)!=g_magic) continue;
            // v16.5: verificar que el comentario coincida para evitar confundir Legacy con G2
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v16.5 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v16.5 | LIVE PARITY (backtest identical) | Mode=",ms,
...
void OnTick()
{
   ...
   if(!g_warmupDone)
   {
      ...
      else
      {
         ...
         {
            g_warmupDone = true;
            Print("MM7 v16.5 | WARMUP LIVE COMPLETADO tras ", secs, "s | M5+M15 barras nuevas | Senales activadas");
         }
         else
         {
            if(TimeCurrent() - g_lastDashTime >= Refresh_Interval_Seconds)
            {
               Comment("MM7 v16.5 | WARMUP LIVE: M5:", (m5_new?"OK":"wait"), " M15:", (m15_new?"OK":"wait"), " (", secs, "s)");
               g_lastDashTime = TimeCurrent();
            }
            return;
         }
