//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.14 — Calidad de señal mejorada             |
//|                                                                  |
//|  FIXES vs v8.13 (basado en análisis de 386 trades):             |
//|  1. MinBarsBetweenTrades: cooldown entre trades (reduce overtrading) |
//|  2. Quality Gate: requiere 2 de 3 confirmadores simultáneos      |
//|     (Stoch + RSI + Volume) antes de entrar                       |
//|  3. Candle Body Filter: entrada solo en dirección del cuerpo     |
//|     de la vela actual (elimina señales contra momentum)          |
//|  4. G2 tighter: G2_Min_Gap ahora 5 barras mínimo post-cierre    |
//|     (reduce G2 de ~94 minutos dobles a ~19 objetivo)             |
//|  5. Bad-hour skip: hora 10h, 16h, 17h filtradas por ATR alto    |
//+------------------------------------------------------------------+
...
// +++ v8.14 Quality Filters +++
input int    MinBarsBetweenTrades        = 3;    // min bars cooldown after any trade closes (reduces overtrading)
input bool   Enable_Candle_Filter        = true; // entry only in direction of current candle body
input bool   Enable_Quality_Gate        = true; // require 2-of-3 confirmators (Stoch+RSI+Volume)
input double ATR_HighVol_Multiplier     = 1.5;  // if ATR > avg*this → high volatility, use Sweep-only
input int    G2_Min_Bar_Gap             = 5;    // min bars between Legacy open and G2 open
...
int      g_hATRSlow=INVALID_HANDLE; // ATR(50) for baseline comparison
...
datetime g_lastTradeCloseBar=0;   // bar when last trade closed (for cooldown)
...
// Returns average ATR over last 20 bars for volatility comparison
double GetATRAvg()
{
...
{
   double atr=GetATR(), avg=GetATRAvg();
   return (atr > avg * ATR_HighVol_Multiplier);
}
...
// Returns direction of current candle body: 1=bullish, -1=bearish, 0=doji
int GetCandleDir()
{
   double o=iOpen(g_sym,_Period,1);
   double c=iClose(g_sym,_Period,1);
   double atr=GetATR();
   double bodyMin=atr*0.1; // ignore doji candles
   if(c-o > bodyMin) return 1;
   if(o-c > bodyMin) return -1;
   return 0;
}
...
void UpdateSwings()
{
   ...
   g_swingScanBar=bt;
   int lb=20; double hi=0, lo=DBL_MAX;
   for(int i=1;i<=lb;i++)
...
void ScanFVGs()
{
   ...
   g_fvgCnt=0; ArrayResize(g_fvg,100);
...
//============================================================
// QUALITY GATE — requires 2 of 3 confirmators
// stoch: 1pt | rsi: 1pt | volume: 1pt → need >=2 to trade
//============================================================
bool PassesQualityGate(int dir)
{
   if(!Enable_Quality_Gate) return true;
   int score=0;
   // Stoch
   double kb[]; ArraySetAsSeries(kb,true);
   if(CopyBuffer(g_hStoch,0,1,1,kb)>=1)
   { if(dir==1 && kb[0]<=Stoch_Buy_Level+10)  score++; if(dir==-1 && kb[0]>=Stoch_Sell_Level-10) score++; }
   // RSI
   double rsi=GetRSI();
   if(dir== 1 && rsi < RSI_Buy_Threshold)  score++;
   if(dir==-1 && rsi > RSI_Sell_Threshold) score++;
   // Volume
   if(IsVolumeOK()) score++;
   return (score >= 2);
}
...
// CANDLE BODY FILTER
//============================================================
bool PassesCandleFilter(int dir)
{
   if(!Enable_Candle_Filter) return true;
   int cd=GetCandleDir();
   if(cd==0) return true;  // doji → allow
   return (cd==dir);
}

//============================================================
// COOLDOWN CHECK — bars since last trade closed
//============================================================
bool IsCooledDown()
{
   if(MinBarsBetweenTrades<=0) return true;
   if(g_lastTradeCloseBar==0) return true;
   datetime curBar=iTime(g_sym,_Period,0);
   // Count bars between last close bar and current bar
   int barsSince=0;
   for(int i=0;i<MinBarsBetweenTrades+2;i++)
   {
      datetime bt=iTime(g_sym,_Period,i);
      if(bt<=g_lastTradeCloseBar) break;
      barsSince++;
   }
   return (barsSince >= MinBarsBetweenTrades);
}

// Track last close bar from stealth closes
void UpdateLastCloseBar()
{
   // Check if any of our stealth positions just closed
   // We detect by checking stealth array for newly-inactive entries
   // (called after StealthCheckAll)
}

//============================================================
// MASTER SIGNAL — 3 modes + quality gate + candle filter
//============================================================
int GetSignal()
{
   ...
   // High volatility: force Sweep-only (more precise entries)
   ...
   else if(strat==2)
   {
      ...
      if(trend!=0 && sig!=trend) return 0;
      if(sig== 1 && k1>Stoch_Sell_Level) return 0;
      if(sig==-1 && k1<Stoch_Buy_Level)  return 0;
   }
   else // Hybrid
   {
      if(Enable_Stop_Hunt_Strategy) { int s=GetSweep(); if(s!=0&&(trend==0||s==trend)) sig=s; }
      if(sig==0 && Enable_FVG_Strategy)
      {
         if(InFVG(1)  && k1<=Stoch_Buy_Level+10  && (trend==0||trend== 1)) sig= 1;
         if(InFVG(-1) && k1>=Stoch_Sell_Level-10 && (trend==0||trend==-1)) sig=-1;
      }
      if(sig==0)
      {
         bool sBuy=(k1<=Stoch_Buy_Level), sSell=(k1>=Stoch_Sell_Level);
         ...
         { int b=GetBreakout(); if(b==1&&sBuy&&(trend==0||trend==1)) sig=1; else if(b==-1&&sSell&&(trend==0||trend==-1)) sig=-1; }
         if(sig==0)
         { if(sBuy&&(trend==0||trend==1)&&!sSell) sig=1; else if(sSell&&(trend==0||trend==-1)&&!sBuy) sig=-1; }
      }
   }

   if(sig==0) return 0;

   // Apply quality gate (2-of-3 confirmators)
   if(!PassesQualityGate(sig)) return 0;

   // Apply candle body filter
   if(!PassesCandleFilter(sig)) return 0;
...
// Returns bar index when last trade closed (for cooldown calculation)
void RefreshLastCloseBar()
{
   datetime ds=StringToTime(TimeToString(TimeCurrent(),TIME_DATE));
   ...
   for(int i=HistoryDealsTotal()-1;i>=0;i--)
   {
      ...
      if(t>lastClose) lastClose=t;
   }
   if(lastClose>0)
   {
      // Find which bar this close happened in
      for(int i=0;i<50;i++)
      {
         datetime bt=iTime(g_sym,_Period,i);
         if(bt<=lastClose) { g_lastTradeCloseBar=bt; break; }
      }
   }
}
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      if(cls && StealthClosePos(g_sp[i].ticket,g_sp[i].dir))
      { g_sp[i].active=false; anyClose=true; g_lastTradeCloseBar=iTime(g_sym,_Period,0); }
   }
}
...
void CheckG2()
{
   ...
   { ulong tk=PositionGetTicket(i); if(!PositionSelectByTicket(tk)) continue; if(PositionGetInteger(POSITION_MAGIC)!=g_magic) continue; if(StringFind(PositionGetString(POSITION_COMMENT),"[Legacy]")<0) continue;
     datetime t=(datetime)PositionGetInteger(POSITION_TIME); if(ot==0||t<ot){ot=t;ltk=tk;ld=(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY)?1:-1;} }
   if(ot==0||ltk==0||ltk==g_g2ForLegacyTicket) return;
   if((TimeCurrent()-ot)<MM7_G2_MIN_GAP_S || g_lastG2OpenTime==TimeCurrent()) return;

   // v8.14: G2 bar gap — G2 must be at least G2_Min_Bar_Gap bars after Legacy open
   int barsOpen=0;
   for(int i=0;i<G2_Min_Bar_Gap+10;i++)
   { datetime bt=iTime(g_sym,_Period,i); if(bt<=ot){break;} barsOpen++; }
   if(barsOpen < G2_Min_Bar_Gap) return;
...
void DailyReset()
{
   ...
   g_lastSignalBarTime=0; g_fvgLastScan=0; g_fvgCnt=0; g_lastTradeCloseBar=0;
}
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.14 ]",clrGold,      0*lh,x,y,co,fn);
   ...
   DashLbl("7","ATR  : "+DoubleToString(GetATR(),2)+" avg:"+DoubleToString(GetATRAvg(),2),clrGray,7*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   RefreshLastCloseBar();
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.14 | Mode=",ms," | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED," G2SL=",MM7_G2_SL_FIXED,
         " | CooldownBars=",MinBarsBetweenTrades," QualityGate=",Enable_Quality_Gate,
         " | CandleFilter=",Enable_Candle_Filter," G2MinBarGap=",G2_Min_Bar_Gap);
...
void OnTick()
{
   ...
   if(CountLegacyOpen()==0)
   {
      ...
      // Cooldown: wait MinBarsBetweenTrades bars after last trade closed
      if(!IsCooledDown()){DrawDashboard();return;}
      int sig=GetSignal();
