//+------------------------------------------------------------------+
//|  MoneyMachine7_v1747.mq5                                        |
//|  v17.47 — SOLO SELL en todas las horas positivas               |
//|                                                                  |
//|  DIAGNOSTICO DEFINITIVO v17.46:                                 |
//|  30 trades | WR=50% | Net=+$497                                 |
//|  BUY: 13 trades, WR=46%, net=-$90 → DESTRUYE capital           |
//|  SELL: 17 trades, WR=53%, net=+$587 → GENERA capital           |
//|                                                                  |
//|  BUY avgWin=$26 vs SELL avgWin=$89 (3.3x mayor)                |
//|  BUY avgLoss=-$36 vs SELL avgLoss=-$27                         |
//|                                                                  |
//|  SIMULATION: 0h+2h SELL ONLY → WR=55%, net=$607 en 20 trades  |
//|  vs actual $497 en 30 trades (incluyendo BUY destructivos)     |
//|                                                                  |
//|  LOGICA NUEVA:                                                   |
//|  XAUUSD estaba en tendencia bajista Mar 2-17.                  |
//|  Mean-reversion desde TOP del rango = SELL con viento a favor  |
//|  Mean-reversion desde BOTTOM del rango = BUY contra tendencia  |
//|                                                                  |
//|  v17.47: SOLO SELL en las mejores horas del historico:         |
//|  0h(SELL avgWin=$62), 2h(SELL), 7h(SELL), 15h(SELL), 19h(SELL)|
//|  + trailing progresivo 4pts + partial 35% TP                   |
//|  + BUY completamente desactivado                               |
//|                                                                  |
//|  CRECIMIENTO EXPONENCIAL: el lot sube con el balance            |
//|  y cada SELL exitoso amplifica el siguiente                     |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.47"
#property strict
...
input double Trend_Min_Move      = 5.0;  // Bloquear SELL solo si tendencia alcista muy fuerte
...
// Horas activas para SELL (las que históricamente generan beneficio en SELL)
// 0h: SELL avgWin=$62 | 2h: SELL net=$274 | 7h: SELL positivo
// 15h: SELL fuerte | 19h: SELL positivo | 22h: SELL positivo
input bool   Use_Hour_Filter     = true;

// Lotaje basado en riesgo con compounding
input bool   Use_Risk_Based_Lot  = true;
input double Risk_Pct_Per_Trade  = 0.020;  // 2% — compounding agresivo
input double Max_Risk_USD        = 100.0;  // Cap subido a $100 (solo SELL = más predecible)
...
input int    InpMagicNumber      = 174700;
...
bool IsGoodHour(int hour)
{
   ...
   // Solo SELL → horas donde SELL es positivo históricamente
   switch(hour) {
...
void OpenOrder(double sl_dist)
{
   ...
   double tp=NormalizeDouble(entry-tp_d,digs);   // SELL: TP abajo
   double sl=NormalizeDouble(entry+sl_dist,digs); // SELL: SL arriba
   ...
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED){
      ...
      Print("MM7 SELL entry=",entry," SL=",sl," TP=",tp," lot=",lot);
...
void MoveSL(ulong ticket, double newSL)
{
   ...
   if(g_openDir==-1 && newSL>=curSL) return;  // SELL: SL solo baja
   MqlTradeRequest req={}; MqlTradeResult res={};
...
int OnInit()
{
   ...
   Print("MM7 v17.47 | ",g_sym," | SOLO SELL",
...
void OnTick()
{
   ...
   // GESTIÓN POSICION ABIERTA (siempre SELL)
   ulong ticket=GetOpenTicket();
   if(ticket>0 && g_openEntry>0){
      // Para SELL: favorable = entry - ask (precio bajó desde entrada)
      double favorable=g_openEntry-ask;
      datetime now=TimeCurrent();

      // 1. TRAILING: SL sigue al precio hacia abajo (para SELL, SL baja)
      if(Use_Trail && favorable>=Trail_Start_Pts){
         double newSL=ask+Trail_Distance_Pts;  // SL a 4pts sobre el precio ask actual
         MoveSL(ticket,newSL);
         if(!g_trailActive){
            g_trailActive=true; g_timeTrailDone=true;
            Print("MM7 TRAIL ON fav=",DoubleToString(favorable,2)," newSL=",DoubleToString(newSL,2));
         }
      }

      // 2. PARTIAL al 35% del TP
         ...
         if(favorable>=g_openTPDist*Partial_Trigger_Pct){
            if(PositionSelectByTicket(ticket)){
               ...
               if(vc<cv){
                  ...
                  g_partialDone=true;
                  // Después del partial: mover SL a BE
   ...
   if(curBar!=g_lastBarTime){
      ...
      // SEÑAL: SOLO cuando precio está en TOP del rango → SELL
   ...
   // Confirmar SELL: precio debe haber bajado Confirm_Points desde el cierre
   if(g_pendingDir==-1 && ask<=g_confirmLevel)
      OpenOrder(g_pendingSL);
}
//+------------------------------------------------------------------+
