...
//|  UN SOLO CAMBIO vs v8.24 — basado en análisis del log original:  |
//|                                                                  |
//|  DIAGNÓSTICO DEFINITIVO:                                         |
//|  v8.24 tiene 302 trades/día vs 132 del original → 2.3x más      |
//|  TP/SL son CORRECTOS ($2.50/$0.77 = 250/77 pts)                  |
//|  WR 28.76% vs 50.76% → señales de baja calidad por exceso       |
//|                                                                  |
//|  ROOT CAUSE: K se queda en zona <30 o >70 → genera señales      |
//|  repetidas de baja calidad. El original solo responde al         |
//|  CRUCE FRESCO hacia la zona extrema.                             |
//|                                                                  |
//|  EL FIX: Zone_Fresh_Entry_Filter                                 |
//|  BUY solo si k_prev > Buy_Level Y k_now <= Buy_Level             |
//|  (K acaba de cruzar HACIA oversold — señal fresca)               |
//|  SELL solo si k_prev < Sell_Level Y k_now >= Sell_Level          |
//|  (K acaba de cruzar HACIA overbought — señal fresca)             |
//|                                                                  |
//|  EFECTO ESPERADO:                                                |
//|  - Trades: 302/día → ~130-160/día (como original 132)            |
//|  - WR: 28.76% → ~45-52% (como original 50.76%)                  |
//|  - TP/SL iguales: $12.50 win / $3.85 loss                        |
//+------------------------------------------------------------------+
...
input bool   Enable_Zone_Exit_Filter     = true;  // Zone Fresh Entry: solo señal en cruce FRESCO hacia zona extrema
input bool   Enable_Candle_Filter        = true;  // cuerpo de vela confirma dirección (soft: bypass en sweep/FVG)
input bool   Enable_BOS_Filter           = false; // Break of Structure (OFF=como original, no necesita BOS)
...
input int    G2_Legacy_Threshold        = 5;      // G2 cada N legacies cerrados (5=más G2, WR actual 46%)
...
input int    Regime_Lookback             = 20;     // barras para calcular régimen (20 M1 = 20 min)
input double Regime_ADX_Threshold        = 25.0;   // ADX > umbral = TRENDING, else RANGING
input bool   Enable_AdaptivePrevCandle   = true;   // body ratio adaptativo según ATR (más permisivo en calma)
...
int GetSignal()
{
   ...
   else // Hybrid (modo 1)
   {
      ...
      {
         // FVG: señal estructural, acepta K en zona ampliada (+/-10)
         // Con Zone_Fresh_Entry ON: requiere cruce fresco (más selectivo)
         // Con Zone_Fresh_Entry OFF: acepta K en zona
         bool stOKbuy, stOKsell;
         if(Enable_Zone_Exit_Filter)
         {
            stOKbuy  = (k_prev > Stoch_Buy_Level-10  && k_now <= Stoch_Buy_Level+10);
            stOKsell = (k_prev < Stoch_Sell_Level+10 && k_now >= Stoch_Sell_Level-10);
         }
         else
         {
            stOKbuy  = (k_now<=Stoch_Buy_Level+10);
            stOKsell = (k_now>=Stoch_Sell_Level-10);
         }
      ...
      // El original solo abre en CRUCE FRESCO hacia zona extrema.
      // Evidencia del log: mediana 100s entre re-aperturas.
      // Cuando TP hit → K sube de <30 a >30 (buy) → debe volver a cruzar <30.
      // Esto es el limitador natural de frecuencia del original (~132 trades/día).
      if(sig==0)
      {
         // Zone Fresh Entry Filter (ON por default en v8.25)
         // BUY:  k_prev estaba FUERA de zona (>Buy_Level) Y k_now acaba de entrar (<=Buy_Level)
         //       = cruce fresco hacia oversold = señal de calidad
         // SELL: k_prev estaba FUERA de zona (<Sell_Level) Y k_now acaba de entrar (>=Sell_Level)
         //       = cruce fresco hacia overbought = señal de calidad
         // Si Enable_Zone_Exit_Filter=false: señal si K está EN zona (cualquier barra) = más trades
         bool stochBuy, stochSell;
         if(Enable_Zone_Exit_Filter)
         {
            stochBuy  = (k_prev > Stoch_Buy_Level  && k_now <= Stoch_Buy_Level);
            stochSell = (k_prev < Stoch_Sell_Level && k_now >= Stoch_Sell_Level);
         }
         else
         {
            stochBuy  = (k_now <= Stoch_Buy_Level);
            stochSell = (k_now >= Stoch_Sell_Level);
         }
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.25 | ZoneFreshEntry=",Enable_Zone_Exit_Filter," | Mode=",ms,
         " | TP=",MM7_TP_FIXED," SL=",MM7_SL_FIXED," G2SL=",MM7_G2_SL_FIXED,
         " | Candle=",Enable_Candle_Filter," | LocalPA=",Enable_LocalPA,
