//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.29 — MECANISMO DUAL TP/SL                  |
//|                                                                  |
//|  APRENDIDO DE v8.28:                                            |
//|  WR=31%, 318 legacy/día vs 122 original (+160%)                 |
//|  H1 MA trend fix: 100% LONGS ✓ (vs 60% shorts de v8.27)        |
//|  PROBLEMA RESTANTE: bar-lock bloquea re-entradas legítimas       |
//|                                                                  |
//|  MECANISMO REAL DEL ORIGINAL (del log forense 264 trades):      |
//|  ✓ 23.6% de trades re-abren en <5s (mismo tick tras SL)        |
//|  ✓ Tras SL: K no sube → sigue en zona → señal válida inmediata  |
//|  ✓ Tras TP: precio sube +2.5pts → K sube a 40-60 → sale zona   |
//|     → espera 3-10 min para que K vuelva a <30 (natural)         |
//|  ✓ Bar-lock NO existe en original: Stoch natural controla freq  |
//|                                                                  |
//|  SOLUCIÓN v8.29: DUAL RE-ENTRY LOGIC                            |
//|  • Tras SL: g_lastSignalBar=0 → re-entry inmediata permitida   |
//|  • Tras TP: g_lastSignalBar=curOpenBar → bar-lock activo        |
//|  • StealthCheckAll distingue TP vs SL al cerrar                 |
//|  • Resultado esperado: ~244 legacy/día con ~50% WR             |
//|    (SL 49% → instant re-open + TP 51% → natural Stoch wait)    |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "8.29"
#property strict
...
void StealthCheckAll()
{
   ...
   for(int i=0;i<g_spCnt;i++)
   {
      ...
      if(g_sp[i].dir==1)
      {
         ...
         else if(g_sp[i].vSL>0 && bid<=g_sp[i].vSL){cls=true;hitTP=false;}
      }
      else
      {
         if(g_sp[i].vTP>0 && ask<=g_sp[i].vTP){cls=true;hitTP=true;}
         else if(g_sp[i].vSL>0 && ask>=g_sp[i].vSL){cls=true;hitTP=false;}
      }
      if(cls && StealthClosePos(g_sp[i].ticket,g_sp[i].dir))
      {
         g_sp[i].active=false;
         // DUAL RE-ENTRY LOGIC (mecanismo real del original):
         // Tras TP: precio se movió +2.5pts → K sube → sale zona → mantener bar-lock
         //          → el EA espera que K vuelva a <30 naturalmente (3-10 min)
         // Tras SL: precio se movió -0.77pts → K apenas cambia → sigue en zona
         //          → re-entry inmediata válida → limpiar bar-lock
         if(!hitTP)
         {
            // SL hit: clear bar-lock → allow immediate re-entry on next tick
            g_lastSignalBar = 0;
         }
         // TP hit: leave g_lastSignalBar as-is → bar-lock prevents immediate re-entry
         // Next signal only fires when new bar completes AND Stoch re-enters zone
      }
   }
}
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.29 ]",clrGold,       0*lh,x,y,co,fn);
...
void OnTick()
{
   ...
   {
      // DUAL RE-ENTRY MECHANISM (replicando comportamiento real del original):
      //
      // Tras TP: StealthCheckAll deja g_lastSignalBar intacto → bar-lock activo
      //   → solo abre en barra NUEVA completada (Stoch necesita volver a <30)
      //   → espera natural de 3-10 barras M1 como el original ✓
      //
      // Tras SL: StealthCheckAll limpia g_lastSignalBar=0 → re-entry inmediata
      //   → precio solo bajó 0.77pts → K sigue en <30 → señal válida
      //   → re-abre en el mismo tick del close (igual que el original 23.6% casos) ✓
      //
      // g_lastSignalBar==0: sin bar-lock → evalúa señal inmediatamente
      // g_lastSignalBar!=0: bar-lock → espera nueva barra completada
      datetime curBar1 = iTime(g_sym,_Period,1);
      bool barLocked = (g_lastSignalBar != 0 && curBar1 == g_lastSignalBar);
      if(barLocked){DrawDashboard();return;}

      int sig=GetSignal();
      if(sig== 1&&CountBuys() >=Max_Buy)  sig=0;
      if(sig==-1&&CountSells()>=Max_Sell) sig=0;
      if(sig==1)  {ulong tk=OpenOrder(ORDER_TYPE_BUY, CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastSignalBar=curBar1;g_lastLegacyTicket=tk;}}
      if(sig==-1) {ulong tk=OpenOrder(ORDER_TYPE_SELL,CommentOrder+" [Legacy]",Auto_SL_Ratio);if(tk>0){g_lastSignalBar=curBar1;g_lastLegacyTicket=tk;}}
   }
   DrawDashboard();
}
