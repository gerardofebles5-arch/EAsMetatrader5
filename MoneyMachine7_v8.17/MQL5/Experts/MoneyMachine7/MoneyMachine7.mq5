//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.17                                         |
//|  BASE: v8.15 (buen crecimiento + SMC completo)                  |
//|  PROBLEMA CORREGIDO: Win rate 30% causado por entrar WHILE K    |
...
int GetSignal()
{
   ...
   else // Hybrid (modo 1)
   {
      ...
      if(sig==0&&Enable_FVG_Strategy)
      {
         ...
         if(InFVG(1) &&stOKbuy &&(trend==0||trend== 1)){sig= 1;fromFVG=true;}
         if(InFVG(-1)&&stOKsell&&(trend==0||trend==-1)){sig=-1;fromFVG=true;}
      }

      // P3: Stoch puro + confirmadores
      if(sig==0)
      {
         int sz=GetStochZone(k_now,k_prev);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.17 | Mode=",ms," | ZoneExit=",Enable_Zone_Exit_Filter,
