//+------------------------------------------------------------------+
//|  MoneyMachine7_v1725.mq5                                        |
//|  v17.25 — PRICE ACTION MINIMALISTA                             |
//|                                                                  |
//|  MOTOR: close[1]>close[2] → BUY | close[1]<close[2] → SELL    |
//|  SESION: NY 14:30-18:30 UTC                                     |
//|  LOTAJE: dinamico (balance / 1000 * 0.01)                      |
//|  SL: 0.50pts | TP: 1.00pts | R:R 1:2                           |
//|  Sin filtros. Sin cooldown. Sin limites. Solo precio.           |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.25"
#property strict
...
input int    InpMagicNumber             = 172500;
...
// MOTOR: vela anterior subio → BUY, bajo → SELL
int GetSignal()
{
   int s = (bool)MQLInfoInteger(MQL_TESTER) ? 0 : 1;
   double cl[];
   ArraySetAsSeries(cl, true);
   if(CopyClose(g_sym, _Period, s, 3, cl) < 3) return 0;
   if(cl[0] > cl[1]) return  1;
   if(cl[0] < cl[1]) return -1;
   return 0;
}
...
void DrawDashboard()
{
   ...
   DashLbl("0", "[ MM7 v17.25 PRICE ACTION ]", clrGold, 0);
   ...
   DashLbl("4", "SL=" + DoubleToString(MM7_SL_FIXED,2) + " TP=" + DoubleToString(MM7_TP_FIXED,2) +
...
int OnInit()
{
   ...
   Print("MM7 v17.25 PA | ", g_sym, " POINT=", g_point,
