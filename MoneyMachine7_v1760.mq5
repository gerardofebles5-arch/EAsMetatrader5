...
//|  v17.60 — VERSION MAESTRA: sintesis de todo el aprendizaje      |
//|                                                                  |
...
//--- LOTAJE CON COMPOUNDING REAL
// lot = min(balance*Risk%, balance*Cap_Pct) / (SL*100)
// El cap sube CON el balance → compounding exponencial automático
input bool   Use_Risk_Based_Lot  = true;
input double Risk_Pct_Per_Trade  = 0.020; // 2% del balance en riesgo
input double Cap_Pct_Of_Balance  = 0.15;  // Cap = 15% del balance actual
// Ejemplo: balance=$30  → Cap=$4.50  → lot máx con SL=20 = 0.002
// Ejemplo: balance=$500 → Cap=$75    → lot máx con SL=20 = 0.037
// Ejemplo: balance=$5000→ Cap=$750   → lot máx con SL=20 = 0.375
...
// COMPOUNDING REAL: el cap crece con el balance
// Esto es lo que genera la curva exponencial
double CalcLot(double sl_dist)
{
   ...
   // El cap es un porcentaje del balance actual → crece con la cuenta
...
int OnInit()
{
   ...
   Print("MM7 v17.60 MASTER | ",g_sym,
         " | SELL-ONLY | Horas:2,7,15,19,22",
         " | TP=",TP_Ratio,"x SL[",SL_Min,"-",SL_Max,"]",
         " | Trail=",Trail_Start_Pts,"→",Trail_Distance_Pts,"pts",
         " | TimeTrail=",Time_Trail_Sec/3600.0,"h",
         " | Risk=",Risk_Pct_Per_Trade*100,"% Cap=",Cap_Pct_Of_Balance*100,"%bal",
