//+------------------------------------------------------------------+
//|                     XAUUSD_V10_1_SUSTAINABLE.mq5                 |
//|          V10.1: SOSTENIBLE - RR flexible + Win Rate alto         |
//+------------------------------------------------------------------+
#property copyright "V10.1 - Sustainable Growth"
...
// Risk - RR MÁS CONSERVADOR para mayor Win Rate
input double SL_ATR_Multiplier = 1.2;
input double TP_ATR_Multiplier = 1.8;  // RR 1:1.5 (más sostenible)
input int ATR_Period = 14;

input bool Use_Break_Even = true;
input double BreakEven_Start_ATR = 1.0;  // Más rápido
input double BreakEven_Profit_ATR = 0.3;

input bool Use_Trailing_Stop = true;
input double Trailing_Start_ATR = 1.5;  // Más rápido
...
int OnInit()
{
    Print("V10.1 - SUSTAINABLE GROWTH");
    Print("RR 1:1.5 - Mayor Win Rate");
...
void OnTick()
{
    ...
    if(Use_Trailing_Stop && PositionSelect(_Symbol)) ManageTrailingStop();
    if(Use_Break_Even && PositionSelect(_Symbol)) ManageBreakEven();
}
...
void CheckForEntry()
{
    ...
    if(uptrend)
    {
        ...
        // FILTRO ADICIONAL: Confirmar momentum
        bool momentum_ok = (close[0] > close[2]);
        
        if(pullback && bounce && trend_intact && momentum_ok)
        {
            Print("LONG: Uptrend + Pullback + Momentum");
    ...
    if(downtrend)
    {
        ...
        if(pullback && bounce && trend_intact && momentum_ok)
        {
            Print("SHORT: Downtrend + Pullback + Momentum");
...
void ExecuteTrade(ENUM_ORDER_TYPE orderType, double atr_value)
{
    ...
    {
        g_tradesExecuted++;
        Print("Trade #", g_tradesExecuted, " RR 1:1.5");
    }
}
...
void OnDeinit(const int reason)
{
    ...
    Print("V10.1 FINALIZADO - Trades: ", g_tradesExecuted);
}
//+------------------------------------------------------------------+
