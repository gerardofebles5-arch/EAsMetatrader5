//+------------------------------------------------------------------+
//|                                            MoneyMachine7.mq5    |
//|  FORENSIC REPLICA v8.27 — EL MECANISMO REAL DEL ORIGINAL        |
//|                                                                  |
//|  ROOT CAUSE v8.26: 802 legacy/dia vs 132 original = 6x exceso  |
//|  Throttle 3s NO es suficiente — señal repite en cada tick       |
//|  Stoch usa bar[1] (mismo valor todos los ticks) sin bar-lock    |
//|  → misma señal fires ~200 veces por barra de 60s               |
//|                                                                  |
//|  DATOS LOG ORIGINAL (264 trades):                               |
//|  ✓ 8.2 trades/hora, median re-open 100s (~1.7 barras M1)       |
//|  ✓ 9.4% tiempo en trade, 90.6% flat                            |
//|  ✓ G2 median age: 31s (siguiente barra completa en ~30-60s)    |
//|                                                                  |
//|  v8.27 = v8.26 + UN SOLO CAMBIO CRÍTICO:                       |
//|  BAR-LEVEL LOCK: 1 señal por barra M1 cerrada máximo           |
//|  g_lastSignalBar = datetime de bar[1] cuando última señal abrió |
//|  Nueva señal solo si iTime(bar[1]) != g_lastSignalBar           |
//|  → máx 1/minuto → 8-10/hora real → ~132/día como original      |
//|  G2_Min_Seconds = 30 (ajustado al median de 31s del original)  |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "8.27"
#property strict
...
void DrawDashboard()
{
   ...
   DashLbl("0","[ MoneyMachine7 v8.27 ]",clrGold,       0*lh,x,y,co,fn);
...
int OnInit()
{
   ...
   string ms=(InpStrategy==0)?"Sweep":(InpStrategy==1)?"Hybrid":"Breakout";
   Print("MM7 v8.27 | BAR-LOCK | Mode=",ms,
