//+------------------------------------------------------------------+
//|                                  QUANTUM_GENIUS_EA.mq5           |
//|           Sistema Ultra-Inteligente con Confluence + Partials   |
//|           Market Regime + Order Flow + Adaptive + Confluence    |
//+------------------------------------------------------------------+
#property copyright "Quantum Genius EA"
...
input int    InpMaxTradesPerDay = 2;         // Max trades/día (CALIDAD > CANTIDAD)
...
input int    InpMinPatternScore = 70;        // Score mínimo del patrón
input bool   InpRequirePatternConfirmation = true; // Requiere confirmación
...
input double InpMinPatternWinRate = 50.0;    // Win rate mínimo del patrón
input bool   InpUseIntelligentScoring = true; // Scoring inteligente
input int    InpMinIntelligentScore = 85;    // Score inteligente mínimo (AUMENTADO)
...
input int    InpMaxHoursInTrade = 2;           // Máx horas sin alcanzar 50% TP
input double InpMinADX = 15.0;                 // ADX mínimo para operar
input double InpMinVolatility = 0.7;           // Volatilidad mínima (ATR ratio)
input double InpMaxVolatility = 2.0;           // Volatilidad máxima (ATR ratio)
input double InpMinOrderFlowBuy = 1.2;         // Imbalance mínimo para compra
input double InpMaxOrderFlowSell = 0.83;       // Imbalance máximo para venta

input group "═══ MEJORAS AVANZADAS ═══"
input bool   InpUseConfluenceScoring = true;   // Scoring por confluencia
input int    InpMinConfluenceFactors = 5;      // Factores mínimos (de 10)
...
input bool   InpUseCorrelationFilter = true;   // Filtro de correlación
...
int OnInit()
{
   ...
   Print("╔═══════════════════════════════════════════════════════════╗");
   Print("║      QUANTUM GENIUS EA - Sistema Ultra-Inteligente v3.0  ║");
   Print("╠═══════════════════════════════════════════════════════════╣");
   Print("║  Base: QUANTUM Mean Reversion                             ║");
   Print("║  MTF: ", InpUseMTF ? "ON" : "OFF", " | NY Breakout: ", InpUseNYBreakout ? "ON" : "OFF", "                      ║");
   Print("║  Akali: ", InpUseAkaliTrailing ? "ON" : "OFF", " | VWAP: ", InpUseVWAP ? "ON" : "OFF", "                    ║");
   Print("║  Structure: ", InpUseStructure ? "ON" : "OFF", " | Pivots: ", InpUsePivots ? "ON" : "OFF", "                  ║");
   Print("║  Patterns: ", InpUseCandlePatterns ? "ON" : "OFF", " | AI Learning: ", InpUseAdaptiveLearning ? "ON" : "OFF", "          ║");
   Print("╠═══════════════════════════════════════════════════════════╣");
   Print("║  ★ FILTROS INTELIGENTES:                                  ║");
   Print("║  Market Regime: ", InpUseMarketRegimeFilter ? "ON" : "OFF", " | Order Flow: ", InpUseOrderFlowFilter ? "ON" : "OFF", "            ║");
   Print("║  Adaptive Risk: ", InpUseAdaptiveRisk ? "ON" : "OFF", " | Smart Exit: ", InpUseSmartExit ? "ON" : "OFF", "              ║");
   Print("╠═══════════════════════════════════════════════════════════╣");
   Print("║  ⭐ MEJORAS AVANZADAS:                                    ║");
   Print("║  Confluence Scoring: ", InpUseConfluenceScoring ? "ON" : "OFF", " (Min: ", InpMinConfluenceFactors, "/10)          ║");
   Print("║  Dynamic SL: ", InpUseDynamicSL ? "ON" : "OFF", " | Partial Profits: ", InpUsePartialProfits ? "ON" : "OFF", "          ║");
   Print("║  Correlation Filter: ", InpUseCorrelationFilter ? "ON" : "OFF", "                            ║");
   Print("║  Score Mínimo: ", InpMinIntelligentScore, " | Max Trades/Día: ", InpMaxTradesPerDay, "              ║");
   Print("╠═══════════════════════════════════════════════════════════╣");
...
void OnTick()
{
   ...
   if(InpUseConfluenceScoring && patternSignal != 0)
   {
      ...
      if(confluenceScore < InpMinConfluenceFactors)
      {
