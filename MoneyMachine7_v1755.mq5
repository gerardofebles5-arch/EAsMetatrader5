//+------------------------------------------------------------------+
//|  MoneyMachine7_v1755.mq5                                        |
//|  v17.55 — v17.53 + filtro momentum doble para reducir falsos   |
//|                                                                  |
//|  ANALISIS DEFINITIVO:                                           |
//|  v17.53 = MEJOR ($1374, WR=61%) — es la base correcta         |
//|  v17.54 regresó porque el filtro de minuto cascadeó efectos    |
//|                                                                  |
//|  6 PÉRDIDAS GRANDES EN v17.53 (-$640 total):                   |
//|  - 3 en hora 19: hold corto (<6min) = momentum alcista fuerte  |
//|  - 2 en hora 0:  hold largo = mercado choppy nocturno          |
//|  - 1 en hora 2:  hold largo                                    |
//|                                                                  |
//|  PATRON COMUN: el precio está en top del rango PERO el mercado |
//|  tiene momentum alcista fuerte en ese momento                   |
//|  → el SELL falla porque el mercado sigue subiendo              |
//|                                                                  |
//|  FIX v17.55 — DOBLE FILTRO MOMENTUM:                          |
//|                                                                  |
//|  1. Filtro de barra previa: la barra M1 anterior debe ser      |
//|     BAJISTA (close < open) para confirmar inicio de reversión  |
//|     Si la barra previa es alcista = el precio sigue subiendo   |
//|     No entrar en SELL                                           |
//|                                                                  |
//|  2. Filtro de velocidad ajustado: bajar Vel_Threshold de 5→4  |
//|     Detecta momentum más moderado y bloquea más entradas       |
//|     cuando hay impulso fuerte reciente                          |
//|                                                                  |
//|  3. Trend_Min_Move: bajar de 5→4pts                            |
//|     Más sensible a tendencias alcistas que bloquean SELL       |
//|                                                                  |
//|  TODO LO DEMAS: IDENTICO A v17.53 (la versión ganadora)       |
//+------------------------------------------------------------------+
#property copyright "MoneyMachine7"
#property version   "17.55"
#property strict
...
input double Vel_Threshold       = 4.0;   // ← BAJADO: 5→4 (más estricto)
input int    Trend_Bars          = 15;
input double Trend_Min_Move      = 4.0;   // ← BAJADO: 5→4 (bloquea antes)

// NUEVO: Filtro de barra previa bajista
input bool   Use_PrevBar_Filter  = true;  // ← NUEVO
// Si la barra M1 anterior es alcista (close>open) → NO hacer SELL
// Confirma que la reversión ya comenzó
...
input int    InpMagicNumber      = 175500;
...
// NUEVO: verifica que la barra anterior sea bajista (cierre < apertura)
// Confirma inicio de reversión antes de entrar en SELL
bool PrevBarIsBearish()
{
   double c1=iClose(g_sym,_Period,1), o1=iOpen(g_sym,_Period,1);
   if(c1==0||o1==0) return false;
   return (c1 < o1);  // barra bajista = cierre por debajo de apertura
}
...
int OnInit()
{
   ...
   Print("MM7 v17.55 | SELL ONLY | Horas:0,2,7,15,19,22",
         " | Trail start=",Trail_Start_Pts,"pts dist=",Trail_Distance_Pts,"pts",
         " | PrevBarFilter=",Use_PrevBar_Filter,
...
void OnTick()
{
   ...
   if(curBar!=g_lastBarTime){
      ...
      // Filtro de velocidad (más estricto: 4 en vez de 5)
      ...
      if(closeNow>=upperZone && !TrendBlocksSell()){
         // FILTRO BARRA PREVIA: solo entrar en SELL si la barra M1 anterior es bajista
         // Confirma que la reversión ya inició — el precio ya empezó a bajar
         if(Use_PrevBar_Filter && !PrevBarIsBearish()) return;
